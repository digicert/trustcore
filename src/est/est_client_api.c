        ...
        ubyte4 *pCertificateLen)
{
    ...
    if ((akt_rsa == keyType) || (akt_ecc == keyType) || (akt_ecc_ed == keyType) || (akt_qs == keyType) ||
       (akt_tap_rsa == keyType) || (akt_tap_ecc == keyType) || (akt_tap_qs == keyType))
    {
    ...
    void *pEvalFunctionArg)
{
    ...
    if ((akt_rsa == currentKeyType) || (akt_ecc == currentKeyType) || (akt_ecc_ed == currentKeyType) || (akt_qs == currentKeyType) ||
        (akt_tap_rsa == currentKeyType) || (akt_tap_ecc == currentKeyType) || (akt_tap_qs == currentKeyType))
    {
        if (NULL == pCurrentKey)
        {
            /* Find the key in the cert store by its alias */
    ...
    void *pEvalFunctionArg)
{
    ...
    if (requestType == RENEW || requestType == REKEY || requestType == ENROLL)
    {
        ...
        /* Check if the key is found in cert store
         */
        if (NULL == pRetIdentityKey)
        {
            status = ERR_NOT_FOUND;
...
MSTATUS EST_calculateSubjectKeyIdentifier(MOC_ASYM(hwAccelDescr hwAccelCtx) AsymmetricKey *pAsymKey, ubyte **pPSubjectKeyId, ubyte4 *pSubjectKeyIdLen)
{
    ...
    TAP_PQCPublicKey *pPqcTapPub = NULL;
#endif
#endif
    ...
    *(pRet+1) = 0x14;
    ...
    switch ( pAsymKey->type)
    {
        ...
        case akt_qs:
        {
            status = CRYPTO_INTERFACE_QS_getPublicKeyAlloc(pAsymKey->pQsCtx, &ptBuffer, &ptBufferLen);
            if (OK != status)
                goto exit;

            status = SHA1_completeDigest( MOC_HASH(hwAccelCtx) ptBuffer, ptBufferLen, pRet+2);
            if (OK != status)
                goto exit;
        ...
        case akt_tap_qs:
        {
            status = CRYPTO_INTERFACE_QS_getTapKey(
                pAsymKey->pQsCtx, &pTapKey);
            if (OK != status)
                goto exit;

            pPqcTapPub = (TAP_PQCPublicKey *)(&(pTapKey->keyData.publicKey.publicKey.pqcKey));

            /* serial number -> generated by SHA-1 hash of the RSA key modulus */
            if (OK > (status = CRYPTO_INTERFACE_SHA1_completeDigest(MOC_HASH(hwAccelCtx)
                            (ubyte *)pPqcTapPub->pPublicKey,
                             pPqcTapPub->publicKeyLen, pRet+2)))
            {
                goto exit;
            }
