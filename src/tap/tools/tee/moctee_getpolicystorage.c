/**
 @file moctee_getpolicystorage.c

 @page moctee_getpolicystorage
 ...
 @endhtmlonly
 This tool reads data from secure storage.

 # SYNOPSIS
 `moctee_getpolicystorage [options]`

 # DESCRIPTION
 <B>moctee_getpolicystorage</B> This tool reads data from secure storage.
...
@verbatim
    ...
    --odf=[output data file]
         Output file that will contain the output data. If ommitted the data will be
...
void printHelp()
{
    LOG_MESSAGE("moctee_getpolicystorage: Help Menu\n");
    LOG_MESSAGE("This tool reads data from secure storage.");
    ...
    LOG_MESSAGE("                Output file that will contain the output data. If ommitted the data will be\n");
    LOG_MESSAGE("                written to standard output.\n");
#ifdef __ENABLE_TAP_REMOTE__
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    status = TAP_getPolicyStorage(pTapContext, NULL, &objInfo, NULL, &recData, &errContext);
    if (OK != status)
    {
        LOG_ERROR("TAP_getPolicyStorage failed with error %d", status);
        goto exit;
    }
    ...
    if (pOpts->dataFileSpecified)
    {
        ...
        status = UTILS_writeFile((const char *)pOpts->dataFile.pBuffer, recData.pBuffer, recData.bufferLen);
        if (OK != status)
        {
            LOG_ERROR("Error writing output data file, status = %d\n", status);
            goto exit;
        }
    }
    else
    {
        ubyte4 i = 0;
        printf("0x");
        for (i; i < recData.bufferLen; i++)
        {
            printf("%02x", recData.pBuffer[i]);
        }
        printf("\n");
    }
...
exit:
    ...
    {
        shredMemory((ubyte **)&(recData.pBuffer), recData.bufferLen, TRUE);
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctee_getpolicystorage failed to complete successfully.*****");
