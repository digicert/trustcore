/**
 @file moctee_delpolicystorage.c

 @page moctee_delpolicystorage
 ...
 @endhtmlonly
 This tool deletes data from secure storage.

 # SYNOPSIS
 `moctee_delpolicystorage [options]`

 # DESCRIPTION
 <B>moctee_delpolicystorage</B> This tool deletes data from secure storage.
...
void printHelp()
{
    LOG_MESSAGE("moctee_delpolicystorage: Help Menu\n");
    LOG_MESSAGE("This tool deletes data from secure storage.");
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    /* We set the object Id and the tokenId as an attribute */
    ...
    attributes.pAttributeList[0].length = sizeof(pOpts->id);
    attributes.pAttributeList[0].pStructOfType = (void *)&pOpts->id;
    ...
    status = TAP_freePolicyStorage(pTapContext, NULL, &storageInfo, &errContext);
    if (OK != status)
    {
        LOG_ERROR("TAP_freePolicyStorage failed with error %d", status);
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctee_delpolicystorage failed to complete successfully.*****");
