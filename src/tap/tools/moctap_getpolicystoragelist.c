/**
 @file moctap_getpolicystoragelist.c

 @page moctap_getpolicystoragelist

 @ingroup tap_tools_tpm2_commands

 @htmlonly
 <h1>NAME</h1>
 moctap_getpolicystoragelist -
 @endhtmlonly
 This tool displays list of configured NVRAM and Persistent entity IDs.

 # SYNOPSIS
 `moctap_getpolicystoragelist [options]`

 # DESCRIPTION
 <B>moctap_getpolicystoragelist</B> This tool displays list of configured NVRAM and Persistent entity IDs.
...
void printHelp()
{
    LOG_MESSAGE("moctap_getpolicystoragelist: Help Menu\n");
    LOG_MESSAGE("This tool lists the policy store indices.");
    ...
    LOG_MESSAGE("                   Policy store read password.\n"
                "                   If not specified , well-known password is used.\n");
    return;
}
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if ( (pOpts->prNameSpecified == FALSE) || (pOpts->modIdSpecified == FALSE)) 
    {
        LOG_ERROR("One or more mandatory options --pn or --mid not specified.");
        goto exit;
    }
...
/* Discover modules */
    ...
    if (OK != status)
    {
        PRINT_STATUS("TAP_getPolicyStorageList", status);
        goto exit;
    }

    if (objectInfoList.count)
    {
        LOG_MESSAGE("%d objects configured", objectInfoList.count);
        for(i= 0 ; i < objectInfoList.count ; i++)
        {
            LOG_MESSAGE("0x%08x", (unsigned int)objectInfoList.pInfo[i].objectId);
        }
    }
    else
        LOG_MESSAGE("No objects configured");
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_getpolicystoragelist failed to complete successfully.*****");
