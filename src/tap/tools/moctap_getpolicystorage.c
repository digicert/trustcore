/**
 @file moctap_getpolicystorage.c

 @page moctap_getpolicystorage

 @ingroup tap_tools_tpm2_commands

 @htmlonly
 <h1>NAME</h1>
 moctap_getpolicystorage.c -
 @endhtmlonly
 Reads the contents at the specified index of the NVRAM in the TPM.

 # SYNOPSIS
 `moctap_getpolicystorage [options]`

 # DESCRIPTION
 <B>moctap_getpolicystorage</B> This tool reads the contents at the specified index of the policy store in the Security Module.
...
@verbatim
        ...
        --pspwd=[policy storage password]
            NVRAM read password. If not specified, well-known password is used.
        --psidx=[policy storage index]
            (Mandatory) Index of the NVRAM to read. Hex values must be prefixed with a "0x" 
        --pssize=[number of policy storage bytes]
            Number of NVRAM bytes to read
        --odf=[output data file]
            (Mandatory) Output file that will contains the NVRAM index.
@endverbatim
...
typedef struct {
    ...
    byteBoolean nvOffsetSpecified;
    ubyte4 nvOffset;

    byteBoolean nvSizeSpecified;
    ubyte4 nvSize;

    byteBoolean outNVFileSpecified;
    TAP_Buffer outNVFile;
...
void printHelp()
{
    LOG_MESSAGE("moctap_getpolicystorage: Help Menu\n");
    LOG_MESSAGE("This tool reads the contents at the specified index of the policy store in the Security Module.");
    ...
    LOG_MESSAGE("                   Policy store read password. If not specified, well-known password is used.\n");
    LOG_MESSAGE("           --psidx=[policy store index]");
    LOG_MESSAGE("                   (Mandatory) Index of the policy store to read. Hex values must be prefixed with \"0x\". \n");
    LOG_MESSAGE("           --pssize=[number of policy store bytes]");
    LOG_MESSAGE("                   Number of policy store bytes to read.\n");
    LOG_MESSAGE("           --odf=[output data file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains contents at the policy store index.\n");
    return;
}
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    const struct option options[] = {
            ...
            {"of", required_argument, NULL, 7},
            {"pssize", required_argument, NULL, 8},
#ifndef __ENABLE_TAP_REMOTE__
            {"conf", required_argument, NULL, 9},
#endif
            {"pn", required_argument, NULL, 10},
            {"mid", required_argument, NULL, 11},
            {"cred", required_argument, NULL, 12},
            {NULL, 0, NULL, 0},
    };
    MSTATUS status;
    sbyte4 filenameLen ;
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 4:
                ...
                if ((pOpts->authValue.bufferLen == 0) || ('-' == optarg[0]))
                {
                    LOG_ERROR("--pspwd Policy storage password not specified");
                    goto exit;
                }

                if (pOpts->authValue.bufferLen > MAX_PASSWORD_LEN)
                {
                    LOG_ERROR("Policy storage password too long. Max length: %d characters",
                            MAX_PASSWORD_LEN);
                    goto exit;
                }
            ...
            case 5:
                ...
                else
                {
                    ...
                    if ((errno == ERANGE) || (endptr == optarg) || (*endptr != '\0') || (index < 0))
                    {
                        LOG_ERROR("Invalid NV index specified");
                        goto exit;
                    }
                    pOpts->nvIndex = (ubyte4)index;
                }

                MOCTAP_DEBUG_PRINT("NVRAM Index: 0x%x", pOpts->nvIndex);
                pOpts->nvIndexSpecified = TRUE;

                break;

            case 6:
                /* --odf output nvram content file */
                pOpts->outNVFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg);

                if ((pOpts->outNVFile.bufferLen == 0) ||
                    ('-' == optarg[0]))
                {
                    LOG_ERROR("-odf NV output file not specified");
                    goto exit;
                }

                if (OK != (status = DIGI_MALLOC((void **)&pOpts->outNVFile.pBuffer, 
                                pOpts->outNVFile.bufferLen + 1)))
                {
                    LOG_ERROR("Unable to allocate %d bytes for signature filename",
                            (int)(pOpts->outNVFile.bufferLen + 1));
                    goto exit;
                }
                if (OK != DIGI_MEMCPY(pOpts->outNVFile.pBuffer, optarg, 
                            pOpts->outNVFile.bufferLen))
                {
                    MOCTAP_DEBUG_PRINT_1("Failed to copy memory");
                    DIGI_FREE(&pOpts->outNVFile.pBuffer);
                    pOpts->outNVFile.bufferLen = 0;
                    goto exit;
                }
                pOpts->outNVFile.pBuffer[pOpts->outNVFile.bufferLen] = '\0';
                pOpts->outNVFile.bufferLen++;
                MOCTAP_DEBUG_PRINT("NVRAM output filename: %s", pOpts->outNVFile.pBuffer);
                pOpts->outNVFileSpecified = TRUE;

                break;

            case 7:
                /* --of NVRAM index offset */
                {
                    char *endptr;
                    long offset;
                    errno = 0;
                    offset = strtol(optarg, &endptr, 10);
                    if ((errno == ERANGE) || (endptr == optarg) || (*endptr != '\0') || (offset < 0))
                    {
                        LOG_ERROR("-of Invalid NV offset specified");
                        goto exit;
                    }
                    pOpts->nvOffset = (ubyte4)offset;
                    MOCTAP_DEBUG_PRINT("NVRAM Read Offset: %d", pOpts->nvOffset);
                    pOpts->nvOffsetSpecified = TRUE;
                }

                break;

            case 8:
                /* --pssize NVRAM read size */
                {
                    char *endptr;
                    long size;
                    errno = 0;
                    ...
                    {
                        LOG_ERROR("-pssize Invalid or no NV size specified");
                        goto exit;
                    }
                    pOpts->nvSize = (ubyte4)size;
                    MOCTAP_DEBUG_PRINT("NVRAM Read size: %d", pOpts->nvSize);
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    TAP_AttributeList getAttributes = { 1, &keyAttribute};
    ...
    if (!pOpts->nvIndex || !pOpts->outNVFileSpecified 
        || (pOpts->prNameSpecified == FALSE) || (pOpts->modIdSpecified == FALSE)) 
    {
        LOG_ERROR("One or more mandatory options --id,  --op, --pn, --mid not specified.");
        goto exit;
    }
 ...
 /* Discover modules */
    ...
    /* Read */
    status = TAP_getPolicyStorage(pTapContext, pEntityCredentials, pObjectInfo,
                  &getAttributes, &nvOut, pErrContext);

    if (OK != status)
    {
        LOG_ERROR("Error reading NVRAM, status = %d\n", status);
        goto exit;
    }
    else
    {
        /* Save output file */
        status = DIGICERT_writeFile((const char *)pOpts->outNVFile.pBuffer,
                nvOut.pBuffer, nvOut.bufferLen);
        if (OK != status)
        {
            LOG_ERROR("Error writing NVRAM content to file, status = %d\n", status);
            goto exit;
        }
...
freeOptions(cmdLineOpts *pOpts)
{
    if (pOpts)
    {
        ...
        if(pOpts->outNVFile.pBuffer)
            shredMemory((ubyte **)&(pOpts->outNVFile.pBuffer), pOpts->outNVFile.bufferLen, TRUE);
...
int main(int argc, char *argv[])
{
    ...
    {
        MOCTAP_DEBUG_PRINT_1("Failed to read NVRAM index");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_getpolicystorage failed to complete successfully.*****");
