/**
 @file moctpm2_stirrandom.c

 @page digicert_tpm2_stirrandom
 ...
 @endhtmlonly
 Add entropy to the random bit generator using a TPM 2.0 key.

 # SYNOPSIS
 `digicert_tpm2_stirrandom [options]`

 # DESCRIPTION
 <B>digicert_tpm2_stirrandom</B> Add entropy to the random bit generator
...
@verbatim
    ...
    --size=[number of bytes to stir]
        (Mandatory) Number of bytes to be used to stir the RNG.
    --idf=[input data file]
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_stirrandom: Help Menu\n");
    LOG_MESSAGE("Add entropy to the random bit generator");
    ...
    LOG_MESSAGE("                   (Mandatory) Number of bytes to be used to stir the RNG.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        case 5:
            /* --size number of bytes to b */
                pOpts->stirSizeSpecified = TRUE;
                pOpts->stirSize = DIGI_ATOL((const sbyte *)optarg, NULL);
                if (pOpts->stirSize == 0)
                {
                    LOG_ERROR("--size Invalid or no size of number of bytes is specified");
                    goto exit;
                }
                if (pOpts->stirSize > MAX_STIR_SIZE)
                {
                    LOG_ERROR("--size Current supported max size is %d", MAX_STIR_SIZE);
                    goto exit;
                }

                TPM2_DEBUG_PRINT("Number of bytes to be stirred: %d", pOpts->stirSize);
             break;

        case 6:
            /* --idf input entropy data file */
            pOpts->entropyDataFileSpecified = TRUE;
            pOpts->entropyDataFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

            if ((pOpts->entropyDataFile.bufferLen == 1) ||
                ('-' == optarg[0]))
            {
                LOG_ERROR("--idf Input entropy data file not specified");
                goto exit;
            }
            /* Validate entropy data file path length */
            if (pOpts->entropyDataFile.bufferLen > FILE_PATH_LEN)
            {
                LOG_ERROR("Entropy data file path too long. Max size: %d bytes", FILE_PATH_LEN);
                goto exit;
            }
            ...
            if (OK != DIGI_MEMCPY(pOpts->entropyDataFile.pBuffer, optarg, 
                        pOpts->entropyDataFile.bufferLen))
            {
                TPM2_DEBUG_PRINT_1("Failed to copy memory");
                goto exit;
            }
            TPM2_DEBUG_PRINT("Entropy data filename: %s", pOpts->entropyDataFile.pBuffer);
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (!pOpts->serverNameSpecified || !pOpts->stirSizeSpecified ||
        !pOpts->entropyDataFileSpecified )
    {
        LOG_ERROR("One of mandatory options --s, --size or --idf not specified.");
        goto exit;
    }
#else
    if (!pOpts->stirSizeSpecified || !pOpts->entropyDataFileSpecified)
    {
        LOG_ERROR("One of mandatory options --size or --idf not specified.");
        goto exit;
    }
#endif
    ...
    status = TAP_stirRandom(pTapContext, pOpts->stirSize, pRandAttributes, &entropyData, &errContext);
    if (OK != status)
    {
    ...
    LOG_MESSAGE("Successfully stirred random number.\n");
    retval = 0;
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_stirrandom failed to complete successfully.*****");
