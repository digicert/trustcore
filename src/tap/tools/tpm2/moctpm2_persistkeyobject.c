/**
 @file moctpm2_persistobject.c

 @page digicert_tpm2_persistobject

 @ingroup tap_tools_tpm2_commands

 @htmlonly
 <h1>NAME</h1>
 digicert_tpm2_persistobject -
 @endhtmlonly
 Persist object in TPM 2.0 memory.

 # SYNOPSIS
 `digicert_tpm2_persistobject [options]`

 # DESCRIPTION
 <B>digicert_tpm2_persistobject</B> Persist object in TPM 2.0
...
@verbatim
    ...
    --obj=[object file]
        Input file that containg the object to persist.
    --pid=[Persistent Key ID]
        Persistent Key ID where object is stored. Must be hex value between 0x81000000 - 0x81ffffff
@endverbatim
...
typedef struct {
    ...
    byteBoolean inObjFileSpecified;
    TAP_Buffer inObjFile;
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_persistobject: Help Menu\n");
    LOG_MESSAGE("This tool persists an existing key to TPM memory.\n");
    ...
    LOG_MESSAGE("           --obj=[input object file]");
    LOG_MESSAGE("                   Mandatory. Input file that contains the object.\n");
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 11:
                ...
                pOpts->inObjFileSpecified = TRUE;
                pOpts->inObjFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

                if (pOpts->inObjFile.bufferLen > FILE_PATH_LEN)
                {
                    LOG_ERROR("Object file path too long. Max size: %d bytes", FILE_PATH_LEN);
                    goto exit;
                }

                if (OK != (status = DIGI_MALLOC((void **)&pOpts->inObjFile.pBuffer, pOpts->inObjFile.bufferLen)))
                {
                    LOG_ERROR("Unable to allocate %d bytes for object filename",
                            (int)pOpts->inObjFile.bufferLen);
                    goto exit;
                }
                if (OK != DIGI_MEMCPY(pOpts->inObjFile.pBuffer, optarg, pOpts->inObjFile.bufferLen))
                {
                    TPM2_DEBUG_PRINT_1("Failed to copy memory");
                    goto exit;
                }
                TPM2_DEBUG_PRINT("Object filename: %s", pOpts->inObjFile.pBuffer);
            ...
            case 18:
                ...
                {
                    LOG_ERROR("Invalid --pid, cannot persist at the EK Object ID");
                    goto exit;
                }

                if (SRK_OBJECT_ID_START == pOpts->objectId)
                {
                    LOG_ERROR("Invalid --pid, cannot persist at the SRK Object");
                    goto exit;
                }
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (!pOpts->serverNameSpecified || !pOpts->inObjFileSpecified ||
        !pOpts->objectIdSpecified)
    {
        LOG_ERROR("One of mandatory option --s, --obj, --pid not specified."); 
        goto exit;
    }
#else
    if (!pOpts->inObjFileSpecified || !pOpts->objectIdSpecified)
    {
        LOG_ERROR("One of mandatory option --obj, --pid "
                "not specified.");
        printHelp();
        retval = 0;
        goto exit;
    ...
    status = DIGICERT_readFile(
        (const char *)pOpts->inObjFile.pBuffer, &keyBlob.pBuffer, &keyBlob.bufferLen);
    if (OK != status)
    {
        LOG_ERROR("Error reading the input object file, status = %d\n", status);
        goto exit;
    }

    /* Deserialize into TAPKey */
    status = TAP_deserializeKey(&keyBlob, &pTapKey, pErrContext);
    if (OK != status)
    {
    ...
    status = TAP_persistObject(pTapContext, pTapKey, &pOpts->id, pErrContext);
    if (OK != status)
    {
        PRINT_STATUS("TAP_persistObject", status);
        goto exit;
    }
...
freeOptions(cmdLineOpts *pOpts)
{
    if (pOpts)
    {
        if (pOpts->inObjFile.pBuffer)
            DIGI_FREE((void **)&pOpts->inObjFile.pBuffer);
    }
