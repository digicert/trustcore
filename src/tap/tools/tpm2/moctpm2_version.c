/**
 @file moctpm2_version.c

 @page digicert_tpm2_version
 ...
 @endhtmlonly
 Get the firmware version and manufacturer information for a TPM 2.0 chip.

 # SYNOPSIS
 `digicert_tpm2_version [options]`

 # DESCRIPTION
 <B>digicert_tpm2_version</B> This tool reports the TPM 2.0 firmware version and manufacturer information.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_version: Help Menu\n");
    LOG_MESSAGE("This tool reports the TPM 2.0 firmware version and manufacturer information.");
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    /* Invoke TAP API */
    status = TAP_getModuleVersionInfo(getTapModule(&moduleList, pOpts->moduleNum), &versionInfo,
                                      pErrContext);
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to get version");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_version failed to complete successfully.*****");
