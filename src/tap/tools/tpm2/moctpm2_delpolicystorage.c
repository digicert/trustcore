/**
 @file moctpm2_delpolicystorage.c

 @page digicert_tpm2_delpolicystorage
 ...
 # SYNOPSIS
 `digicert_tpm2_delpolicystorage [options]`

 # DESCRIPTION
 <B>digicert_tpm2_delpolicystorage</B> This tool deletes the NVRAM format at the specified index.
...
@verbatim
    ...
    --conf=[TPM 2.0 configuration file]
        Path to the TPM 2.0 module configuation file.
    --s=[server name]
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_delpolicystorage: Help Menu\n");
    LOG_MESSAGE("This tool deletes the NVRAM format at the specified index.");
    ...
    LOG_MESSAGE("                   Path to the TPM 2.0 module configuration.\n");
#endif
    ...
    LOG_MESSAGE("                   (Mandatory) Index of the NVRAM to be configured. Hex values must be prefixed with \"0x\" \n");
    LOG_MESSAGE("           --pssize=[policy storage size]");
    LOG_MESSAGE("                   (Mandatory:) Size in bytes of the NVRAM index\n");
    LOG_MESSAGE("           --pstype=[NVRAM index type]");
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 4:
                ...
                TPM2_DEBUG_PRINT("NVRAM password: %s", optarg); 
            ...
            case 6:
                ...
                TPM2_DEBUG_PRINT("NVRAM index size: %d", pOpts->nvSize);
            ...
            case 7:
                {
                    ...
                    /* Check if NV index type string length exceeds maximum allowed */
                    if(DIGI_STRLEN((const sbyte *)optarg) > FILE_PATH_LEN)
                    {
                        LOG_ERROR("--pstype NV index type string too long");
                        goto exit;
                    }
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    TAP_EntityCredentialList UsageCredentials = { 0 };
...
/*    int numCredentials = 0;
    ...
    if ( !pOpts->nvIndex )             
    {
        LOG_ERROR("storage index is mandatory parameter.");
        goto exit;
    }
...
exit:
    ...
    if (NULL != UsageCredentials.pEntityCredentials)
    {
        status = TAP_UTILS_clearEntityCredentialList(&UsageCredentials);
        if (OK != status)
            PRINT_STATUS("TAP_UTILS_clearEntityCredentialList", status);
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_delpolicystorage failed to complete successfully.*****");
