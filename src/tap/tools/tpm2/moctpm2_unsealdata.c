/**
 @file moctpm2_unsealdata.c

 @page digicert_tpm2_unsealdata
 ...
 @endhtmlonly
 Unsealed data using a TPM 2.0 chip.

 # SYNOPSIS
 `digicert_tpm2_unsealdata [options]`

 # DESCRIPTION
 <B>digicert_tpm2_unsealdata</B> unseals the sealed input data, previously sealed via <B>digicert_tpm2_sealdata</B>.
...
@verbatim
    ...
    --auth=[password]
        Authorization password to seal data.
    ...
    --idf=[input data file]
        (Mandatory) Input file name that contains the previously sealed data to unseal.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_unsealdata: Help Menu\n");
    LOG_MESSAGE("This tool unseals the sealed input data, previously sealed via digicert_tpm2_sealdata");
    LOG_MESSAGE("Options:");
    ...
    LOG_MESSAGE("                   Optional: PCR value to unseal data.\n"
                "                   Multiple PCRs may be specified with a --tdidx=option for each PCR index.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        case 7:
            ...
            {
                LOG_ERROR("Output data file path too long.");
                goto exit;
            }
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    {
        LOG_ERROR("One or more mandatory options --auth or --tdidx not specified.");
        goto exit;
    }
    ...
    {
        LOG_ERROR("One or more mandatory options --s, --idf, --odf not specified.");
        goto exit;
    }
    ...
    {
        LOG_ERROR("One or more mandatory options --idf, --odf not specified.");
        goto exit;
    }    
#endif
    ...
    LOG_MESSAGE("Successfully unsealed data\n");
    retval = 0;
...
freeOptions(cmdLineOpts *pOpts)
{
    if (pOpts)
    {
        ...
        /* Don't free dataAuthValue TAP_Buffer as it is freed by TAP_UTILS_clearCredentialList */
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to unseal data.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_unseal failed to complete successfully.*****");
