/**
 ...
 digicert_tpm2_sign -
 @endhtmlonly
 Sign data using a TPM 2.0 key.

 # SYNOPSIS
 `digicert_tpm2_sign [options]`

 # DESCRIPTION
 <B>digicert_tpm2_sign</B> signs data using the key provided.
...
@verbatim
    ...
    --halg=[hash algorithm]
        (Mandatory) Hash algorithm (sha1, sha256, sha384 or sha512) to hash the data in the input file. 
    --ss=[key signing scheme]
        (Optional) Signing scheme (pkcs1, pkcs1_sha256, pkcs1_sha384, pkcs1_sha512, ecdsa1, ecdsa256, ecdsa384, ecdsa512,
        pss256, pss384 or pss512) used during key creation.
    --pri=[input private key file]
        (Mandatory) Input file that contains the private key.
    --idf=[input data file]
        (Mandatory) Input file that contains the data to be signed.
    --osf=[output signature file]
        (Mandatory) Output file that contains the signature
@endverbatim
...
@endverbatim
 ...
 # SEE ALSO
 digicert_tpm2_createasymkey,  digicert_tpm2_createsymkey, digicert_tpm2_verify
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_sign: Help Menu\n");
    LOG_MESSAGE("This tool generates a signature using an asymmetric key, or a mac using a symmetric key.");
    ...
    LOG_MESSAGE("                   (Mandatory for asymmetric key) Hash algorithm (sha1, sha256, sha384 or sha512) to hash the data in the input file.\n");
    LOG_MESSAGE("           --ss=[key signing scheme]");
    LOG_MESSAGE("                   (Optional) Signing scheme (pkcs1, pkcs1_sha256, pkcs1_sha384, pkcs1_sha512, ecdsa1, ecdsa256, ecdsa384, ecdsa512, \n"
                "                   pss256, pss384 or pss512) used during key creation.\n");
    ...
    LOG_MESSAGE("                   (Mandatory) Input file that contains the data to be signed.\n");
    LOG_MESSAGE("           --raw [output signature in raw form]\n");
    LOG_MESSAGE("           --osf=[output signature file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains the signature.\n");
    return;
}
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    const struct option options[] = {
            ...
            {"osf", required_argument, NULL, 8},
            {"spass", required_argument, NULL, 9},
            {"hpass", required_argument, NULL, 10},
#ifndef __ENABLE_TAP_REMOTE__
            {"conf", required_argument, NULL, 11},
#endif
            {"ss", required_argument, NULL, 12},
            {"modulenum", required_argument, NULL, 13},
            {"raw", no_argument, NULL, 14},
            {NULL, 0, NULL, 0},
    };
    MSTATUS status;
    sbyte4 cmpResult;
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 4:
                {
                    ...
                    for (oIndex = 0; optionValues[oIndex].pName; oIndex++)
                    {
                                ...
                                DIGI_STRLEN((const sbyte *)optarg))
                        {
                            ...
                            if (!cmpResult)
                            {
                                ...
                                TPM2_DEBUG_PRINT("Setting digest algorithm to %s", 
                    ...
                    {
                        LOG_ERROR("--halg not sha1 or sha256 or sha384 or sha512");
                        goto exit;
                    }
                }
                break;
            ...
            case 6:
                ...
                {
                    LOG_ERROR("Private key file path too long. Max size: %d bytes", FILE_PATH_LEN);
                    goto exit;
                }
            ...
            case 7:
                ...
                {
                    LOG_ERROR("Input data file path too long. Max size: %d bytes", FILE_PATH_LEN);
                    goto exit;
                }
            ...
            case 8:
                ...
                /* Validate output signature file path length */
                if (pOpts->outSignatureFile.bufferLen > FILE_PATH_LEN)
                {
                    LOG_ERROR("Output signature file path too long. Max size: %d bytes", FILE_PATH_LEN);
                    goto exit;
                }
                ...
                TPM2_DEBUG_PRINT("Signature filename: %s", pOpts->outSignatureFile.pBuffer);
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    byteBoolean isSymKey = FALSE;
    byteBoolean isDataNotDigest = TRUE;
    TAP_Buffer dataBuffer = {0, NULL};
...
/*    int numCredentials = 0;
    ...
    TAP_HASH_ALG *pHashAlg = NULL;
    TAP_Attribute *pHashAlgAttr = NULL;
    ...
    if (!pOpts->serverNameSpecified ||  
            ...
            !pOpts->outSignatureFileSpecified) 
    {
        LOG_ERROR("One or more mandatory options --s, --p, --pri, --idf, --osf not specified.");
        goto exit;
    }
    ...
    if (!pOpts->inPrivKeyFileSpecified || !pOpts->inFileSpecified ||
        !pOpts->outSignatureFileSpecified)
    {
        LOG_ERROR("One or more mandatory options --pri, --idf, --osf not specified.");
        goto exit;
    }
#endif
    ...
    status = DIGICERT_readFile((const char *)pOpts->inFile.pBuffer, &(dataBuffer.pBuffer), &(dataBuffer.bufferLen));
    ...
    if (OK == status)
    {
        ...
        if (OK == status)
        {
            ...
            isSymKey = (TAP_KEY_ALGORITHM_HMAC == pLoadedTapKey->keyData.keyAlgorithm);

            if (!isSymKey)
            {
                if (!pOpts->hashAlgSpecified)
                {
                    LOG_ERROR("mandatory options `--halg` for asymmetric key is not specified.");
                    goto exit;
                }
                ...
                switch (pOpts->hashAlg)
                {
                    case TAP_HASH_ALG_SHA1:
                        SHA1_completeDigest(MOC_HASH(hwAccelCtx) dataBuffer.pBuffer, dataBuffer.bufferLen, digest.pBuffer);
                        digest.bufferLen = SHA1_RESULT_SIZE;
                        break;

                    case TAP_HASH_ALG_SHA512:
                        SHA512_completeDigest(MOC_HASH(hwAccelCtx) dataBuffer.pBuffer, dataBuffer.bufferLen, digest.pBuffer);
                        digest.bufferLen = SHA512_RESULT_SIZE;
                        break;

                    case TAP_HASH_ALG_SHA384:
                        SHA384_completeDigest(MOC_HASH(hwAccelCtx) dataBuffer.pBuffer, dataBuffer.bufferLen, digest.pBuffer);
                        digest.bufferLen = SHA384_RESULT_SIZE;
                        break;

                    case TAP_HASH_ALG_SHA256:
                    default:
                        SHA256_completeDigest(MOC_HASH(hwAccelCtx) dataBuffer.pBuffer, dataBuffer.bufferLen, digest.pBuffer);
            ...
            /* Sign using the correct API */
            if (isSymKey)
            {
                if (TRUE == pOpts->hashAlgSpecified)
                {
                    status = DIGI_CALLOC((void**)&pHashAlgAttr, 1, sizeof(*pHashAlgAttr));
                    if (OK != status)
                    {
                        goto exit;
                    }
                    status = DIGI_CALLOC((void**)&pHashAlg, 1, sizeof(*pHashAlg));
                    if (OK != status)
                    {
                        goto exit;
                    }
                    *pHashAlg = pOpts->hashAlg;
                    pHashAlgAttr->type = TAP_ATTR_HASH_ALG;
                    pHashAlgAttr->length = sizeof(*pHashAlg);
                    pHashAlgAttr->pStructOfType = pHashAlg;

                    status = DIGI_CALLOC((void**)&pAttrList, 1, sizeof(*pAttrList));
                    if (OK != status)
                    {
                        goto exit;
                    }
                    pAttrList->listLen = 1;
                    pAttrList->pAttributeList = pHashAlgAttr;
                }
                status = TAP_symSign(pLoadedTapKey, pEntityCredentials, pAttrList,
                                isDataNotDigest,
                                isDataNotDigest ? &dataBuffer : &digest, 
                                &signature, pErrContext);
            }
            else
            {
                ...
                status = TAP_asymSign(pLoadedTapKey, pEntityCredentials, NULL, sigScheme,
                                        isDataNotDigest, &digest, &signature, pErrContext);
            }

            if (OK != status)
            {
                PRINT_STATUS("Signing", status);
                goto exit;
            }
            else
            {
                if (pOpts->isRawSig)
                {
                    switch (pLoadedTapKey->keyData.keyAlgorithm)
                    {
                        case TAP_KEY_ALGORITHM_HMAC:
                            status = DIGI_MEMCPY(signatureBuffer, signature.signature.hmacSignature.pSignature, signature.signature.hmacSignature.signatureLen);
                            if (OK != status)
                                goto exit;
                            
                            offset = signature.signature.hmacSignature.signatureLen;
                            break;

                        case TAP_KEY_ALGORITHM_RSA:
                            status = DIGI_MEMCPY(signatureBuffer, signature.signature.rsaSignature.pSignature, signature.signature.rsaSignature.signatureLen);
                            if (OK != status)
                                goto exit;
                            
                            offset = signature.signature.rsaSignature.signatureLen;
                            break;
                        ...
                        case TAP_KEY_ALGORITHM_ECC:
                        {
                            ...
                            status = DIGI_MEMCPY(signatureBuffer + elementLen - signature.signature.eccSignature.rDataLen, 
                                                signature.signature.eccSignature.pRData, signature.signature.eccSignature.rDataLen);
                            if (OK != status)
                                goto exit;
                            
                            status = DIGI_MEMCPY(signatureBuffer + 2*elementLen - signature.signature.eccSignature.sDataLen, 
                                                signature.signature.eccSignature.pSData, signature.signature.eccSignature.sDataLen);
                            if (OK != status)
                                goto exit;
                ...
                else
                {
                    /* Serialize signature */
                    offset = 0;
                    status = TAP_SERIALIZE_serialize(&TAP_SHADOW_TAP_Signature, TAP_SD_IN, (void *)&signature, sizeof(signature), 
                                                    signatureBuffer, sizeof(signatureBuffer), 
                                                    &offset);
                    if (OK != status)
                    {
                ...
                LOG_MESSAGE("Input data signed successfully!\n");
    ...
    {
        LOG_ERROR("Error reading the input data file, status = %d\n", status);
    }
...
exit:
    ...
    if (NULL != (dataBuffer.pBuffer))
        DIGICERT_freeReadFile(&(dataBuffer.pBuffer));
    ...
    if (NULL != pAttrList)
    {
        TAP_UTILS_freeAttributeList(pAttrList);
	    DIGI_FREE((void**)&pAttrList);
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_sign failed to complete successfully.*****");
