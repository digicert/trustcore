/**
 @file moctpm2_getpolicystorage.c

 @page digicert_tpm2_getpolicystorage

 @ingroup tap_tools_tpm2_commands

 @htmlonly
 <h1>NAME</h1>
 digicert_tpm2_getpolicystorage.c -
 @endhtmlonly
 Reads the contents at the specified index of the NVRAM in the TPM.

 # SYNOPSIS
 `digicert_tpm2_getpolicystorage [options]`

 # DESCRIPTION
 <B>digicert_tpm2_getpolicystorage</B> Reads the contents at the specified index of the NVRAM in the TPM.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_getpolicystorage: Help Menu\n");
    LOG_MESSAGE("This tool reads the contents at the specified index of the NVRAM in the TPM.");
    ...
    LOG_MESSAGE("                   NVRAM read password. If not specified, well-known password is used.\n");
    LOG_MESSAGE("           --psidx=[policy storage index]");
    LOG_MESSAGE("                   (Mandatory) Index of the NVRAM to read. Hex values must be prefixed with a \"0x\" \n");
    LOG_MESSAGE("           --odf=[output data file]");
    LOG_MESSAGE("                   (Mandatory) Output file that will contains the NVRAM index.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    const struct option options[] = {
            ...
            {"authcontext", required_argument, NULL, 12},
            {NULL, 0, NULL, 0},
    };
    MSTATUS status;
    sbyte4 cmpResult = 1;
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 4:
                ...
                TPM2_DEBUG_PRINT("NVRAM read password: %s", optarg);

                break;

            case 5:
                /* --id NVRAM index value */
                ...
                TPM2_DEBUG_PRINT("NVRAM Index: 0x%x", pOpts->nvIndex);
            ...
            case 6:
                ...
                pOpts->outNVFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

                if ((pOpts->outNVFile.bufferLen == 1) ||
                    ('-' == optarg[0]))
                {
                ...
                if (pOpts->outNVFile.bufferLen > FILE_PATH_LEN)
                {
                    LOG_ERROR("NVRAM output filename too long. Max size: %d bytes",
                            FILE_PATH_LEN);
                    goto exit;
                }
                ...
                TPM2_DEBUG_PRINT("NVRAM output filename: %s", pOpts->outNVFile.pBuffer);
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    TAP_AttributeList getAttributes = { 2, keyAttribute};
...
/*    int numCredentials = 0;*/
    ...
    {
        LOG_ERROR("One or more mandatory options --psidx, --odf not specified.");
        goto exit;
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to read NVRAM index");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_getpolicystorage failed to complete successfully.*****");
