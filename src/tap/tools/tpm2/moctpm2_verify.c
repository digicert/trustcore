/**
 @file moctpm2_verify.c

 @page digicert_tpm2_verify
 ...
 @endhtmlonly
 Verify a signature using a TPM 2.0 key.

 # SYNOPSIS
 `digicert_tpm2_verify [options]`

 # DESCRIPTION
 <B>digicert_tpm2_verify</B> verifies a signature using the key provided.
...
@verbatim
    ...
    --idf=[input data file]
        (Mandatory) Input file that contains the data to be verified.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_verify: Help Menu\n");
    LOG_MESSAGE("This tool verifies the signature of the hash of data in an input file using an asymmetric key.");
    ...
    LOG_MESSAGE("                   (Mandatory) Hash algorithm (sha1, sha256, sha384 or sha512) to hash the data in the input file. \n");
    LOG_MESSAGE("           --ss=[key signing scheme]");
    LOG_MESSAGE("                    Signing scheme (pkcs1, pkcs1_sha384, pkcs1_sha512, ecdsa1, ecdsa256, ecdsa384, ecdsa512, \n"
                "                   pss256, pss384 or pss512) used to create this signing key.\n");
    LOG_MESSAGE("           --pri=[input private key file]");
    LOG_MESSAGE("                   (Mandatory) Input file name contains the private key\n");
    LOG_MESSAGE("           --idf=[input data file]");
    LOG_MESSAGE("                   (Mandatory) Input file name that contains the data to be verified.\n");
    LOG_MESSAGE("           --isf=[input signature file]");
    LOG_MESSAGE("                   (Mandatory) input file name that contains the signature.\n");
    return;
}
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        case 8:
            ...
            {
                LOG_ERROR("Input signature file path too long.");
                goto exit;
            }
            ...
            case 12:
                {
                    ...
                    if (NULL == optionValues[oIndex].pName)
                    {
                        LOG_ERROR("--ss not pkcs1, pkcs1_sha384, pkcs1_sha512, ecdsa1, ecdsa256, ecdsa384, ecdsa512, pss256, pss384 or pss512");
                        goto exit;
                    }
                }
                break;
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (!pOpts->serverNameSpecified ||  
            ...
            !pOpts->inSignatureFileSpecified) 
    {
        LOG_ERROR("One or more mandatory options --s, --pri, --idf, --isf not specified.");
        goto exit;
    }
    ...
    if (!pOpts->inPrivKeyFileSpecified || !pOpts->inFileSpecified ||
        !pOpts->inSignatureFileSpecified)
    {
        LOG_ERROR("One or more mandatory options --pri, --idf, --isf not specified.");
        goto exit;
    }
#endif
    if (!pOpts->hashAlgSpecified)
    {
        LOG_ERROR("mandatory options --halg is  not specified.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_verify failed to complete successfully.*****");
