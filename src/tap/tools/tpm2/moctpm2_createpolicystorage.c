/**
 @file moctpm2_createpolicystorage.c

 @page digicert_tpm2_createpolicystorage
 ...
 # SYNOPSIS
 `digicert_tpm2_createpolicystorage [options]`

 # DESCRIPTION
 <B>digicert_tpm2_createpolicystorage</B> This tool configures the NVRAM format at the specified index in the TPM.
...
@verbatim
    ...
    --pspwd=[policy storage password]
        ...
        If none is specified with this option, the well-known password is used.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_createpolicystorage: Help Menu\n");
    ...
    LOG_MESSAGE("                   NVRAM access authorization password.\n"
                "                   If none is specified with this option, the well-known password is used.\n");
    LOG_MESSAGE("           --psidx=[policy storage index]");
    LOG_MESSAGE("                   (Mandatory) Index of the NVRAM to be configured. Hex values must be prefixed with a \"0x\"\n");
    LOG_MESSAGE("           --pssize=[size of the policy storage index]");
    LOG_MESSAGE("                   (Mandatory) Size of data in bytes at the NVRAM index\n");
    LOG_MESSAGE("           --pstype=[policy storage index type]");
    LOG_MESSAGE("                   (Mandatory) Type of the NVRAM index (ordinary, counter, bits, extend) to configure.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    const struct option options[] = {
            ...
            {"authcontext", required_argument, NULL, 10},
            {NULL, 0, NULL, 0},
    };
    MSTATUS status;
    sbyte4 cmpResult = 1;
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 4:
                ...
                TPM2_DEBUG_PRINT("Policy storage password: %s", optarg); 
            ...
            case 5:
                ...
                TPM2_DEBUG_PRINT("Policy storage Index: %d", pOpts->nvIndex);
            ...
            case 6:
                ...
                pOpts->nvSize = DIGI_ATOL((const sbyte *)optarg, NULL);
                if (pOpts->nvSize == 0)
                {
                    LOG_ERROR("Invalid or no Policy storage size specified");
                    goto exit;
                }

                TPM2_DEBUG_PRINT("Policy Storage index size: %d", pOpts->nvSize);
            ...
            case 7:
                {
                    ...
                    if ((DIGI_STRLEN((const sbyte *)optarg) == 0) ||
                        ('-' == optarg[0]))
                    {
                        LOG_ERROR("--pstype Policy storage index type not specified");
                        goto exit;
                    }
                    /* Check length of policy storage index type string is within allowed limit */
                    if (DIGI_STRLEN((const sbyte *)optarg) > DIGI_STRLEN((const sbyte *)"ordinary"))
                    {
                        LOG_ERROR("--pstype Invalid policy storage index type. Must be ordinary, counter, bits or extend");
                        goto exit;
                    }
                    ...
                    for (oIndex = 0; optionValues[oIndex].pName; oIndex++)
                    {
                                ...
                                DIGI_STRLEN((const sbyte *)optarg))
                        {
                            ...
                            if (!cmpResult)
                            {
                                ...
                                TPM2_DEBUG_PRINT("Setting NVRAM Type to %s", 
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    {
        LOG_ERROR("One or more mandatory options --pssize, --psidx, --pstype not specified.");
        goto exit;
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to configure NVRAM index");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_createpolicystorage failed to complete successfully.*****");
