/**
 @file moctpm2_updatetrusteddata.c

 @page digicert_tpm2_updatetrusteddata
 ...
 @endhtmlonly
 Reads the TPM’s PCRs using a TPM 2.0 key.

 # SYNOPSIS
 `digicert_tpm2_updatetrusteddata [options]`

 # DESCRIPTION
 <B>digicert_tpm2_updatetrusteddata</B> This tool reads the TPM’s PCRs.
...
@verbatim
    ...
    --tdtype=[data type]
        Type of trusted data (measurement, identifier, report.)
    --tdsubtype=[subtype]
        Sub type of trusted data.Set to 1 for TPM 2.0.
    --tdidx=[PCR index]
        The PCR index to extend.
    --idf=[input data file]
        (Mandatory) Input file that contains the value to extend with (20 or 32 bytes).
    --odf=[output data file]
        (Mandatory) Output file that contains the updated (extended) trusted data
@endverbatim
...
@endverbatim
 ...
 # SEE ALSO
 digicert_tpm2_createasymkey,  digicert_tpm2_createsymkey, digicert_tpm2_decrypt
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_updatetrusteddata: Help Menu\n");
    LOG_MESSAGE("This tool extends data to the specified PCR");
    ...
    LOG_MESSAGE("                   Sub type of trusted data. Set to 1 for TPM 2.0.\n");
    LOG_MESSAGE("           --tdidx=[PCR index]");
    LOG_MESSAGE("                   The PCR index to extend.\n");    
    LOG_MESSAGE("           --idf=[input data file]");
    LOG_MESSAGE("                   (Mandatory) Input file that contains the value to extend with (20 or 32 bytes)\n");
    LOG_MESSAGE("           --odf=[output data file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains the updated (extended) trusted data\n");
    return;
}
    ...
    while (TRUE)
    {
        ...
        case 5:
            ...
            OPT_VAL_INFO dataTypeOptionValues[] = {
                        {"measurement", TAP_TRUSTED_DATA_TYPE_MEASUREMENT},
                        {"identifier", TAP_TRUSTED_DATA_TYPE_IDENTIFIER},
            ...
            if (DIGI_STRLEN((const sbyte *)optarg) > DIGI_STRLEN((const sbyte *)"measurement"))
            {
                LOG_ERROR("--tdtype Invalid data type. Must be measurement, identifier, or report");
                goto exit;
            }
            ...
            for (oIndex = 0; dataTypeOptionValues[oIndex].pName; oIndex++)
            {
                ...
                if (OK != DIGI_MEMCMP((const ubyte *)optarg, (const ubyte *)dataTypeOptionValues[oIndex].pName,
                                DIGI_STRLEN((const sbyte *)dataTypeOptionValues[oIndex].pName), &cmpResult))
                {
                ...
                if (!cmpResult)
                {
                    ...
                    TPM2_DEBUG_PRINT("Setting data type to %s", dataTypeOptionValues[oIndex].pName);
                    break;
                }
            }
            ...
            {
                 LOG_ERROR("--tdtype not measurement, identifier, or report");
                goto exit;
            }
            break;
        ...
        case 6:
            /* --tdsubtype subtype of trusted data */
                ...
                pOpts->subType = DIGI_ATOL((const sbyte *)optarg, NULL);
                if (pOpts->subType == 0)
                {
                ...
                TPM2_DEBUG_PRINT("Sub type: %d", pOpts->subType);
             break;

        case 7:
            /* --osf output signature file */
            pOpts->outTrustedDataFileSpecified = TRUE;
            pOpts->outTrustedDataFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

            if ((pOpts->outTrustedDataFile.bufferLen == 1) ||
                ('-' == optarg[0]))
            {
                LOG_ERROR("--osf Updated trusted data file not specified");
                goto exit;
            }

            /* Validate output trusted data file path length */
            if (pOpts->outTrustedDataFile.bufferLen > FILE_PATH_LEN)
            {
                LOG_ERROR("Output trusted data file path too long. Max size: %d bytes", FILE_PATH_LEN);
                goto exit;
            }
            ...
            if (OK != DIGI_MEMCPY(pOpts->outTrustedDataFile.pBuffer, optarg, 
                        pOpts->outTrustedDataFile.bufferLen))
            {
                TPM2_DEBUG_PRINT_1("Failed to copy memory");
                goto exit;
            }
            TPM2_DEBUG_PRINT("Updated trusted data filename: %s", pOpts->outTrustedDataFile.pBuffer);
        ...
        case 9:
            ...
            pOpts->inDataFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

            if ((pOpts->inDataFile.bufferLen == 1) ||
                ('-' == optarg[0]))
            {
            ...
            if (pOpts->inDataFile.bufferLen > FILE_PATH_LEN)
            {
            ...
            if (OK != DIGI_MEMCPY(pOpts->inDataFile.pBuffer, optarg, 
                        pOpts->inDataFile.bufferLen))
            {
                TPM2_DEBUG_PRINT_1("Failed to copy memory");
                goto exit;
            }
            TPM2_DEBUG_PRINT("Input data filename: %s", pOpts->inDataFile.pBuffer);
        ...
        case 10:
            /* -- trusted data key */
                ...
                pOpts->trustedDataKey = DIGI_ATOL((const sbyte *)optarg, NULL);
                if (pOpts->trustedDataKey == 0)
                {
                    LOG_ERROR("--tdidx Invalid or no trustedDataKey specified");
                    goto exit;
                }
                TPM2_DEBUG_PRINT("Trusted data key: %d", pOpts->trustedDataKey);
             break;
...
/*    int numCredentials = 0;
    ...
    if (!pOpts->serverNameSpecified || !pOpts->dataTypeSpecified ||
        ...
        !pOpts->trustedDataKeySpecified)
    {
        LOG_ERROR("One of mandatory options --s, --tdtype, --tdsubtype, --tdidx or --osf not specified.");
        goto exit;
    }
#else
    ...
    {
        LOG_ERROR("One of mandatory options --tdtype, --tdsubtype, --tdidx or --osf not specified.");
        goto exit;
    }
#endif
    ...
    status = TAP_updateTrustedData(pTapContext, pEntityCredentials, pOpts->dataType, &dataInfo,
                                   operation, &inData, &updatedData, &errContext);
    if (OK != status)
    {
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to get trusted data.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_updatetrusteddata failed to complete successfully.*****");
