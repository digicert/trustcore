/**
 @file moctpm2_encrypt.c

 @page digicert_tpm2_encrypt
 ...
 # SYNOPSIS
 `digicert_tpm2_encrypt [options]`

 # DESCRIPTION
 <B>digicert_tpm2_encrypt</B> encrypts data using the key provided.

@verbatim
    --h [display command line option])
        Help menu.
    --conf=[TPM 2.0 configuration file]
        Path to TPM 2.0 module configur
    --s=[server name]
    ...
    --es=[encryption scheme]
        (Optional) Encryption scheme (pkcs1, oaepsha1, oaepsha256, oaepsha384 or oaepsha512) for asymmetric key encryption.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_encrypt: Help Menu\n");
    LOG_MESSAGE("This tool encrypts the input file using an asymmetric key.");
    ...
    LOG_MESSAGE("                   (Optional) Encryption scheme (pkcs1, oaepsha1, oaepsha256, \n"
                "                   oaepsha384, oaepsha512) for asymmetric key encryption.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        case 7:
            ...
            /* Check if Private key input file string length exceeds maximum allowed */
        ...
        case 8:
            ...
            /* Check if Input data file string length exceeds maximum allowed */
            if(DIGI_STRLEN((const sbyte *)optarg) > FILE_PATH_LEN)
            {
                LOG_ERROR("--idf Input data file path too long");
                goto exit;
            }
        ...
        case 9:
            ...
            pOpts->outEncryptedFile.bufferLen = DIGI_STRLEN((const sbyte *)optarg) + 1;

            if ((pOpts->outEncryptedFile.bufferLen == 1) ||
                ('-' == optarg[0]))
            {
            ...
            /* Check if Output signature file string length exceeds maximum allowed */
            if(DIGI_STRLEN((const sbyte *)optarg) > FILE_PATH_LEN)
            {
                LOG_ERROR("--odf Output signature file path too long");
                goto exit;
            }
                            ...
                            pOpts->outEncryptedFile.bufferLen)))
            {
                LOG_ERROR("Unable to allocate %d bytes for signature filename",
                        (int)pOpts->outEncryptedFile.bufferLen);
                goto exit;
            }
            ...
            TPM2_DEBUG_PRINT("Encrypted filename: %s", pOpts->outEncryptedFile.pBuffer);
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to encrypt input file.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_encrypt failed to complete successfully.*****");
