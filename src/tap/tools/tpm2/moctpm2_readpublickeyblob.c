/**
 @file moctpm2_readpublickeyblob.c

 @page digicert_tpm2_readpublickeyblob
 ...
 @endhtmlonly
 This Tool returns the public key blob of the asymmetric key in mocana internal blob format.

 # SYNOPSIS
 `digicert_tpm2_readpublickeyblob [options]`

 # DESCRIPTION
 <B>digicert_tpm2_readpublickeyblob</B> This Tool returns the public key blob of the asymmetric key in mocana internal blob format.
...
@verbatim
    ...
    --eid=[Entity ID]
        Public key of the specified entity id will be read. Entity ID of Storage Root Key is 0x81000000. Entity ID of Endorsement Key is 0x81010000.
    --pub=[public key file]
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_readpublickeyblob: Help Menu\n");
    LOG_MESSAGE("This tool reads the public portion of the entity (SRK or EK) and writes to a file\n");
    ...
    LOG_MESSAGE("           --eid=[Entity ID]");
    LOG_MESSAGE("                   Public key of the specified entity id will be read. Entity ID of Storage Root key is 0x81000000.\n"
                "                   Entity ID of Endorsement Key is 0x81010000.\n");
    LOG_MESSAGE("           --pub=[public key file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains the public key data.\n");
    return;
}
    ...
    while (TRUE)
    {
        ...
        case 4:
            ...
            if ((pOpts->pubKeyFile.bufferLen == 1) ||
                ('-' == optarg[0]))
            {
                LOG_ERROR("--pub Output public key file not specified");
                goto exit;
            }
            ...
            TPM2_DEBUG_PRINT("Public key data filename: %s", pOpts->pubKeyFile.pBuffer);
            break;
        ...
        case 7:
            ...
            {
                LOG_ERROR("Entity ID must be specified");
                goto exit;
            }
            if (('-' == optarg[0]))
            {
                pOpts->objectId = SRK_OBJECT_ID_START;
            }
            else 
            {
                if (('0' == optarg[0]) && ('x' == optarg[1]))
                {
                    if (OK != DIGI_convertHexString((const char *)&optarg[2], 
                                    (ubyte *)&objectId, sizeof(objectId)))
                    {
                        TPM2_DEBUG_PRINT_1("Failed to convert hex string");
                        goto exit;
                    }
                    pOpts->objectId = DIGI_NTOHL((ubyte *)&objectId);
                }
                else
                {
                    objectId = DIGI_ATOL((const sbyte *)optarg, NULL);
                    pOpts->objectId = objectId;
                }
            }

            TPM2_DEBUG_PRINT("Entity ID: 0x%0x", pOpts->objectId);
            break;
            case 8:
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (!pOpts->serverNameSpecified || !pOpts->pubKeyFileSpecified || !pOpts->bEidpub) 
    {
        LOG_ERROR("Mandatory option --s, --pub not specified.");
        goto exit;
    }
...
#else
    if (!pOpts->pubKeyFileSpecified) 
    {
        LOG_ERROR("Mandatory option --pub, not specified.");
        goto exit;
    }
#endif
    ...
    status = TAP_getRootOfTrustPublicKeyBlob(pTapContext, pOpts->objectId, 
                                TAP_ROOT_OF_TRUST_TYPE_UNKNOWN,
                                &entityPubKey, pErrContext);

    if (OK != status)
    {
        PRINT_STATUS("TAP_getRootOfTrustPublicKeyBlob", status);
        goto exit;    
    }
    ...
    status = DIGICERT_writeFile((const char *)pOpts->pubKeyFile.pBuffer,
                                (const ubyte *)entityPubKey.pBuffer, entityPubKey.bufferLen);
    if (OK != status)
    {
    ...
    LOG_MESSAGE("Successfully read public key data\n");
    retval = 0;
...
exit:
    ...
    if(NULL != pOpts->pubKeyFile.pBuffer)
    {
        DIGI_FREE((void **)&(pOpts->pubKeyFile.pBuffer));
    }
    if(NULL != entityPubKey.pBuffer)
    {
        DIGI_FREE((void **)&(entityPubKey.pBuffer));
    }
    return retval;
}
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to read public Key blob.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_readpublickeyblob failed to complete successfully.*****");
