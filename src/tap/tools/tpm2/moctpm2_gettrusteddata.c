/**
 @file moctpm2_gettrusteddata.c

 @page digicert_tpm2_gettrusteddata
 ...
 # SYNOPSIS
 `digicert_tpm2_gettrusteddata [options]`

 # DESCRIPTION
 <B>digicert_tpm2_gettrusteddata</B> This tool reads the TPM’s PCRs.
...
@verbatim
    ...
    --tdtype=[data type]
        Type of trusted data (measurement, identifier or report).
    --tdsubype=[subtype]
        Sub type of trusted data. Set to 1 for TPM 2.0.
    --tdidx=[PCR index]
        (Optional) The PCR index to read. Defaults to all PCRs. 
    --odf=[output data file]
        (Mandatory) Output file that contains the PCRs specified by --tdidx option.
@endverbatim
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_gettrusteddata: Help Menu\n");
    LOG_MESSAGE("This tool reads the TPM’s PCRs.");
    ...
    LOG_MESSAGE("                   (Optional) The PCR index to read. Defaults to all PCRs.\n");    
    LOG_MESSAGE("           --odf=[output data file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains the PCRs specified by --tdidx option.\n");
    return;
}
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
...
#endif
            {"tdtype", required_argument, NULL, 4},
            {"tdsubtype", required_argument, NULL, 5},
            {"odf", required_argument, NULL, 6},
            {"tdidx", required_argument, NULL, 7},
#ifndef __ENABLE_TAP_REMOTE__
            {"conf", required_argument, NULL, 8},
#endif
            {"modulenum", required_argument, NULL, 9},
            {NULL, 0, NULL, 0},
    };
    MSTATUS status;
    sbyte4 cmpResult;
    ...
    while (TRUE)
    {
        ...
        case 4:
            ...
            /* Check if the input is longer than the longest valid option */
        ...
        case 6:
            ...
            {
                LOG_ERROR("Trusted data filename too long. Max size: %d bytes",
                        FILE_PATH_LEN);
                goto exit;
            }
            ...
            TPM2_DEBUG_PRINT("Trusted data filename: %s", pOpts->outTrustedDataFile.pBuffer);
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    {
        LOG_ERROR("One of mandatory options --s, --tdtype, --tdsubtype or --odf not specified.");
        goto exit;
    }
#else
    if (!pOpts->dataTypeSpecified || !pOpts->subTypeSpecified || 
        !pOpts->outTrustedDataFileSpecified)
    {
        LOG_ERROR("One of mandatory options --tdtype, --tdsubtype or --odf not specified.");
        goto exit;
    }
#endif
    ...
    {
        pOpts->numPcrs = sizeof(pOpts->pcrList);
        for (i = 0; i < pOpts->numPcrs; i++)
        {
            pOpts->pcrList[i] = i;
        }
    }

    tapTrustKeyAttr.pBuffer = pOpts->pcrList;
    tapTrustKeyAttr.bufferLen = pOpts->numPcrs;
    ...
    LOG_MESSAGE("Trusted data written to file successfully\n");
    retval = 0;
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_gettrusteddata failed to complete successfully.*****");
