/**
 @file moctpm2_readpubek.c

 @page digicert_tpm2_readpubek
 ...
 # SYNOPSIS
 `digicert_tpm2_readpubek [options]`

 # DESCRIPTION
 <B>digicert_tpm2_readpubek</B> This tool displays the public portion of the Endorsement key.
...
void printHelp()
{
    LOG_MESSAGE("digicert_tpm2_readpubek: Help Menu\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
...
#endif
            {"modulenum", required_argument, NULL, 6},
            {NULL, 0, NULL, 0},
    };
    ...
    while (TRUE)
    {
        ...
        case 4:
            ...
            {
                LOG_ERROR("Output public key file path too long. Max size: %d bytes",
                        FILE_PATH_LEN);
                goto exit;
            }
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (NULL == pRootKey)
    {
        ...
        PRINT_STATUS("TAP_getRootOfTrustKey:Returned Invalid output", status);
        goto exit;
    }
    ...
    LOG_MESSAGE("Successfully read Root of Trust key data\n");
    retval = 0;
...
exit:
    ...
    {
        /* Unload key object */
        status = TAP_unloadKey(pRootKey, pErrContext);
        if (OK != status)
...
int main(int argc, char *argv[])
{
    ...
    {
        TPM2_DEBUG_PRINT_1("Failed to read public Endorsement Key.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****digicert_tpm2_readpubek failed to complete successfully.*****");
