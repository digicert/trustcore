/**
 ...
 moctap_verify -
 @endhtmlonly
 This tool verifies a previously signed buffer using an asymmetric key.

 # SYNOPSIS
 `moctap_verify [options]`

 # DESCRIPTION
 <B>moctap_verify</B> verifies a signature using the key provided.
...
void printHelp()
{
    LOG_MESSAGE("moctap_verify: Help Menu\n");
    LOG_MESSAGE("This tool verifies a previously created signature of the hash of the input file using TPM.");
    ...
    LOG_MESSAGE("                   (Optional) Hash algorithm (sha1, sha256, or sha512) to hash \n"
                "                   the data in the input file. Defaults to sha256.\n");
    ...
    LOG_MESSAGE("                   (Mandatory) Input file that contains the data to be verified.\n");
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    if (!pOpts->serverNameSpecified ||  
            ...
            !pOpts->inSignatureFileSpecified || !pOpts->prNameSpecified || !pOpts->modIdSpecified) 
    {
        LOG_ERROR("One or more mandatory options --s, --pri, --idf, --isf, --pn or --mid not specified.");
        goto exit;
    }
    ...
    if (!pOpts->inPrivKeyFileSpecified || !pOpts->inFileSpecified ||
        !pOpts->inSignatureFileSpecified || !pOpts->prNameSpecified || !pOpts->modIdSpecified)
    {
        LOG_ERROR("One or more mandatory options --pri, --idf, --isf,  --pn or --mid not specified.");
        goto exit;
    }
#endif
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_verify failed to complete successfully.*****");
