/**
 ...
 moctap_sign -
 @endhtmlonly
 This tool signs input buffer using an asymmetric key

 # SYNOPSIS
 `moctap_sign [options]`

 # DESCRIPTION
 <B>moctap_sign</B> signs data using the key provided.
...
@endverbatim
 ...
 # SEE ALSO
 moctap_createasymkey,  moctap_createsymkey, moctap_verify
...
void printHelp()
{
    LOG_MESSAGE("moctap_sign: Help Menu\n");
    LOG_MESSAGE("This tool generates a signature from hash of the input file using TPM.");
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 8:
                ...
                {
                    ubyte4 optValLen = DIGI_STRLEN((const sbyte *)optarg);
                    pOpts->outSignatureFile.bufferLen = optValLen + 1;
                    ...
                    if (OK != DIGI_MEMCPY(pOpts->outSignatureFile.pBuffer, optarg, optValLen))
                    {
                        MOCTAP_DEBUG_PRINT_1("Failed to copy memory");
                    ...
                    pOpts->outSignatureFile.pBuffer[optValLen] = '\0';
                    MOCTAP_DEBUG_PRINT("Signature filename: %s", pOpts->outSignatureFile.pBuffer);
...
/*    TAP_Buffer userCredBuf = {0} ;*/
    ...
    if (!pOpts->serverNameSpecified ||  
            ...
            !pOpts->outSignatureFileSpecified || !pOpts->prNameSpecified || !pOpts->modIdSpecified) 
    {
        LOG_ERROR("One or more mandatory options --s, --pr, --idf, --odf, --pn or --mid not specified.");
        goto exit;
    }
    ...
    if (!pOpts->inPrivKeyFileSpecified || !pOpts->inFileSpecified ||
        !pOpts->outSignatureFileSpecified || !pOpts->prNameSpecified || !pOpts->modIdSpecified)
    {
        LOG_ERROR("One or more mandatory options --pr, --idf, --odf, --pn or --mid not specified.");
        goto exit;
    }
#endif
    ...
    if (OK == status)
    {
        ...
        if (OK == status)
        {
            ...
            {
                status = TAP_symSign(pLoadedTapKey, pEntityCredentials, NULL,
                                        0, &digest, &signature, pErrContext);
            }
            else
            {
            ...
            else
            {
                ...
                LOG_MESSAGE("Data successfully signed!\n");
...
int main(int argc, char *argv[])
{
    ...
    {
        MOCTAP_DEBUG_PRINT_1("Failed to generate signature.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_sign failed to complete successfully.*****");
