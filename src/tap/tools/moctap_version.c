/**
 ...
 moctap_version -
 @endhtmlonly
 Get the firmware version and manufacturer information of the secure element .

 # SYNOPSIS
 `moctap_version [options]`

 # DESCRIPTION
 <B>moctap_version</B> reports the firmware version and manufacturer information on the secure element. The information reported is manufacturer specific.
...
@verbatim
    ...
    --mid=[module id]
        id of the module being interfaced with
...
void printHelp()
{
    LOG_MESSAGE("moctap_version: Help Menu\n");
    LOG_MESSAGE("This tool reports the firmware version and manufacturer information of the secure element.");
...
/*
static MSTATUS validateProviderType(TAP_PROVIDER provider)
{
    MSTATUS status = OK;
    tapProviderEntry *pProvider = NULL;

    pProvider = gTapProviders;
    while (NULL != pProvider)
    {
        if (TAP_PROVIDER_UNDEFINED == pProvider->providerType)
        {
            status = ERR_TAP_UNSUPPORTED;
            break;
        }
        if (pProvider->providerType == provider)
        {
            status = OK;
            break;
        }
...
int executeOptions(cmdLineOpts *pOpts)
{
    ...
    {
        LOG_ERROR("Mandatory option --s, --pn, --mid not specified.");
        goto exit;
    }
...
#else
    if (!pOpts->prNameSpecified || !pOpts->modIdSpecified) 
    {
        LOG_ERROR("Mandatory option --pn, --mid not specified.");
        goto exit;
    }
#endif
    ...
    {
#if defined(__RTOS_WIN32__)
        status = TAP_UTILS_getWinConfigFilePath(&pSmpConfigFile,
                            pOpts->pTapProviderEntry->configFilePath);
        if (OK != status)
        {
            goto exit;
        }
    ...
    /* TODO - Parse the value read from TAP_readConfigFile and validate the
     * provider-type configured in the conf file */
    /*
    status = validateProviderType(configInfoList.pConfig[0].providerType);
    if (OK != status)
    {
        LOG_ERROR("Unsupported provider type '%d' specified in config file, "
                  "status = %d", configInfoList.pConfig[0].provider, status);
        goto exit;
    }
    */
#endif /* ifndef __ENABLE_TAP_REMOTE__ */
    ...
    if(pOpts->credFileSpecified)
    {
        status = DIGICERT_readFile(pOpts->credFileName, &(userCredBuf.pBuffer),
                             &(userCredBuf.bufferLen));
        if (OK != status)
        {
            LOG_ERROR("Failed to read user credential file %s, status = %d", 
                   pOpts->credFileName, status);
            goto exit;
        }
    ...
    status = TAP_getModuleList(&connInfo, pOpts->pTapProviderEntry->providerType,
                               NULL, &moduleList, pErrContext);
#else
    status = TAP_getModuleList(NULL, pOpts->pTapProviderEntry->providerType,
                               NULL, &moduleList, pErrContext);
#endif
    if (OK != status)
    {
    ...
    /* Invoke TAP API */
    status = TAP_getModuleVersionInfo(&(moduleList.pModuleList[0]), &versionInfo,
                                      pErrContext);
    
    if (OK != status)
    {
        PRINT_STATUS("TAP_getModuleVersionInfo", status);
        goto exit;
    }

    if (NULL == versionInfo.pAttributeList)
    {
        LOG_ERROR("TAP_getModuleVersionInfo returned an empty attribute list\n");
        goto exit;
    }
    ...
    if (OK != status)
        PRINT_STATUS("TAP_UTILS_freeAttributeList", status);
    else
        retval = 0;
...
int main(int argc, char *argv[])
{
    ...
    {
        MOCTAP_DEBUG_PRINT_1("Failed to get version");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_version failed to complete successfully.*****");
