/**
 @file moctap_sealdata.c

 @page moctap_sealdata
 ...
 # SYNOPSIS
 `moctap_sealdata [options]`

 # DESCRIPTION
 <B>moctap_sealdata</B> This tool seals data with the ownership keys, and optionally with the Security Module register configuration. The result can be unsealed with moctap_unsealdata.</B>.
...
@verbatim
    ...
    --idf=[input data file]
        (Mandatory) Input file that contains the data to be sealed.
    --odf=[output data file]
        (Mandatory) Output file that contains the sealed data.
@endverbatim
...
void printHelp()
{
    LOG_MESSAGE("moctap_sealdata: Help Menu\n");
    LOG_MESSAGE("This tool seals data with the ownership keys, and optionally with the Security Module register configuration. The result can be unsealed with moctap_unsealdata.");
    ...
    LOG_MESSAGE("                   If no authorization value is specified, the well-known password is used.\n");
    LOG_MESSAGE("           --idf=[input data file]");
    LOG_MESSAGE("                   (Mandatory) Input file that contains the data to be sealed.\n");
    LOG_MESSAGE("           --odf=[output signature file]");
    LOG_MESSAGE("                   (Mandatory) Output file that contains the sealed data.\n");
    LOG_MESSAGE("           --tdidx=[PCR]");
    LOG_MESSAGE("                   Optional: PCR to seal the data.\n");
...
int parseCmdLineOpts(cmdLineOpts *pOpts, int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        case 9:
            /* TODO: Add this after have policy support for PCRs */
            {
                char *endptr;
                long pcr;
                errno = 0;
                pcr = strtol(optarg, &endptr, 0);
                if ((errno == ERANGE) || (endptr == optarg) || (*endptr != '\0') ||
                    (pcr < 0) || (pcr >= MAX_PCR_REGISTERS))
                {
                    LOG_ERROR("Invalid PCR number specified. Must be 0-%d", MAX_PCR_REGISTERS - 1);
                    goto exit;
                }
                currPcr = (ubyte)pcr;
                pOpts->pcrList[pOpts->numPcrs] = currPcr;
...
int main(int argc, char *argv[])
{
    ...
    {
        MOCTAP_DEBUG_PRINT_1("Failed to seal data.");
        goto exit;
    }
...
exit:
    ...
    if (0 != retval)
        LOG_ERROR("*****moctap_seal failed to complete successfully.*****");
