/*
 * trustedge_scep_message.c
 *
 * SCEP -- Simple Certificate Enrollment Protocol Messages
 *
...
static MSTATUS
SCEP_MESSAGE_integerToString(ubyte *number, ubyte4 numberLen, sbyte* pBuf, ubyte4 bufLen)
{
    MSTATUS status = OK;
    ubyte4 i;
                               ...
                               transactionAttributes *pTransAttrs)
{
    ...
    if (!pTransAttrs->transactionID)
    {
        ...
        if (OK > (status = SCEP_MESSAGE_integerToString(serialNumberBuf, pSerialNumber->length,
...
MSTATUS
SCEP_MESSAGE_generatePayLoad(AsymmetricKey *pKey, byteBoolean unloadKey, requestInfo *pReqInfo, ubyte** ppPayLoad, ubyte4* pPayLoadLen)
{
    MOC_UNUSED(unloadKey);
    MSTATUS         status;
    ...
    *ppPayLoad = NULL;
    ...
    case scep_PKCSReq:
        ...
        if (OK > (status = PKCS10_GenerateCertReqFromDNEx(pKey, pReqInfo->value.certInfoAndReqAttrs.pCsrCtx->hashId,
                                                    pReqInfo->value.certInfoAndReqAttrs.pCsrCtx->pCertSubjectInfo,
                                                    &pReqInfo->value.certInfoAndReqAttrs.pCsrCtx->reqAttr,
        ...
        if (unloadKey)
        {
            (void) TRUSTEDGE_TAP_unloadKey(pKey);
        }
#endif
        break;
                                          ...
                                          ubyte **ppSigned, ubyte4 *pSignedLen)
{
    ...
    ASN1_ITEMPTR*   pCertArray   = NULL;
    CStream*        pCertCS      = NULL;
    ASN1_ITEMPTR*   pCrlArray    = NULL;
    CStream*        pCrlCS       = NULL;
    hwAccelDescr    hwAccelCtx;
    ASN1_ITEM*      pCertRoot = NULL;
    CStream         certCS;
    MemFile         certMemFile;
    ASN1_ITEM*      pCrlRoot = NULL;
    CStream         crlCS;
    MemFile         crlMemFile;
    ubyte4          i;
    MSTATUS         status = OK;
    ...
    /* init certs and crls arrays */
    if (certificateLen > 0)
    {
        if (NULL == (pCertArray = MALLOC((numCACerts+1)*sizeof(ASN1_ITEMPTR))))
        {
            status = ERR_MEM_ALLOC_FAIL;
            goto exit;
        }

        if (NULL == (pCertCS = MALLOC((numCACerts+1)*sizeof(CStream))))
        {
            status = ERR_MEM_ALLOC_FAIL;
            goto exit;
        }

        MF_attach(&certMemFile, certificateLen, (ubyte*) pCertificate);
        CS_AttachMemFile(&certCS, &certMemFile );
        if (OK > (status = X509_parseCertificate(certCS, &pCertRoot)))
            goto exit;
        pCertArray[0] = pCertRoot;
        pCertCS[0] = certCS;

        for (i = 1; i <= (ubyte4)numCACerts; i++)
        {
            pCertArray[i] = pCACertificates[i-1];
            pCertCS[i] = pCAStreams[i-1];
        }
        numCACerts = numCACerts + 1;
    } else
    {
        pCertArray = pCACertificates;
        pCertCS = pCAStreams;
    }
    if (crlLen > 0)
    {
        if (NULL == (pCrlArray = MALLOC((numCrls+1)*sizeof(ASN1_ITEMPTR))))
        {
            status = ERR_MEM_ALLOC_FAIL;
            goto exit;
        }
        if (NULL == (pCrlCS = MALLOC((numCrls+1)*sizeof(CStream))))
        {
            status = ERR_MEM_ALLOC_FAIL;
            goto exit;
        }
        MF_attach(&crlMemFile, crlLen, (ubyte*) pCrl);
        CS_AttachMemFile(&crlCS, &crlMemFile );
        if (OK > (status = ASN1_Parse(crlCS, &pCrlRoot)))
            goto exit;

        pCrlArray[0] = pCrlRoot;
        pCrlCS[0] = crlCS;

        for (i = 1; i <= (ubyte4)numCrls; i++)
        {
            pCrlArray[i] = pCrls[i-1];
            pCrlCS[i] = pCrlStreams[i-1];
        }
        numCrls = numCrls + 1;
    } else
    {
    ...
    /* wrap in ContentInfo */
    if ( OK > (status = SCEP_MESSAGE_createContentInfo(pkcs7_signedData_OID, &pContentInfo, &pContent)))
        goto exit;

    if (OK > (status = PKCS7_SignData(MOC_ASYM(hwAccelCtx) 0, pContentInfo, pContent,
                                      pCertArray, pCertCS, numCACerts,
                                      pCrlArray, pCrlCS, numCrls,
                                      NULL,/* no signers */ 0,
                                      pkcs7_data_OID, NULL /* no payload */, 0, NULL, 0,
                                      ppSigned, pSignedLen)))
        goto exit;
exit:
    ...
    if (pCrlRoot)
    {
        TREE_DeleteTreeItem((TreeItem*)pCrlRoot);
    }
                                ...
                                ubyte **ppPkiMessage, ubyte4 *pPkiMessageLen)
{
    ...
    *pPkiMessageLen = 0;
...
#if 0
    /* Server Response Requires this - uncomment, Also increment the authAttributeLen */
    /* pkiStatus */
     (pAuthAttributes+offset)->typeOID = verisign_pkiAttrs_pkiStatus_OID;
     (pAuthAttributes+offset)->type = PRINTABLESTRING;
     (pAuthAttributes+offset)->value = MALLOC(1);
     (pAuthAttributes+offset)->value[0] = '0' + pEnvelopeSignParams->pTransAttrs->pkiStatus;
    ...
    /* record senderNonce to verify with response receiverNonce */

    /*
    //pEnvelopeSignParams->pTransAttrs->senderNonce = (ubyte*)MALLOC(senderNonceLen);
    //DIGI_MEMCPY(pEnvelopeSignParams->pTransAttrs->senderNonce, senderNonce, senderNonceLen);
    //pEnvelopeSignParams->pTransAttrs->senderNonceLen = senderNonceLen;
    */
...
#if 0
    if (pAuthAttributes && (pAuthAttributes+authAttributeLen-1)->value)
        FREE((pAuthAttributes+authAttributeLen-1)->value);
#endif
...
{
    MSTATUS status = OK;
    transactionAttributes *pTransAttrs = NULL;
    ubyte* pPayLoad = NULL;
    ubyte4 payLoadLen = 0;
    ...
    *pPkiMessageLen = 0;
    ...
    {
        if (OK > (status = SCEP_MESSAGE_generatePayLoad(pPkcsCtx->pKey, pPkcsCtx->isTapPw, pScepContext->pReqInfo, &pPayLoad, &payLoadLen)))
            goto exit;
    }
    else
    {
     ...
     /* if ( OK > ( status = DIGICERT_writeFile( "c:\\ws\\src\\scep\\test\\payload.der", pPayLoad, payLoadLen)))
        goto exit; */
    ...
    pEnvSignParams->pRecipientKey = pPkcsCtx->pKey;
    ...
    pEnvSignParams->pSignerKey = pPkcsCtx->pSignKey;
...
extern MSTATUS
SCEP_MESSAGE_generatePkiResponseMessage(pkcsCtxInternal *pPkcsCtx,
                                        scepContext *pScepContext,
                                        ...
                                        ubyte** ppPkiMessage, ubyte4* pPkiMessageLen)
{
    ...
    if (pScepContext->roleType != SCEP_SERVER)
    {
        status = ERR_SCEP_INVALID_ROLETYPE;
        goto exit;
    }
    ...
    /*
    //pTransAttrs->recipientNonce = pTransAttrs->senderNonce;
    //pTransAttrs->recipientNonceLen = pTransAttrs->senderNonceLen;
    //if (NULL == (pTransAttrs->senderNonce = MALLOC(16)))
    //{
    //    status = ERR_MEM_ALLOC_FAIL;
    //    goto exit;
    //}
    //RANDOM_numberGenerator(g_pRandomContext, pTransAttrs->senderNonce, 16);
    //pTransAttrs->senderNonceLen = 16;
    */
    ...
    pEnvSignParams->pTransAttrs = pTransAttrs;
    pEnvSignParams->encryptAlgoOID = pPkcsCtx->encryptAlgoOID;
    pEnvSignParams->rCS = pPkcsCtx->requesterCertStream;
    pEnvSignParams->pRecipientCert = pPkcsCtx->pRequesterCert;
    pEnvSignParams->sCS = pPkcsCtx->CACertStream;
    pEnvSignParams->pSignerCert = pPkcsCtx->pCACertificate;
    pEnvSignParams->digestAlgoOID = pPkcsCtx->digestAlgoOID;
    pEnvSignParams->pSignerKey = pPkcsCtx->pKey;
    pEnvSignParams->rngFun = pPkcsCtx->rngFun;
    pEnvSignParams->rngFunArg = pPkcsCtx->rngFunArg;
                      ...
                      ubyte **bufToDelete)
{
    ...
    */
    if (OK == ASN1_VerifyType(pContent, SEQUENCE))
    {
        ...
        {
            ASN1_GetChildWithTag(pContent, 0, ppEnvelopedData);
            *pStream = cs;
        }
    }
    else
    {
        ...
        *bufToDelete = envelopedDataBuf;
        ...
        ASN1_GetChildWithTag(pContentInfo, 0, ppEnvelopedData);
        if (!(*ppEnvelopedData))
        {
            status = ERR_SCEP_BAD_MESSAGE;
            goto exit;
                                          ...
                                          ubyte* pMessage, ubyte4 messageLen)
{
    ...
    /* save the first certificate as the requester certificate */
    if (pCertificates && pScepContext->roleType == SCEP_SERVER)
    {
        ubyte *buf = NULL;
        if (pPkcsCtx->pRequesterCert)
        {
            TREE_DeleteTreeItem((TreeItem*) pPkcsCtx->pRequesterCert);
            pPkcsCtx->pRequesterCert = NULL;
        }
        if (pPkcsCtx->requesterCertDescriptor.pCertificate)
        {
            if (0 == pPkcsCtx->requesterCertDescriptor.cookie)
            {
                FREE(pPkcsCtx->requesterCertDescriptor.pCertificate);
            } else
            {
                SCEP_scepSettings()->funcPtrCertificateStoreRelease(0, &pPkcsCtx->requesterCertDescriptor);
            }

            pPkcsCtx->requesterCertDescriptor.pCertificate = NULL;
            pPkcsCtx->requesterCertDescriptor.certLength = 0;
        }
        buf = (ubyte*)CS_memaccess(signedStream, pCertificates->dataOffset, pCertificates->length);
        if (NULL == (pPkcsCtx->requesterCertDescriptor.pCertificate = MALLOC(pCertificates->length)))
        {
            status = ERR_MEM_ALLOC_FAIL;
            goto exit;
        }
        DIGI_MEMCPY(pPkcsCtx->requesterCertDescriptor.pCertificate, buf, pCertificates->length);
        pPkcsCtx->requesterCertDescriptor.cookie = 0;
        MF_attach(&pPkcsCtx->requesterCertMemFile, pCertificates->length, (ubyte*) pPkcsCtx->requesterCertDescriptor.pCertificate);
    ...
    */
        ...
        pScepContext->pTransAttrs->pkiStatus == (SCEP_pkiStatus)scep_SUCCESS))
    {
        ...
        if (OK > (status = PKCS7_DecryptEnvelopedData(MOC_HW(hwAccelCtx) pEnvelopedData,
         ...
         /* if ( OK > ( status = DIGICERT_writeFile( "c:\\ws\\src\\scep\\test\\decryptedData.der", pkcsData, pkcsDataLen)))
                goto exit; */

        if (pScepContext->roleType == SCEP_SERVER)
        {
            /* depending on request type, it could be a pkcs#10 or an issuerAndSubject or
            * an issuerAndSerialNumber or a pkcsGetCRL */
            if (NULL == (pScepContext->pReceivedData = (ubyte*) MALLOC(pkcsDataLen)))
            {
                status = ERR_MEM_ALLOC_FAIL;
                goto exit;
            }
            DIGI_MEMCPY(pScepContext->pReceivedData, pkcsData, pkcsDataLen);
            pScepContext->receivedDataLength = pkcsDataLen;
        } else
        {
            ...
            /* CA Certificate is optionally included.
            * if there is more than one cert, CA cert is included */
#if 0
            //if (pCertificate && ASN1_NEXT_SIBLING(pCertificate))
            //{
            //    /* generate a getCertFun for accessing the certs
            //    * from inside the degenerate SignedData */
            //    callbacks->getCertFun = internalGetCertFunc;
            //}
#endif
        }
...
extern MSTATUS
SCEP_MESSAGE_parsePkiRequest(pkcsCtxInternal *pPkcsCtx, scepContext *pScepContext, ubyte *pRequest, ubyte4 requestLen)
{
    return SCEP_MESSAGE_parsePkcsSignedEnvelopedData(pPkcsCtx, pScepContext, pRequest, requestLen);
}
