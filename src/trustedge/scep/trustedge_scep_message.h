/*
 * trustedge_scep_message.h
 *
 * SCEP definitions and message generation and parsing routines
 *
...
@flags      This file requires that one of the following flags be defined:
    + \c \__ENABLE_DIGICERT_SCEP_CLIENT__

@filedoc    trustedge_scep_message.h
*/


#ifndef __TRUSTEDGE_SCEP_MESSAGE_HEADER__
#define __TRUSTEDGE_SCEP_MESSAGE_HEADER__
...
@inc_file   trustedge_scep_message.h
...
@funcdoc    trustedge_scep_message.h
*/
MOC_EXTERN MSTATUS
...
@details    Generates a PKCS#7 response message.

@param pPkcsCtx         Pointer to the pkcsCtxInternal structure.
@param pScepContext     Pointer to the scepContext structure.
@param pPayLoad         Pointer to the payload.
@param payLoadLen       Length of the payload.
@param ppPkiMessage     On return, pointer to the PKCS#7 signed response.
@param pPkiMessageLen       On return, length of the PCSK#7 signed response.
...
@param key              Pointer to the asymmetric key with which CSR
                        to be signed.
@param unloadKey        For TAP builds, unload the TAP key after operation if \c TRUE.
...
*/
MOC_EXTERN MSTATUS
SCEP_MESSAGE_generatePayLoad(AsymmetricKey *key, byteBoolean unloadKey, requestInfo *pReqInfo, ubyte** pPayLoad, ubyte4* pPayLoadLen);
...
#endif  /*#ifndef __TRUSTEDGE_SCEP_MESSAGE_HEADER__ */
