 /*
 * crypto_interface_sha256.c
 *
 * Cryptographic Interface for SHA256.
 *
...
#if (defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_SHA256__))

/*----------------------------------------------------------------------------*/

#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__))
#define MOC_SHA256_ALLOC(_status, _pContext)                                  \
    _status = SHA256_allocDigest(MOC_HASH(hwAccelCtx) _pContext);
#else
#define MOC_SHA256_ALLOC(_status, _pContext)                                  \
...
#define MOC_SHA256_INIT(_status, _pContext)                                   \
    _status = SHA256_initDigest(MOC_HASH(hwAccelCtx) _pContext);
#else
#define MOC_SHA256_INIT(_status, _pContext)                                   \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
...
#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__)) && \
    (!defined(__DISABLE_DIGICERT_SHA256__))
#define MOC_SHA256_UPDATE(_status, _pContext, _pData, _dataLen)               \
    _status = SHA256_updateDigest(MOC_HASH(hwAccelCtx) _pContext, _pData, _dataLen);
#else
#define MOC_SHA256_UPDATE(_status, _pContext, _pData, _dataLen)               \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
...
#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__)) && \
    (!defined(__DISABLE_DIGICERT_SHA256__))
#define MOC_SHA256_FINAL(_status, _pContext, _pOutput)                       \
    _status = SHA256_finalDigest(MOC_HASH(hwAccelCtx) _pContext, _pOutput);
#else
#define MOC_SHA256_FINAL(_status, _pContext, _pOutput)                       \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
...
#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__))
#define MOC_SHA256_FREE(_status, _pContext)                                  \
    _status = SHA256_freeDigest(MOC_HASH(hwAccelCtx) _pContext);
#else
#define MOC_SHA256_FREE(_status, _pContext)                                  \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
...
#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__)) && \
    (!defined(__DISABLE_DIGICERT_SHA256__))
#define MOC_SHA256_COMPLETE(_status, _pData, _dataLen, _pOutput)             \
    _status = SHA256_completeDigest(MOC_HASH(hwAccelCtx) _pData, _dataLen, _pOutput);
#else
#define MOC_SHA256_COMPLETE(_status, _pData, _dataLen, _pOutput)             \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
...
#if (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__)) && \
    (!defined(__DISABLE_DIGICERT_SHA256__))
#define MOC_SHA256_CLONE(_status, _pDest, _pSrc)                             \
    _status = SHA256_cloneCtx(MOC_HASH(hwAccelCtx) _pDest, _pSrc);
#else
#define MOC_SHA256_CLONE(_status, _pDest, _pSrc)                             \
    _status = ERR_CRYPTO_INTERFACE_NO_IMPLEMENTATION_AVAILABLE;
#endif
  ...
  )
{
  ...
  /* This API is used for allocating a context for either SHA256 or SHA224
     We don't know yet which one it is since some SHA224 cipher suites will refer
     to this API. If either is operator enabled then we'll allocated the context
     directly rather than passthrough */

  /* First determine if we have a SHA256 implementation */
  status = CRYPTO_INTERFACE_checkSymAlgoStatus(moc_alg_sha256, &algoStatus, &index);
  if (OK != status)
    goto exit;

  /* If not then check for sha224 */
  if (CRYPTO_INTERFACE_ALGO_ENABLED != algoStatus)
  {
    status = CRYPTO_INTERFACE_checkSymAlgoStatus(moc_alg_sha224, &algoStatus, &index);
    if (OK != status)
      goto exit;  
  }
  ...
  if (CRYPTO_INTERFACE_ALGO_ENABLED == algoStatus)
  {
    /* Simply allocate the shell. We have to wait on the MocSymCtx until we know if it's SHA224 or SHA256 */
    status = DIGI_CALLOC((void **)&pShaCtx, 1, sizeof(SHA256_CTX));
    if (OK != status)
      goto exit;
  ...
  {
    MOC_SHA256_ALLOC(status, pp_context)
  }
  ...
  )
{
  ...
  pContext->hashId = ht_sha256;

  /* Now that we know it's SHA256, determine if we have a SHA256 implementation */
  status = CRYPTO_INTERFACE_checkSymAlgoStatus(moc_alg_sha256, &algoStatus, &index);
  if (OK != status)
    goto exit;
  ...
  {
    /* SHA256 wasn't enabled afterall, mark as such */
    pContext->enabled = 0;
    pContext->pMocSymCtx = NULL;
    MOC_SHA256_INIT(status, pContext)
  }
  ...
  )
{
  ...
  {
    MOC_SHA256_UPDATE(status, pContext, pData, dataLen)
  }
  ...
  )
{
  ...
  if (CRYPTO_INTERFACE_ALGO_ENABLED == pContext->enabled)
  {
    status = CRYPTO_digestFinal (
      pContext->pMocSymCtx, NULL, 0, pOutput, SHA256_RESULT_SIZE, &outputLen);
  }
  else
  {
    MOC_SHA256_FINAL(status, pContext, pOutput)
  }
  ...
  )
{
  ...
  /* Determine if we have a SHA256 implementation */
  status = CRYPTO_INTERFACE_checkSymAlgoStatus (
    moc_alg_sha256, &algoStatus, &index);
  if (OK != status)
    goto exit;
  ...
  if (CRYPTO_INTERFACE_ALGO_ENABLED == algoStatus)
  {
    ...
    status = CRYPTO_digestFinal (
      pNewSymCtx, NULL, 0, pShaOutput, SHA256_RESULT_SIZE, &outputLen);
  }
  else
  {
    MOC_SHA256_COMPLETE(status, pData, dataLen, pShaOutput)
  }
  ...
  )
{
  ...
  {
    MOC_SHA256_FREE(status, pp_context)
  }
  ...
  )
{
  ...
  {
    MOC_SHA256_CLONE(status, pDest, pSrc)
  }
...
#endif /* if (defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_SHA256__)) */
