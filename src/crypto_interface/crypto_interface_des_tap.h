/*
 * crypto_interface_des_tap.h
 *
 * Cryptographic Interface header file for declaring DES TAP functions
...
#ifndef __CRYPTO_INTERFACE_DES_TAP_HEADER__
#define __CRYPTO_INTERFACE_DES_TAP_HEADER__
...
/**
 * @brief   Create a new DES CBC context from a previously existing TAP symmetric key.
 * @details Create a new DES CBC context from a previously existing TAP symmetric key.
 *          This API allocates memory for the new DES CBC context, and takes ownership of
 ...
 *                  a TAP key.
 * @param ppNewCtx  Location that will recieve a pointer to the new DES CBC context.                
...
/**
 * @brief   Create a new DES ECB context from a previously existing TAP symmetric key.
 * @details Create a new DES ECB context from a previously existing TAP symmetric key.
 *          This API allocates memory internally and takes ownership of
 *          the input \c SymmetricKey structure, so be sure to call
 *          \c CRYPTO_INTERFACE_DES_clearKey in order to free this memory when done.
 *
 * @param pSymKey   Pointer to the input \c SymmetricKey structure containing
 *                  a TAP key.
 * @param pCtx      The DES context to be initialized.
 *
 * @return          \c OK (0) if successful, otherwise a negative number
 *                  error code from merrors.h.
 */
...
/**
 * @brief   Sets an DES context (ECB or CBC) to not unload the internal TAP Key upon cipher completion. 
 * @details Sets an DES context (ECB or CBC) to not unload the internal TAP Key upon cipher completion. 
 *
 * @param pCtx      Pointer to a TAP enabled DES context containing a TAP Key.
...
/**
 * @internal This API is private. One should use \c CRYPTO_INTERFACE_DES_encipher.
 */
...
/**
 * @internal This API is private. One should use \c CRYPTO_INTERFACE_DES_decipher.
 */
...
/**
 * @internal This API is private. It's provied as a common API for DES/TDES.
 */
...
#endif /* __CRYPTO_INTERFACE_DES_TAP_HEADER__ */
