/*
*  crypto_interface_aes_eax_test.c
*
*   unit test for AES-EAX
*
...
static int test_vector_test_encrypt(  const AES_EAX_TestVector* pWhichTest, int hint)
{
    ...
    {
        status = AES_EAX_init(MOC_SYM(gpHwAccelCtx)
                              pWhichTest->key, pWhichTest->keyLen,
                              pWhichTest->nonce, pWhichTest->nonceLen, &ctx);
        errors += UNITTEST_STATUS( hint, status);

        status = AES_EAX_updateHeader(MOC_SYM(gpHwAccelCtx) pWhichTest->header, pWhichTest->headerLen, &ctx);
        errors += UNITTEST_STATUS( hint, status);

        status = AES_EAX_final(MOC_SYM(gpHwAccelCtx) buffer, AES_BLOCK_SIZE, &ctx);
        errors += UNITTEST_STATUS( hint, status);

        DIGI_MEMCMP( buffer, pWhichTest->result, AES_BLOCK_SIZE, &cmpRes);
        errors += UNITTEST_INT( hint, cmpRes, 0);
        
        status = AES_EAX_clear(MOC_SYM(gpHwAccelCtx) &ctx);
        errors += UNITTEST_STATUS( hint, status);
    }
    else
    {
        ...
        for ( i = 1; i <= pWhichTest->messageLen; ++i)
        {
            ...
            if ( i & 1)
            {
                AES_EAX_updateHeader(MOC_SYM(gpHwAccelCtx)
                                     pWhichTest->header,
            ...
            while ( sent < pWhichTest->messageLen)
            {
                ...
                status = AES_EAX_encryptMessage(MOC_SYM(gpHwAccelCtx) buffer + sent, toSend, &ctx);
                errors += UNITTEST_STATUS( hint, status);
                sent += toSend;
            }
            ...
            if (!( i & 1))
            {
                AES_EAX_updateHeader(MOC_SYM(gpHwAccelCtx) pWhichTest->header, pWhichTest->headerLen, &ctx);
            }

            status = AES_EAX_final(MOC_SYM(gpHwAccelCtx) buffer + pWhichTest->messageLen, AES_BLOCK_SIZE, &ctx);
            errors += UNITTEST_STATUS( hint, status);
    ...
    if ( 0 != pWhichTest->messageLen)
    {
        ...
        for ( i = 1; i <= pWhichTest->messageLen; ++i)
        {
            ...
            while ( sent < pWhichTest->messageLen)
            {
                ...
                status = AES_EAX_decryptMessage(MOC_SYM(gpHwAccelCtx) buffer + sent, toSend, &ctx);
                errors += UNITTEST_STATUS( hint, status);
                sent += toSend;
            }
            ...
            /* get the tag */
            status = AES_EAX_final(MOC_SYM(gpHwAccelCtx) tag, AES_BLOCK_SIZE, &ctx);
            errors += UNITTEST_STATUS( hint, status);
...
static int test_vector_test_decrypt_alt(  const AES_EAX_TestVector* pWhichTest, int hint)
{
    ...
    if ( 0 != pWhichTest->messageLen)
    {
        ...
        status = AES_EAX_generateTag( MOC_SYM(gpHwAccelCtx) pWhichTest->result, pWhichTest->messageLen,
        ...
        for ( i = 1; i <= pWhichTest->messageLen; ++i)
        {
            ...
            while ( sent < pWhichTest->messageLen)
            {
                ...
                status = AES_EAX_getPlainText(MOC_SYM(gpHwAccelCtx) buffer + sent, toSend, &ctx);
                errors += UNITTEST_STATUS( hint, status);
                sent += toSend;
            }
...
#endif /* #ifndef __DISABLE_AES_CIPHERS__ */

/*---------------------------------------------------------------------------*/

int crypto_interface_aes_eax_test()
{
    MSTATUS status;
    int i, retVal = 0;
