/*
* crypto_interface_aes_xcbc_mac_96_test.c
*
* test file for AES-XCBC-MAC-96
*
...
/* Test Case #2 : AES-XCBC-MAC-96 with 3-byte input */
    { (const ubyte *) "\x00\x01\x02", 3,
    { 0x5b, 0x37, 0x65, 0x80, 0xae, 0x2f, 0x19, 0xaf, 0xe7, 0x21, 0x9c, 0xee} },

/* Test Case #3 : AES-XCBC-MAC-96 with 16-byte input */
    { (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f", 16,
    { 0xd2, 0xa2, 0x46, 0xfa, 0x34, 0x9b, 0x68, 0xa7, 0x99, 0x98, 0xa4, 0x39} },

/* Test Case #4 : AES-XCBC-MAC-96 with 20-byte input */
    { (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
        "\x10\x11\x12\x13", 20,
...
/* Test Case #7: Test Case #4 with key length = 10 */
    {
       (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09", 10,
       (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
        "\x10\x11\x12\x13", 20,
...
/* Test Case #8: Test Case #4 with key length = 18 */
    {
       (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\xed\xcb", 18,
       (const ubyte *) "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
        "\x10\x11\x12\x13", 20,
...
big_test_mac_96()
{
    ...
    for ( i = 1; i <= 100; ++i)
    {
        ...
        status = AES_XCBC_MAC_96_init(MOC_SYM(gpHwAccelCtx)K, &ctx);
        ...
        while ( sent < 1000)
        {
            ...
            status = AES_XCBC_MAC_96_update(MOC_SYM(gpHwAccelCtx) message, toSend, &ctx);
            errors += UNITTEST_STATUS( 0, status);
            sent += toSend;
        ...
        status = AES_XCBC_MAC_96_final(MOC_SYM(gpHwAccelCtx) mac, &ctx);
        errors += UNITTEST_STATUS( 0, status);
        ...
        /* test reset */
        sent = 0;
        status = AES_XCBC_MAC_96_reset(MOC_SYM(gpHwAccelCtx) &ctx);
        errors += UNITTEST_STATUS( 0, status);
        while ( sent < 1000)
        {
            sbyte4 toSend;
        ...
        status = AES_XCBC_clear (MOC_SYM(gpHwAccelCtx) &ctx);
        errors += UNITTEST_STATUS( 0, status);
...
big_test_prf_128()
{
    ...
    for ( i = 1; i <= 100; ++i)
    {
        ...
        status = AES_XCBC_PRF_128_init(MOC_SYM(gpHwAccelCtx)K, 16, &ctx);
        errors += UNITTEST_STATUS( 0, status);
        while ( sent < 1000)
        {
            sbyte4 toSend;
            ...
            status = AES_XCBC_PRF_128_update(MOC_SYM(gpHwAccelCtx) message, toSend, &ctx);
            errors += UNITTEST_STATUS( 0, status);
            sent += toSend;
        ...
        status = AES_XCBC_PRF_128_final(MOC_SYM(gpHwAccelCtx) mac, &ctx);
        errors += UNITTEST_STATUS( 0, status);
        ...
        /* test reset */
        sent = 0;
        status = AES_XCBC_PRF_128_reset(MOC_SYM(gpHwAccelCtx) &ctx);
        errors += UNITTEST_STATUS( 0, status);
        while ( sent < 1000)
        {
            sbyte4 toSend;
...
test_vector_mac_96(  AES_CBC_MAC_96_TestVector* pWhichTest, int hint)
{
    ...
    for ( i = 1; i <= pWhichTest->messageLen; ++i)
    {
        ...
        while ( sent < pWhichTest->messageLen)
        {
            ...
            status = AES_XCBC_MAC_96_update(MOC_SYM(gpHwAccelCtx) pWhichTest->message + sent, toSend, &ctx);
            errors += UNITTEST_STATUS( hint, status);
            sent += toSend;
...
test_vector_prf_128(  AES_CBC_PRF_128_TestVector* pWhichTest, int hint)
{
    ...
    if (  0 == pWhichTest->messageLen)
    {
        status = AES_XCBC_PRF_128_init(MOC_SYM(gpHwAccelCtx)pWhichTest->key, pWhichTest->keyLen, &ctx);
        errors += UNITTEST_STATUS( hint, status);
    ...
    for ( i = 1; i <= pWhichTest->messageLen; ++i)
    {
        ...
        while ( sent < pWhichTest->messageLen)
        {
            ...
            status = AES_XCBC_PRF_128_update(MOC_SYM(gpHwAccelCtx) pWhichTest->message + sent, toSend, &ctx);
            errors += UNITTEST_STATUS( hint, status);
            sent += toSend;
...
int crypto_interface_aes_xcbc_mac_96_test_mac_96()
{
    int retVal = 0;
    
#if (!defined(__DISABLE_AES_CIPHERS__)) && (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__))
...
#endif /* #if (!defined(__DISABLE_AES_CIPHERS__)) && (!defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE_EXPORT__)) */
...
int crypto_interface_aes_xcbc_mac_96_test_prf_128()
{
    int retVal = 0;
