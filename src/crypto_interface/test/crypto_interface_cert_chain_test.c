/*
 * crypto_interface_cert_chain_test.c
 *
...
static CertChainTest gTests1[] =
{
    ...
    {
        FILE_PATH("ssl_msg_certificate1.dat"), (1 << digitalSignature) | (1  << keyEncipherment),
        3, 0,
...
#endif
    {
        FILE_PATH("ssl_msg_certificate2.dat"), (1 << digitalSignature) | (1  << keyEncipherment),
        3, 0,
        (const sbyte*)"gs-loc.apple.com\0\0",
        (const sbyte*)"server1.gs-loc.apple.com\0\0"
    },
    {
        FILE_PATH("ssl_msg_certificate3.dat"), (1 << digitalSignature),
        3, 0,
    ...
    {
        FILE_PATH("ssl_msg_certificate4.dat"), (1 << digitalSignature),
        3, 0,
    ...
    {
        FILE_PATH("ssl_msg_certificate5.dat"),  (1 << digitalSignature) | (1  << keyEncipherment),
        3, 1,
...
{
    FILE_PATH("Equifax_Secure_Certificate_Authority.der"),
    FILE_PATH("VerisignClass3RootG5.der"),
    FILE_PATH("VerisignClass3RootMD2.der"),
    FILE_PATH("VerisignClass3RootSHA1.der"),
};
...
{
    FILE_PATH("VerisignClass1PublicPrimaryCAMD2.der"),
    FILE_PATH("VerisignClass1PublicPrimaryCASHA1.der"),
};

/* These ones are fake trust points (issuer names are good but keys are wrong) */
static const char* gFakeRootCerts[] =
{
    FILE_PATH("FakeEquifax_Secure_Certificate_Authority.der"),
    FILE_PATH("FakeVerisignClass3RootG5.der"),
    FILE_PATH("FakeVerisignClass3RootMD2.der"),
    FILE_PATH("FakeVerisignClass3RootSHA1.der"),
};

static const sbyte4 kNumTests1 = COUNTOF(gTests1);

static ubyte gpRoot[944] = {
...
static ubyte4 gRootLen = 944;

static ubyte gpIntermediate[881] = {
...
static ubyte4 gIntermediateLen = 881;

static ubyte gpChild[847] = {
...
static ubyte4 gChildLen = 847;

/*----------------------------------------------------------------------------*/

static int BuildSSLCertChain(int hint, CertChainTest* pTest,
                             certChainPtr* pRetCertChain)
{
    ...
    UNITTEST_STATUS_GOTO(hint, CERTCHAIN_createFromSSLRecord(MOC_ASYM(gpHwAccelCtx)
                                                             &certChain,
                                                             msg,
                                                             msgLen),
                         retVal, exit);
    ...
    for (i = 0; i < numCertsInChain; ++i)
    {
        ...
        status = CERTCHAIN_getKey(MOC_ASYM(gpHwAccelCtx) certChain, i, &pubKey);
        retVal += UNITTEST_STATUS((hint << 16 | i), status);
    ...
    {
        vc.commonName = names;
        status = CERTCHAIN_validate(MOC_ASYM(gpHwAccelCtx) certChain, &vc);
...
static int cert_chain_test()
{
    ...
    {
        retVal += BuildSSLCertChain(i, gTests1 + i, pTestCertChains+i);
    }
    ...
    for ( i = 0; i < kNumTests1; ++i)
    {
        ...
        vc.anchorCert = 0;
        vc.anchorCertLen = 0;
        status = CERTCHAIN_validate(MOC_ASYM(gpHwAccelCtx) pTestCertChains[i], &vc);
...
static int validateSelfSigned()
{
    ...
    status = CERTCHAIN_validateAll(MOC_ASYM(gpHwAccelCtx)
...
static int validateFullChainTest(
    ubyte *pCert1,
    ubyte4 cert1Len,
    ubyte *pCert2,
    ubyte4 cert2Len,
...
static int test_validate_cert_chain(sbyte *pCertFile)
{
    int ret = 0;
    MSTATUS status;
    ubyte *pB64Encoded = NULL;
    ubyte4 b64EncodedLen = 0;
    ubyte *pCertMsg = NULL;
    ubyte4 certMsgLen = 0;
    ...
    status = DIGICERT_readFile(pCertFile, &pB64Encoded, &b64EncodedLen);
    if (OK != status)
        goto exit;

    status = BASE64_decodeMessage(pB64Encoded, b64EncodedLen, &pCertMsg, &certMsgLen);
    if (OK != status)
        goto exit;

    status = CERTCHAIN_createFromSSLRecordEx(MOC_ASYM(gpHwAccelCtx)
                              &pCertChain,
                              pCertMsg,
                              certMsgLen,
                              3); /* 3 == tls 1.2, 4 == tls 1.3 */
    if (OK != status)
        goto exit;

exit:
    DIGI_FREE((void **) &pB64Encoded);
    BASE64_freeMessage(&pCertMsg);
...
static int validateOutOfOrder()
{
    int errorCount = 0;

    errorCount += test_validate_cert_chain(FILE_PATH("leaf_int_root_chain.pem"));
    errorCount += test_validate_cert_chain(FILE_PATH("leaf_root_int_chain.pem"));
    errorCount += test_validate_cert_chain(FILE_PATH("int_leaf_root_chain.pem"));
    errorCount += test_validate_cert_chain(FILE_PATH("root_int_leaf_chain.pem"));

    return errorCount;
}

static int validateAgainstTrustStore()
{
    MSTATUS status, fstatus;
    ValidationConfig config = { 0 };
    certStorePtr pCertStore = NULL;
...
static int validateSingleCertNoTrustAnchor()
{
    MSTATUS status, fstatus;
    ValidationConfig config = { 0 };
    certStorePtr pCertStore = NULL;
...
static int validateCertChainNoTrustAnchor()
{
    MSTATUS status, fstatus;
    ValidationConfig config = { 0 };
    certStorePtr pCertStore = NULL;
...
static int validateInverseChildParentLink()
{
    MSTATUS status, fstatus;
    ValidationConfig config = { 0 };
    certStorePtr pCertStore = NULL;
...
static int validateMissingIntermediateLink()
{
    MSTATUS status, fstatus;
    ValidationConfig config = { 0 };
    certStorePtr pCertStore = NULL;
...
int crypto_interface_cert_chain_test_init ()
{
  ...
  /* START Tests */
  errorCount += cert_chain_test ();
  errorCount += test_validate_all ();
  errorCount += validateOutOfOrder();
  /* END   Tests */
exit:
