/*
 * crypto_interface_aes_ctr_tap.h
 *
 * Cryptographic Interface header file for declaring AES-CTR TAP functions
...
#ifndef __CRYPTO_INTERFACE_AES_CTR_TAP_HEADER__
#define __CRYPTO_INTERFACE_AES_CTR_TAP_HEADER__
...
/**
 * @brief   Create a new AES-CTR context from a previously existing TAP symmetric key.
 * @details Create a new AES-CTR context from a previously existing TAP symmetric key.
 ...
 *          \c CRYPTO_INTERFACE_DeleteAESCTRCtx in order to free these structures when done.
 *s
 * @param pSymKey   Pointer to the input \c SymmetricKey structure containing
 *                  a TAP key.
 * @param ppNewCtx  Location that will recieve a pointer to the new AES-CTR context.           
 * @param pIv       Buffer holding the 16 byte initialization vector.
 *
...
/**
 * @brief   Sets an AES-CTR context to not unload the internal TAP Key upon cipher completion. 
 * @details Sets an AES-CTR context to not unload the internal TAP Key upon cipher completion. 
 *
 * @param pCtx                Pointer to a TAP enabled AES-CTR context containing a TAP Key.
 ...
 *                  error code from merrors.h.
 */
MOC_EXTERN MSTATUS CRYPTO_INTERFACE_TAP_AesCtrGetKeyInfo (
    BulkCtx pCtx,
...
/**
 * @internal This API is private. One should use \c CRYPTO_INTERFACE_DoAESCTREx.
 */
...
/**
 * @internal This API is private. One should use \c CRYPTO_INTERFACE_DeleteAESCTRCtx.
 */
...
#endif /* __CRYPTO_INTERFACE_AES_CTR_TAP_HEADER__ */
