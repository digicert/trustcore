 ...
 * cast128.c
 *
 * CAST128 aka CAST5 Encipher & Decipher
 *
...
#if (defined(__ENABLE_CAST128_CIPHER__))
...
static const ubyte4 cast_sbox1[256] = {
...
static const ubyte4 cast_sbox2[256] = {
...
static const ubyte4 cast_sbox3[256] = {
...
static const ubyte4 cast_sbox4[256] = {
...
static const ubyte4 cast_sbox5[256] = {
...
static const ubyte4 cast_sbox6[256] = {
...
static const ubyte4 cast_sbox7[256] = {
...
static const ubyte4 cast_sbox8[256] = {
...
#define U8a(x) ( (ubyte) (x>>24) )
#define U8b(x) ( (ubyte) ((x>>16)& 0xFF) )
#define U8c(x) ( (ubyte) ((x>>8) & 0xFF) )
#define U8d(x) ( (ubyte) ((x)    & 0xFF) )
...
MSTATUS CAST128_encryptBlock(cast128_ctx* key,
                          ...
                          ubyte outblock[/*CAST128_BLOCK_SIZE*/])
{
	...
	l = ((ubyte4)inblock[0] << 24) | ((ubyte4)inblock[1] << 16) |
	 ((ubyte4)inblock[2] << 8) | (ubyte4)inblock[3];
	r = ((ubyte4)inblock[4] << 24) | ((ubyte4)inblock[5] << 16) |
	 ((ubyte4)inblock[6] << 8) | (ubyte4)inblock[7];
	/* Do the work */
	F1(l, r,  0);
	F2(r, l,  1);
	F3(l, r,  2);
...
MSTATUS CAST128_decryptBlock( cast128_ctx* key,
                          ubyte inblock[/*CAST128_BLOCK_SIZE*/],
                          ubyte outblock[/*CAST128_BLOCK_SIZE*/])
{
    ubyte4 t, l, r;
	...
	/* Get inblock into l,r */
	r = ((ubyte4)inblock[0] << 24) | ((ubyte4)inblock[1] << 16) |
	 ((ubyte4)inblock[2] << 8) | (ubyte4)inblock[3];
	l = ((ubyte4)inblock[4] << 24) | ((ubyte4)inblock[5] << 16) |
	 ((ubyte4)inblock[6] << 8) | (ubyte4)inblock[7];
	/* Do the work */
	/* Only do full 16 rounds if key length > 80 bits */
	if (key->rounds > 12) {
...
MSTATUS CAST128_initKey(cast128_ctx* key, const ubyte* rawkey, sbyte4 keybytes)
{
    ubyte4 t[4], z[4], x[4];
    sbyte4 i;
	...
	for (i = 0; i < 4; i++) {
		x[i] = 0;
		if ((i*4+0) < keybytes) x[i] = (ubyte4)rawkey[i*4+0] << 24;
		if ((i*4+1) < keybytes) x[i] |= (ubyte4)rawkey[i*4+1] << 16;
		if ((i*4+2) < keybytes) x[i] |= (ubyte4)rawkey[i*4+2] << 8;
		if ((i*4+3) < keybytes) x[i] |= (ubyte4)rawkey[i*4+3];
	}
...
extern BulkCtx
CreateCast128Ctx(MOC_SYM(hwAccelDescr hwAccelCtx) const ubyte* keyMaterial, sbyte4 keyLength, sbyte4 encrypt)
{
    cast128_ctx* xkey = (cast128_ctx*) MALLOC(sizeof(cast128_ctx));
    
    if (xkey)
    {
        CAST128_initKey( xkey, keyMaterial, (ubyte4) keyLength);
    }
...
extern MSTATUS
DoCast128(MOC_SYM(hwAccelDescr hwAccelCtx) BulkCtx ctx, ubyte* data, sbyte4 dataLength,
      sbyte4 encrypt, ubyte* iv)
{
    ...
    if (ctx)
    {
        cast128_ctx *xkey = (cast128_ctx*) ctx;
        
        if (dataLength % CAST128_BLOCK_SIZE)
        {
            status = ERR_CAST128_BAD_LENGTH;
            goto exit;
        }
        ...
        if (encrypt)
        {
            while (dataLength > 0)
            {
                ...
                for (i = 0; i < CAST128_BLOCK_SIZE; ++i)
                {
                    data[i] ^= iv[i];
                }
                
                /* encrypt */
                CAST128_encryptBlock( xkey, data, data);
                
                /* save into iv */
                DIGI_MEMCPY(iv, data, CAST128_BLOCK_SIZE);
                
                /* advance */
                dataLength -= CAST128_BLOCK_SIZE;
                data += CAST128_BLOCK_SIZE;
            }
        }
        else
        {
            ...
            {
                sbyte4 i;
                ubyte nextIV[ CAST128_BLOCK_SIZE];
                
                /* save block in next IV */
                DIGI_MEMCPY( nextIV, data, CAST128_BLOCK_SIZE);
                
                /* decrypt */
                CAST128_decryptBlock(xkey, data, data);
                ...
                /* put nextIV into iv */
                DIGI_MEMCPY(iv, nextIV, CAST128_BLOCK_SIZE);
