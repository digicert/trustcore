    ...
    )
{
    ...
    switch (keyOp)
    {
        ...
        case MOC_ASYM_OP_GET_LOCAL_TYPE:
            ...
            {
                status = OK;
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_KEY_ECC_P521_PUB_OPERATOR;
            }
            break;

        case MOC_ASYM_OP_CREATE:
        case MOC_ASYM_OP_CREATE_PUB:
            pMocAsymKey->KeyOperator = KeyOperatorEccNistP521;
            pMocAsymKey->localType = MOC_LOCAL_KEY_ECC_P521_PUB_OPERATOR;
            status = OK;
            break;

        case MOC_ASYM_OP_CREATE_PRI:
            pMocAsymKey->KeyOperator = KeyOperatorEccNistP521;
            pMocAsymKey->localType = MOC_LOCAL_KEY_ECC_P521_PRI_OPERATOR;
            status = OK;
            break;

        case MOC_ASYM_OP_GENERATE:
            status = EccMbedGenerateKeyPair(
                pMocCtx, KeyOperatorEccNistP521,
        ...
        case MOC_ASYM_OP_PUB_FROM_PRI:
            status = EccMbedGetPubFromPri(
                pMocAsymKey, KeyOperatorEccNistP521,
        ...
        case MOC_ASYM_OP_SET_KEY_DATA:
            status = EccMbedSetKeyData(
                pMocAsymKey, KeyOperatorEccNistP521,
                MOC_LOCAL_KEY_ECC_P521_PUB_OPERATOR, (MEccKeyTemplate *)pInputInfo,
                MBEDTLS_ECP_DP_SECP521R1);
            break;
...
#endif /* __ENABLE_DIGICERT_ECC_P521_MBED__ */
