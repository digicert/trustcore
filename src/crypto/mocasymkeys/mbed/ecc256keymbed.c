...
#ifdef __ENABLE_DIGICERT_ECC_P256_MBED__
    ...
    )
{
    ...
    switch (keyOp)
    {
        ...
        case MOC_ASYM_OP_GET_LOCAL_TYPE:
            ...
            {
                status = OK;
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_KEY_ECC_P256_PUB_OPERATOR;
            }
            break;

        case MOC_ASYM_OP_CREATE:
        case MOC_ASYM_OP_CREATE_PUB:
            pMocAsymKey->KeyOperator = KeyOperatorEccNistP256;
            pMocAsymKey->localType = MOC_LOCAL_KEY_ECC_P256_PUB_OPERATOR;
            status = OK;
            break;

        case MOC_ASYM_OP_CREATE_PRI:
            pMocAsymKey->KeyOperator = KeyOperatorEccNistP256;
            pMocAsymKey->localType = MOC_LOCAL_KEY_ECC_P256_PRI_OPERATOR;
            status = OK;
            break;

        case MOC_ASYM_OP_GENERATE:
            status = EccMbedGenerateKeyPair(
                pMocCtx, KeyOperatorEccNistP256,
        ...
        case MOC_ASYM_OP_PUB_FROM_PRI:
            status = EccMbedGetPubFromPri(
                pMocAsymKey, KeyOperatorEccNistP256,
        ...
        case MOC_ASYM_OP_SET_KEY_DATA:
            status = EccMbedSetKeyData(
                pMocAsymKey, KeyOperatorEccNistP256,
                MOC_LOCAL_KEY_ECC_P256_PUB_OPERATOR, (MEccKeyTemplate *)pInputInfo,
                MBEDTLS_ECP_DP_SECP256R1);
            break;
