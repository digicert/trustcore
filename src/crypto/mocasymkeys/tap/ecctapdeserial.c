...
} /* DeserializeEccTapKey */

/*----------------------------------------------------------------------------*/

MSTATUS ReadEccTapKeyBlob (
  ubyte *pSerializedKey,
  ubyte4 serializedKeyLen,
  TAP_Key **ppTapKey
  )
{
  ...
  if (MOC_ECC_BLOB_START > serializedKeyLen)
    goto exit;

  /* Make sure the prefix is what we expect */
  status = DIGI_MEMCMP (
    (void *)pSerializedKey, (void *)pBlobStart, MOC_ECC_TAP_BLOB_START_LEN,
    &cmpResult);
  if (OK != status)
    goto exit;
  ...
  pTapBlob = pSerializedKey + MOC_ECC_TAP_BLOB_START_LEN;
  tapBlobLen = serializedKeyLen - MOC_ECC_TAP_BLOB_START_LEN;
  ...
  )
{
  ...
  /* For the ECC TAP Key, the privateKeyData is defined as follows:
  * SEQ {
  *   Version version,
  *   ModuleId moduleId,
  ...
  * }
  *
  * Since we only support named curves, we only look for the curve OID.
  */
  ...
  status = ASN1_compareOID (
    pAlgId, MOP_ECC_TAP_KEY_ALG_ID_LEN, pGetAlgId, getAlgIdLen,
    NULL, &cmpResult);
  if (OK != status)
    goto exit;
  ...
  if (0 != cmpResult) /* might be a pw protected key */
  {
    ...
    status = ASN1_compareOID (pAlgId, MOP_ECC_TAP_KEY_ALG_ID_LEN, pGetAlgId, getAlgIdLen, NULL, &cmpResult);
    if (OK != status)
      goto exit;
