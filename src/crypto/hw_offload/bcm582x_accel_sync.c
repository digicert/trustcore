/*
 * bcm582x_accel_sync.c
 *
 * Broadcom 580x/582x Hardware Acceleration Synchronous Adapter
 *
...
#if (defined(__ENABLE_HARDWARE_ACCEL_SYNC_CRYPTO__) && defined(__ENABLE_BCM582x_HARDWARE_ACCEL__))
...
#include </brcm/linuxsdk/srl/ubsec.h>
#include </brcm/linuxsdk/linux/ubsio.h>
#include </brcm/linuxsdk/ubslib/ubsec_lib.h>
...
extern MSTATUS
BCM582x_openChannel(enum moduleNames moduleId, hwAccelDescr *pHwAccelCookie)
{
    MSTATUS status;

    DEBUG_CONSOLE_printError(DEBUG_CRYPTO, "BCM582x_openChannel: Mocana module = ", (sbyte4)moduleId);
...
extern MSTATUS
BCM582x_closeChannel(enum moduleNames moduleId, hwAccelDescr *pHwAccelCookie)
{
    MSTATUS status = OK;

    DEBUG_CONSOLE_printError(DEBUG_CRYPTO, "BCM582x_closeChannel: Mocana module = ", (sbyte4)moduleId);
...
RANDOM_acquireContext(randomContext **pp_randomContext)
{
    ...
    status = BCM582x_openChannel(MOCANA_MSS, &hwAccelCtx);
...
{
    hwAccelDescr    hwAccelCtx = (hwAccelDescr)(*pp_randomContext);
    MSTATUS         status;
    ...
    status = BCM582x_closeChannel(MOCANA_MSS, &hwAccelCtx);
...
{
    hwAccelDescr    hwAccelCtx = (hwAccelDescr)pRandomContext;
    MSTATUS         status;
...
PRIME_generateSizedPrime(randomContext *pRandomContext, vlong **ppRetPrime, ubyte4 numBitsLong)
{
    ...
    *ppRetPrime = NULL;
    ...
    if (OK > (status = VLONG_allocVlong(&pPrime, NULL)))
        goto exit;
...
{
    MD5_CTX         context;
    unsigned char   kpad[MD5_BLOCK_SIZE];
    ...
    /* XOR key padded with 0 to MD5_BLOCK_SIZE with 0x36 */
    for (i=0; i < keyLen; ++i)
    ...
    if (OK > (status = MD5Init_m(hwAccelCtx, &context)))
        goto exit;
    if (OK > (status = MD5Update_m(hwAccelCtx, &context, kpad, MD5_BLOCK_SIZE)))
        goto exit;
...
{
    shaDescr        context;
    unsigned char   kpad[SHA1_BLOCK_SIZE];
    ...
    if (OK > (status = SHA1_initDigest(hwAccelCtx, &context)))
        goto exit;
    if (OK > (status = SHA1_updateDigest(hwAccelCtx, &context, kpad, SHA1_BLOCK_SIZE)))
        goto exit;
    ...
    /* XOR key padded with 0 to SHA1_BLOCK_SIZE with 0x5C*/
    for (i = 0; i < keyLen; i++)
        kpad[i] = (ubyte)(key[i] ^ OPAD);
    for (; i < SHA1_BLOCK_SIZE; i++)
        kpad[i] = (ubyte)(0 ^ OPAD);
...
VLONG_operatorModSignedVlongs(hwAccelDescr hwAccelCtx, vlong* a, vlong* n, vlong **ppC)
{
    ...
    if (OK > (status = VLONG_allocVlong(ppC, NULL)))
        goto exit;
    ...
    if (0 > (status = (MSTATUS)ubsec_modrem(hwAccelCtx,
                                            (unsigned char *)a->pUnits, &a_bits,
                                            (unsigned char *)n->pUnits, &n_bits,
                                            (unsigned char *)((*ppC)->pUnits), &c_bits)))
    {
        goto exit;
    }
...
VLONG_modularInverse(hwAccelDescr hwAccelCtx, vlong *b, vlong *n, vlong **ppT)
{
    ...
    if (0 > (status = (MSTATUS)ubsec_modinv(hwAccelCtx,
                                            (unsigned char *)n->pUnits, &n_bits,
                                            (unsigned char *)b->pUnits, &b_bits,
                                            (unsigned char *)((*ppT)->pUnits), &t_bits)))
    {
        goto exit;
    }
...
VLONG_modexp(hwAccelDescr hwAccelCtx, vlong *a, vlong *e, vlong *n, vlong **ppResult)
{
    ...
    if (OK > (status = VLONG_allocVlong(ppResult, NULL)))
        goto exit;
    ...
    if (0 > (status = (MSTATUS)math_accelerate_ioctl(hwAccelCtx, UBSEC_MATH_MODEXP,
                                                     (unsigned char *)n->pUnits, &ModN_len,
                                                     (unsigned char *)e->pUnits, &ExpE_len,
                                                     (unsigned char *)a->pUnits, &ParmA_len,
                                                     0, 0,
                                                     (unsigned char *)((*ppResult)->pUnits), &Result_len)))
    {
        goto exit;
    }
...
#endif /* (defined(__ENABLE_HARDWARE_ACCEL_SYNC_CRYPTO__) || defined(__ENABLE_BCM5823_HARDWARE_ACCEL__)) */
