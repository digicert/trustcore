 ...
 *
 * Adapter to Mocana SoftChip Asynchronous Accelerator
 *
...
#if (defined(__ENABLE_HARDWARE_ACCEL_ASYNC_CRYPTO__) && defined(__ENABLE_DIGICERT_SOFTCHIP__))
...
typedef struct SECdescriptor {
    ...
    struct SECdescriptor *nextDescriptorPtr;
...
extern MSTATUS
SOFTCHIP_openChannel(enum moduleNames moduleId, sbyte4 *pHwAccelCookie)
{
    /* open socket to the softchip */
...
extern MSTATUS
SOFTCHIP_closeChannel(enum moduleNames moduleId, sbyte4 *pHwAccelCookie)
{
    /* close socket to the softchip */
...
extern MSTATUS
VLONG_modexp(hwAccelDescr hwAccelCtx, vlong *a, vlong *e, vlong *n, vlong **ppResult, vlong **ppVlongQueue)
{
    int     Result_len;
    int     ModN_len;
    int     ExpE_len;
    ...
    if (OK > (status = VLONG_allocVlong(ppResult, ppVlongQueue)))
        goto exit;

    if (OK > (status = VLONG_reallocVlong(*ppResult, n->numUnitsUsed, ppVlongQueue)))
        goto exit;
...
exit:
    if (OK > status)
        VLONG_freeVlong(ppResult, ppVlongQueue);
