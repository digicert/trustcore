...
static MSTATUS TDesTapGenerate (
    MocCtx pMocCtx,
    ...
    )
{
    ...
    status = CRYPTO_createMocSymCtx (
        MTDesTapOperator, (void *)pKeyGenArgs, pMocCtx, &pNewCtx);
    if (OK != status)
        goto exit;
    ...
    /* Default to a mode-less DES key with no usage defined */
    ...
    {
        keyInfo.algKeyInfo.tdesInfo.symMode = pKeyGenArgs->symMode;
        pKeyData->symMode = pKeyGenArgs->symMode;
    }

    /* We ignore the keySize in pKeyGenArgs, it should have already been validated */
    ...
    )
{
    ...
    switch (symOp)
    {
        case MOC_SYM_OP_GET_LOCAL_TYPE:
            if (NULL != pOutputInfo)
            {
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_TYPE_TDES_TAP;
                status = OK;
            }
            break;

        case MOC_SYM_OP_CREATE:
            status = SymTapCreate(pMocSymCtx, (MSymTapKeyGenArgs *) pInputInfo, MOC_LOCAL_TYPE_TDES_TAP, MTDesTapOperator);
            break;

        case MOC_SYM_OP_GENERATE_KEY_EX:
            status = TDesTapGenerate (
