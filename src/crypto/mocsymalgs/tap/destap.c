    ...
    )
{
    ...
    status = CRYPTO_createMocSymCtx (
        MDesTapOperator, (void *)pKeyGenArgs, pMocCtx, &pNewCtx);
    if (OK != status)
        goto exit;
    ...
    {
        keyInfo.algKeyInfo.desInfo.symMode = pKeyGenArgs->symMode;
    ...
    )
{
    ...
    switch (symOp)
    {
        case MOC_SYM_OP_GET_LOCAL_TYPE:
            if (NULL != pOutputInfo)
            {
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_TYPE_DES_TAP;
                status = OK;
            }
            break;

        case MOC_SYM_OP_CREATE:
            status = SymTapCreate(pMocSymCtx, (MSymTapKeyGenArgs *) pInputInfo, MOC_LOCAL_TYPE_DES_TAP, MDesTapOperator);
            break;

        case MOC_SYM_OP_GENERATE_KEY_EX:
            status = DesTapGenerate (
