    ...
    )
{
    ...
    switch (symOp)
    {
        ...
        case MOC_SYM_OP_GET_LOCAL_TYPE:
            ...
            if (NULL != pOutputInfo)
            {
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_TYPE_AES_CTR_OPERATOR;
                status = OK;
            }
            break;
        ...
        case MOC_SYM_OP_CREATE:
            status = MAesMbedCreate(
                ...
                MOC_LOCAL_TYPE_AES_CTR_OPERATOR, SymOperatorAesCtr);
            break;

         case MOC_SYM_OP_UPDATE_OP_DATA:
            {
                MAesMbedInfo *pInfo = NULL;
                ...
                pInfo = (MAesMbedInfo *) pMocSymCtx->pLocalData;
                status = MAesCtrMbedUpdateInfo(pMocSymCtx, (MAesCtrUpdateData *) pInputInfo);

                DIGI_MEMCPY(pInfo->pWorkingIv, pInfo->pIv, 16);
            }
            break;
        ...
        case MOC_SYM_OP_GET_OP_DATA:
            ...
            status = MAesCtrMbedGetCounterBlock(pMocSymCtx->pLocalData, (MSymOperatorData *) pOutputInfo);
            break;
    }
