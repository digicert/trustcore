...
#define SHA512_START_FLAG 0

int MbedSha512StartWrapper(
  void *pCtx
  );

extern MSTATUS SymOperatorSha512(
  ...
  )
{
  ...
  switch (symOp)
  {
    ...
    case MOC_SYM_OP_GET_LOCAL_TYPE:
      ...
      if (NULL != pOutputInfo)
      {
        *((ubyte4 *)pOutputInfo) = MOC_LOCAL_TYPE_SHA512_OPERATOR;
        status = OK;
      }
      break;
    ...
    case MOC_SYM_OP_CREATE:
      status = MbedDigestCreate(
        pMocSymCtx, SymOperatorSha512, pInputInfo,
        sizeof(mbedtls_sha512_context), MOC_LOCAL_TYPE_SHA512_OPERATOR);
      break;
    
    case MOC_SYM_OP_DIGEST_INIT:
      status = MbedDigestInit(
        pMocSymCtx, (MbedDigestInitFunc) mbedtls_sha512_init,
        MbedSha512StartWrapper);
      break;
    ...
    case MOC_SYM_OP_DIGEST_SIZE:
      ...
      if (NULL != pOutputInfo)
      {
        *((ubyte4 *)pOutputInfo) = SHA512_RESULT_SIZE;
        status = OK;
      }
      break;
    ...
    case MOC_SYM_OP_BLOCK_SIZE:
      ...
      if (NULL != pOutputInfo)
      {
        *((ubyte4 *)pOutputInfo) = SHA512_BLOCK_SIZE;
        status = OK;
      }
      break;
...
{
  return mbedtls_sha512_starts_ret(pCtx, SHA512_START_FLAG);
}
#endif
