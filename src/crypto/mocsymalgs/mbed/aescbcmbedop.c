    ...
    )
{
    ...
    switch (symOp)
    {
        ...
        case MOC_SYM_OP_GET_LOCAL_TYPE:
            ...
            if (NULL != pOutputInfo)
            {
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_TYPE_AES_CBC_OPERATOR;
                status = OK;
            }
            break;
        ...
        case MOC_SYM_OP_CREATE:
            status = MAesMbedCreate(
                ...
                MOC_LOCAL_TYPE_AES_CBC_OPERATOR, SymOperatorAesCbc);
            break;
        ...
        case MOC_SYM_OP_DECRYPT_INIT:
            status = MAesMbedInit(
                pMocSymCtx, mbedtls_aes_setkey_dec, MBEDTLS_AES_DECRYPT);
            break;
