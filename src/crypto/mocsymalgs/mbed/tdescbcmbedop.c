    ...
    )
{
    ...
    switch (symOp)
    {
        ...
        case MOC_SYM_OP_GET_LOCAL_TYPE:
            ...
            if (NULL != pOutputInfo)
            {
                *((ubyte4 *) pOutputInfo) = MOC_LOCAL_TYPE_TDES_CBC_OPERATOR;
                status = OK;
            }
            break;
        ...
        case MOC_SYM_OP_CREATE:
            status = MDesMbedCreate(
                pMocSymCtx, pInputInfo, MOC_LOCAL_TYPE_TDES_CBC_OPERATOR, SymOperatorTDesCbc);
            break;
