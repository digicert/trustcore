 ...
 * locality.c
 *
 * Functions for handling the LocalityName element of an X.500 Name.
 *
...
extern MSTATUS NameTypeLocality (
  ubyte4 operation,
  ubyte *pValue,
  ubyte4 valueLen,
  void *pInfo
...
{
  MSTATUS status;
  ubyte pOid[MOP_LOCALITY_NAME_OID_LEN] = { MOP_LOCALITY_NAME_OID };
  ...
  switch (operation)
  {
    ...
    case MOC_NAME_OP_DECODE_RDN:
      status = MGetSimpleValue (
        (MGetAttributeData *)pInfo, pOid, MOP_LOCALITY_NAME_OID_LEN);
      goto exit;
    ...
    case MOC_NAME_OP_ENCODE_RDN:
      ...
      status = MEncodeSimpleRdnAlloc (
        pOid + 2, MOP_LOCALITY_NAME_OID_LEN - 2,
