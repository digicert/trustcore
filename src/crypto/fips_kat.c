...
FIPS_NIST_CTRDRBG_DoKAT(sbyte4 useDf, const NIST_DRBG_TestVectorPR *pTest, sbyte4 keyLen)
{
    ...
    status = NIST_CTRDRBG_reseed_internal( MOC_SYM(hwAccelCtx)
...
static const ubyte expect[] = {
  
};
...
MOC_EXTERN MSTATUS FIPS_rsaKat(hwAccelDescr hwAccelCtx)
{
    ...
    ubyte           hashId = sha512withRSAEncryption;
    ubyte*          pSignature = NULL;
    RSAKey*         pRSAKey = NULL;
    RSAKey*         pRSAPublicKey = NULL;
    vlong*          pQueue  = NULL;
    sbyte4          signatureTextLen = 0;
    MSTATUS         status = OK;
    ubyte*          pKeyBlob = NULL;
    sbyte4          resCmp;
    intBoolean      validity = -1;

    if (FIPS_TESTLOG_ENABLED)
        FIPS_startTestMsg("FIPS_rsaKat", "RSA-2048");
    ...
    /* CMVP/Verbose test log */
    FIPS_TESTLOG_FMT(1061, "FIPS_rsaKat: RSA N size = %d", VLONG_bitLength(RSA_N(pRSAKey)));

    /* Get the cipher text length */
    if (OK > (status = RSA_getCipherTextLength(pRSAKey, &signatureTextLen)))
    {
    ...
    if (signatureTextLen != sizeof(expect))
    {
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    }
    ...
    /* Allocate memory for signature text */
    pSignature = MALLOC(signatureTextLen);
    if (NULL == pSignature)
    {
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    ...
    /* Clear all memory */
    DIGI_MEMSET(pSignature, 0x00, signatureTextLen);

    if (FIPS_FORCE_FAIL_RSA_SIGGEN_TEST)
    {
        testmsg[0] ^= 0x01;
    }

    /* RSA signature/verification API 'RSA_[sign,verify]Data' will use the identfied digest to create or
     * verify the signature.
     */
    /* Calculate the signature */
    if (OK > (status = RSA_signData(MOC_RSA(hwAccelCtx) pRSAKey,
                                    (ubyte*)testmsg, (ubyte4)DIGI_STRLEN(testmsg)+1,
                                    hashId, pSignature, &pQueue)))
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - rsaKat - RSA_signData - Failed");
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    }

    /* Verify the Cipher Text with the expected data */
    DIGI_CTIME_MATCH( pSignature, expect, signatureTextLen, &resCmp);
    if (0 != resCmp)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - rsaKat - Signing - Failed");
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_RSA_SIGVER_TEST)
    {
        testmsg[0] ^= 0x01;
    }

    /* Verify the signature */
    if (OK > (status = RSA_verifyData(MOC_RSA(hwAccelCtx) pRSAPublicKey,
                                      (ubyte*)testmsg, (ubyte4)DIGI_STRLEN(testmsg)+1,
                                      hashId, pSignature, signatureTextLen,
                                      &validity, &pQueue)))
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - rsaKat - RSA_verifyData - Failed");
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    }
    ...
    if (TRUE != validity)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - rsaKat - Verification - Failed");
        status = ERR_FIPS_RSA_KAT_FAILED;
        goto exit;
    }
...
exit:
    ...
    if (FIPS_TESTLOG_ENABLED)
        FIPS_endTestMsg("FIPS_rsaKat", "RSA-2048", status);
...
FIPS_dsaKat(hwAccelDescr hwAccelCtx)
{
    ...
    status = NIST_CTRDRBG_newContext_internal(MOC_SYM(hwAccelCtx) &pRandomContext,
    ...
    if (FIPS_FORCE_FAIL_DSA_SIGGEN_TEST)
    {
        pMsg[0] ^= 0x01;  
    }
    ...
    if (0 != cmpRes)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - dsaKat - Signing (pR) - Failed");
        status = ERR_FIPS_DSA_FAIL;
        goto exit;
    }
    ...
    if (0 != cmpRes)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - dsaKat - Signing (pS) - Failed");
        status = ERR_FIPS_DSA_FAIL;
        goto exit;
    }

    if (FIPS_FORCE_FAIL_DSA_SIGVER_TEST)
    {
        pMsg[0] ^= 0x01;
    }
    ...
    /* Verify the signature */
    if (OK > (status = DSA_verifySignature(MOC_DSA(hwAccelCtx) pDSAKey, pBuff, pR, pS, &isGoodSig, NULL)))
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - dsaKat - DSA_verifySignature - Failed");
        goto exit;
    }

    if (FALSE == isGoodSig)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - dsaKat - Verification - Failed");
        status = ERR_FIPS_DSA_FAIL;
        goto exit;
    }
...
FIPS_ecdsaKat(hwAccelDescr hwAccelCtx)
{
    ...
    /* ECDSA signature = 2 x 28 bytes, stored in 'r' and 's' */
    static const ubyte pR_expect[] = {
    ...
    if (FIPS_FORCE_FAIL_ECDSA_SIGGEN_TEST)
    {
        msghash[0] ^= 0x01;
    }
    ...
    /* Check the signature for the hash after it passed KAT test */
    DIGI_MEMCPY((void*)msghash, (void*)hash, hashLen);

    if (FIPS_FORCE_FAIL_ECDSA_SIGVER_TEST)
    {
        msghash[0] ^= 0x01;
    }

    status = ECDSA_verifySignature(pNewKey->pCurve, pNewKey->Qx, pNewKey->Qy,
                                   msghash, hashLen, r, s);
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - ecdsaKat - ECDSA_verifySignature - Bad signature");
        goto exit;
    }
...
MOC_EXTERN MSTATUS
FIPS_eddsa25519Kat(hwAccelDescr hwAccelCtx)
{
    ...
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - edECC_newKey - Failed");
        goto exit;
    }
    ...
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - edECC_setKeyParameters - Failed");
        goto exit;
    }

    DIGI_MEMCPY((void*)msg, (void*)mesg, mesgLen);

    if (FIPS_FORCE_FAIL_EDDSA2_SIGGEN_TEST)
    {
        msg[0] ^= 0x01;
    }
    ...
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - edDSA_Sign - Failed");
        goto exit;
    }
    ...
    if (OK != DIGI_CTIME_MATCH(pSignature, pSig_expect, signatureLen, &cmpRes))
    {
        ...
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - DIGI_CTIME_MATCH(pSig) - Failed");
        goto exit;
    }
    if (0 != cmpRes)
    {
        status = ERR_FIPS_EDDSA_SIGN_VERIFY_FAIL;
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - cmpRes(pSig) - Failed");
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_EDDSA2_SIGVER_TEST)
    {
        msg[0] ^= 0x01;
    }

    pKey->isPrivate = FALSE;
    status = edDSA_VerifySignature(pKey, (ubyte *) msg, mesgLen, pSignature, signatureLen, &verifyStatus,
                                   &shaSuite, FALSE, NULL, 0, NULL);
    if (OK != verifyStatus)
    {
        status = ERR_FIPS_EDDSA_SIGN_VERIFY_FAIL;
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa25519Kat - edDSA_VerifySignature - Failed");
        goto exit;
    }
...
MOC_EXTERN MSTATUS
FIPS_eddsa448Kat(hwAccelDescr hwAccelCtx)
{
    BulkHashAlgo    shaSuite = {0};
    edECCKey*       pKey = NULL;
    ubyte           pSignature[57*2] = {0};
    ...
    const ubyte pSig_expect[] =
    {
        0xe9, 0xe6, 0xa2, 0xd0, 0xe7, 0xa3, 0x48, 0x54, 0x24, 0x78, 0xfa, 0x35,
        0x9c, 0x47, 0x45, 0x8d, 0x20, 0x92, 0xce, 0x31, 0xcd, 0x57, 0x92, 0xdc,
        0xb4, 0xb8, 0x15, 0x6c, 0xda, 0x90, 0x84, 0xa5, 0x6b, 0x6b, 0x43, 0x43,
        0xee, 0x6c, 0x43, 0xdc, 0x64, 0xba, 0xcd, 0x76, 0x3d, 0x7a, 0xad, 0x83,
        0x7d, 0x78, 0x21, 0x7d, 0xdc, 0xdb, 0x86, 0x56, 0x80, 0x1f, 0x6b, 0x86,
        0xb5, 0x5a, 0x46, 0xa7, 0x86, 0xa6, 0x18, 0x5d, 0x2b, 0xc8, 0x6b, 0x65,
        0xe1, 0x00, 0x4b, 0xe2, 0xb7, 0x4c, 0x4b, 0xa9, 0x94, 0x55, 0x59, 0x4a,
        0x27, 0xee, 0x43, 0xd4, 0xa2, 0x87, 0x23, 0xc9, 0x88, 0x79, 0x37, 0x8e,
        0x84, 0xca, 0x2d, 0xd2, 0x75, 0xf2, 0xe6, 0x2d, 0xdd, 0x37, 0x86, 0xe8,
        0x2e, 0xdf, 0x3d, 0xb3, 0x0d, 0x00
    };

    static ubyte gpP448priv[57] =
    {
    ...
    if (FIPS_TESTLOG_ENABLED)
        FIPS_startTestMsg("FIPS_eddsaKat", "EdDSA-448");

    status = edECC_newKey(&pKey, curveEd448, NULL);
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - edECC_newKey - Failed");
        goto exit;
    }

    /* Create signature */
    status = edECC_setKeyParameters(pKey, gpP448pub, sizeof(gpP448pub),
                                    gpP448priv, sizeof(gpP448priv), NULL, NULL);
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - edECC_setKeyParameters - Failed");
        goto exit;
    }

    DIGI_MEMCPY((void*)msg, (void*)mesg, mesgLen);

    if (FIPS_FORCE_FAIL_EDDSA4_SIGGEN_TEST)
    {
        msg[0] ^= 0x01;
    }
    ...
    if (OK != status)
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - edDSA_Sign - Failed");
        goto exit;
    }
    ...
    {
        status = ERR_FIPS_EDDSA_SIGN_VERIFY_FAIL;
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - DIGI_CTIME_MATCH(pSig) - Failed");
        goto exit;
    }
    if (0 != cmpRes)
    {
        status = ERR_FIPS_EDDSA_SIGN_VERIFY_FAIL;
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - cmpRes(pSig) - Failed");
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_EDDSA4_SIGVER_TEST)
    {
        msg[0] ^= 0x01;
    }
    ...
    {
        status = ERR_FIPS_EDDSA_SIGN_VERIFY_FAIL;
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - eddsa448Kat - edDSA_VerifySignature - Failed");
        goto exit;
    }

exit:
    if (FIPS_TESTLOG_ENABLED)
        FIPS_endTestMsg("FIPS_eddsaKat", "EdDSA-448", status);
...
#if (defined(__ENABLE_DIGICERT_ECC_EDDSA_25519__) || \
     defined(__ENABLE_DIGICERT_ECC_EDDSA_448__))
MOC_EXTERN MSTATUS
...
#if (defined(__ENABLE_DIGICERT_ECC_EDDSA_25519__))
    status = FIPS_eddsa25519Kat(hwAccelCtx);
    if (OK != status)
        goto exit;
#endif
#if (defined(__ENABLE_DIGICERT_ECC_EDDSA_448__))
    status = FIPS_eddsa448Kat(hwAccelCtx);
    if (OK != status)
        goto exit;
#endif
...
static const ubyte kat_key_input[] = {
  0x17, 0xfc, 0xdd, 0xbd, 0x4c, 0x6c, 0xdc, 0xa2, 0x82, 0xaa, 0xc0, 0x44,
  0x65, 0x4f, 0x43, 0x08, 0x6b, 0xf5, 0x59, 0x1d, 0xd3, 0x80, 0x25, 0x93,
  0xe4, 0x2a, 0xb2, 0x15, 0x32, 0x34, 0x98, 0xd2
};
static const ubyte4 kat_key_input_len = sizeof(kat_key_input);
                  ...
                  ubyte **ppRetIv)
{
    ...
    *pRetDataLen = 32;
    ...
    if (OK > (status = DIGI_MEMCPY(pTempKey, kat_key_input,
                                   (keyLen>kat_key_input_len)?kat_key_input_len:keyLen)))
        goto exit;
                       ...
                       ubyte **ppRetHashData, ubyte4 *pRetHashDataLen)
{
    ...
    *pRetHashDataLen = 32;
    ...
    if (OK > (status = DIGI_MEMCPY(pTempKey, kat_key_input,
                                   (*pRetKeyLen>kat_key_input_len)?kat_key_input_len:*pRetKeyLen)))
        goto exit;
...
{
    ubyte expect[] = {
        0x4d, 0xb3, 0x16, 0x12, 0x72, 0x53, 0x8f, 0x19, 0xcb, 0xee, 0x76, 0x8f,
        0x95, 0x8b, 0x8f, 0xe9, 0x58, 0xa6, 0xf5, 0xad
    };
...
{
    ubyte expect[] = {
        0x27, 0x76, 0x1b, 0x80, 0x15, 0xac, 0xa3, 0x91, 0xbb, 0x1d, 0xa3, 0x48,
        0x8c, 0x6b, 0xb4, 0xac, 0x33, 0x49, 0x51, 0x5e, 0x12, 0xdf, 0x70, 0x11,
        0x30, 0x5a, 0xfe, 0x79
    };
...
{
    ubyte expect[] = {
        0x77, 0xd7, 0x99, 0x92, 0x3c, 0xae, 0xea, 0x12, 0x5f, 0x72, 0xf7, 0x48,
        0xad, 0x65, 0x4d, 0x3d, 0x7b, 0x08, 0xef, 0xdf, 0xc1, 0x84, 0x8d, 0x66,
        0xac, 0x83, 0x7f, 0x3d, 0x6e, 0xdd, 0x49, 0x8c
    };
...
{
    ubyte expect[] = {
        0x26, 0xab, 0xd6, 0x37, 0xdb, 0x7a, 0x9e, 0xa2, 0xcf, 0xec, 0x6f, 0x05,
        0xd6, 0x66, 0x44, 0x88, 0xa1, 0xfe, 0x07, 0x20, 0x90, 0xac, 0xa9, 0xac,
        0x86, 0xde, 0x63, 0xcb, 0x8a, 0x33, 0x7b, 0x5d, 0xd2, 0xa0, 0xaf, 0x00,
        0x41, 0x35, 0xfa, 0x69, 0xa9, 0x91, 0x7c, 0xd8, 0x11, 0xe5, 0x8c, 0x0c
    };
...
{
    ubyte expect[] = {
        0x39, 0x83, 0xf6, 0x55, 0x02, 0x0e, 0xb4, 0x20, 0x76, 0x0a, 0xe1, 0xc7,
        0xd2, 0x0c, 0xcb, 0xfa, 0xce, 0x29, 0xc9, 0x5b, 0x68, 0xf7, 0xc8, 0xb5,
        0x2e, 0x3b, 0xf6, 0x48, 0xea, 0x50, 0xd8, 0x55, 0x4a, 0xdb, 0x4c, 0xaa,
        0x4e, 0x1b, 0xce, 0xb9, 0xc3, 0x1c, 0x53, 0x8e, 0x53, 0x4b, 0x56, 0xd3,
        0x61, 0xce, 0x33, 0xa9, 0x8c, 0x41, 0x89, 0x13, 0x09, 0x94, 0x5b, 0x71,
        0x54, 0x39, 0x53, 0x7d
    };
...
{
    ubyte expect[] = {
        0xb2, 0xad, 0xa9, 0x92, 0x95, 0xe1, 0x73, 0xed, 0x82, 0xd0, 0x2b, 0x3d,
        0xbe, 0xec, 0xf8, 0xf5, 0x66, 0x85, 0x68, 0xe6, 0xbc, 0x7e, 0xfd, 0xe4,
        0xa0, 0xc3, 0xf2, 0x6b
    };
...
{
    ubyte expect[] = {
        0xb5, 0x68, 0xc3, 0x35, 0xea, 0xbd, 0x53, 0xcd, 0xb6, 0x6f, 0xed, 0xbf,
        0xcd, 0x5c, 0xc7, 0x46, 0x77, 0x95, 0x24, 0x3f, 0x34, 0x12, 0x98, 0x19,
        0x99, 0xa9, 0xd1, 0x73, 0xbd, 0xbc, 0xfe, 0xbe
    };
...
{
    ubyte expect[] = {
        0xec, 0x62, 0x74, 0xbb, 0xa8, 0x4c, 0x77, 0x01, 0xa6, 0xcd, 0x05, 0x4d,
        0xda, 0xeb, 0xab, 0x08, 0x1b, 0x09, 0x4c, 0xf4, 0x64, 0x54, 0x91, 0x85,
        0x17, 0x6f, 0xf7, 0x7a, 0x3f, 0xca, 0x7b, 0x97, 0xab, 0x7e, 0x84, 0x2e,
        0x65, 0x10, 0xb9, 0xaf, 0xb2, 0x37, 0x19, 0x48, 0x4c, 0x05, 0x04, 0xdc
    };
...
{
    ubyte expect[] = {
        0x97, 0xfb, 0x50, 0xfe, 0x53, 0x6d, 0x82, 0x7a, 0x32, 0x5a, 0xd5, 0x0f,
        0x25, 0x48, 0x1c, 0x40, 0x7b, 0x91, 0x08, 0xa5, 0x48, 0x71, 0xee, 0xbf,
        0xea, 0x02, 0x08, 0x4e, 0xeb, 0x69, 0x51, 0x8f, 0x7c, 0xe5, 0x40, 0x7c,
        0xa9, 0xc5, 0x81, 0x18, 0x57, 0xb5, 0xde, 0x04, 0x4f, 0x7e, 0xce, 0x01,
        0xd5, 0xfc, 0xe4, 0x4d, 0x83, 0x23, 0xc0, 0xc2, 0xc9, 0xe6, 0x76, 0x32,
        0x84, 0x42, 0x96, 0xd2
    };
...
{
    ubyte expect_enc[] = {
        0x20, 0x1c, 0x25, 0xed, 0xc5, 0xb0, 0x95, 0x19, 0x8e, 0xa2, 0xff, 0x20,
        0x5c, 0xee, 0xc4, 0xcc, 0xfb, 0xc0, 0x43, 0xe1, 0x58, 0x30, 0x3d, 0xfc,
        0x1d, 0x04, 0xac, 0xb7, 0x4f, 0xf9, 0x2f, 0x8d
    };
    ubyte expect_dec[] = {
        0x06, 0x69, 0xba, 0xb2, 0xaa, 0x45, 0x07, 0x96, 0x92, 0x52, 0x10, 0xd8,
        0xab, 0x5c, 0xb6, 0x33, 0x06, 0x69, 0xba, 0xb2, 0xaa, 0x45, 0x07, 0x96,
        0x92, 0x52, 0x10, 0xd8, 0xab, 0x5c, 0xb6, 0x33
    };
...
FIPS_aesCfbKat(hwAccelDescr hwAccelCtx)
{
    ...
    ubyte expect_dec[] = {
        0x20, 0x1c, 0x25, 0xed, 0xc5, 0xb0, 0x95, 0x19, 0x8e, 0xa2, 0xff, 0x20,
        0x5c, 0xee, 0xc4, 0xcc, 0x20, 0x1c, 0x25, 0xed, 0xc5, 0xb0, 0x95, 0x19,
        0x8e, 0xa2, 0xff, 0x20, 0x5c, 0xee, 0xc4, 0xcc
    };
...
{
    ubyte expect_enc[] = {
        0x85, 0x58, 0xfc, 0x3f, 0x15, 0x50, 0x78, 0x62, 0x7b, 0xaf, 0x8a, 0x8e,
        0x23, 0x09, 0x89, 0xd3, 0xbd, 0x70, 0x00, 0xca, 0x6b, 0xd4, 0xf8, 0x1e,
        0x9e, 0x66, 0x5c, 0xac, 0xca, 0x3e, 0x93, 0x34
    };
    ubyte expect_dec[] = {
        0x85, 0x58, 0xfc, 0x3f, 0x15, 0x50, 0x78, 0x62, 0x7b, 0xaf, 0x8a, 0x8e,
        0x23, 0x09, 0x89, 0xd3, 0xbd, 0x70, 0x00, 0xca, 0x6b, 0xd4, 0xf8, 0x1e,
...
{
    ubyte expect_enc[] = {
        0x01, 0xdc, 0x2e, 0xc4, 0x2f, 0xed, 0xe9, 0xc6, 0x70, 0xb0, 0x67, 0xd5,
        0x50, 0xf2, 0x86, 0x02, 0xbc, 0x0e, 0x6b, 0xea, 0xd2, 0x04, 0xf4, 0xb1,
        0x59, 0xd9, 0x58, 0xf1, 0x8b, 0x32, 0x7b, 0x6c
    };
    ubyte expect_dec[] = {
        0x40, 0xc3, 0x99, 0x88, 0x67, 0xf7, 0x65, 0x2f, 0x40, 0xc3, 0x99, 0x88,
        0x67, 0xf7, 0x65, 0x2f, 0x40, 0xc3, 0x99, 0x88, 0x67, 0xf7, 0x65, 0x2f,
        0x40, 0xc3, 0x99, 0x88, 0x67, 0xf7, 0x65, 0x2f
    };
...
FIPS_aesGcmKat(hwAccelDescr hwAccelCtx)
{
    ubyte expectedCipherText[16] = {
        0xf0, 0x4d, 0xa0, 0x95, 0x54, 0x4d, 0x06, 0x03, 0x53, 0x58, 0x88, 0xa9,
        0xa3, 0x91, 0x47, 0x9c
    };
    ubyte expectedTag[AES_BLOCK_SIZE] = {
        0x64, 0x59, 0x6a, 0xd7, 0x32, 0x1b, 0x73, 0x3b, 0xc2, 0xdc, 0x8c, 0x8c,
        0x26, 0x22, 0xef, 0x2b
    };
...
#if defined(__ENABLE_DIGICERT_GCM_4K__)
    DIGI_MEMCPY(key, kat_key_input, 16);
    ...
    if (FIPS_FORCE_FAIL_AES_GCM_4K_ENC_TEST)
    {
        *data ^= 0x01;
    }
    ...
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - AES-GCM-4K Encode - Failed");
        status = ERR_FIPS_SYM_KAT_FAILED;
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_AES_GCM_4K_DEC_TEST)
    {
        *data ^= 0x01;
    }
    ...
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - AES-GCM-4K Decode - Failed");
        status = ERR_FIPS_SYM_KAT_FAILED;
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_AES_GCM_64K_ENC_TEST)
    {
        *data ^= 0x01;
    }
    ...
    if ((OK > DIGI_CTIME_MATCH(data, expectedCipherText, 16, &result)) || (0 != result))
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - AES-GCM-64K Encode - Failed");
        status = ERR_FIPS_SYM_KAT_FAILED;
        goto exit;
    }
    ...
    if (FIPS_FORCE_FAIL_AES_GCM_64K_DEC_TEST)
    {
        *data ^= 0x01;
    }
    ...
    if ((OK > DIGI_CTIME_MATCH(data, plainText, 16, &result)) || (0 != result))
    {
        DEBUG_PRINTNL(DEBUG_CRYPTO, (sbyte *) "FIPS_KAT - AES-GCM-64K Decode - Failed");
        status = ERR_FIPS_SYM_KAT_FAILED;
        goto exit;
    }
...
{
    ubyte expect[] = {
        0x01, 0xe9, 0xce, 0xee, 0x53, 0x30, 0xf6, 0x1f, 0x45, 0x90, 0x3e, 0x24,
        0xf0, 0xa7, 0x96, 0x47, 0xa6, 0xec, 0x2e, 0x04, 0x0f, 0x9c, 0xd4, 0x6d,
        0x1b, 0x93, 0x7a, 0x00, 0x9b, 0x04, 0x1a, 0x4f
    };
...
{
    ubyte expect[] = {
        0xbf, 0xc3, 0x43, 0xac, 0xbc, 0xd0, 0xa0, 0xf6, 0xf7, 0xde, 0x79, 0xde,
        0xef, 0x50, 0xda, 0xe5, 0x99, 0x0e, 0xe5, 0xa5, 0x6a, 0x49, 0x41, 0xe6,
        0x84, 0x2b, 0x1b, 0x11, 0x1e, 0x44, 0x05, 0x20
    };
...
{
    ubyte expect[] = {
        0xd7, 0xec, 0xf6, 0x11, 0x9f, 0x97, 0x56, 0x97, 0x36, 0xc6, 0xb5, 0x3b,
        0x51, 0xef, 0xc5, 0x65, 0x2d, 0x7d, 0x41, 0x66, 0x14, 0x98, 0x92, 0xbd,
        0xec, 0x49, 0x6f, 0x68, 0x2d, 0x3b, 0xbf, 0x60
    };
...
{
    ubyte expect[] = {
        0xa6, 0xed, 0xfb, 0x08, 0x39, 0x58, 0x53, 0x99, 0xfa, 0xab, 0x8f, 0x63,
        0xca, 0xa9, 0x2f, 0x15, 0x2c, 0x2f, 0xa1, 0x4c, 0x78, 0xe3, 0x6c, 0xa6,
        0x4b, 0x02, 0x16, 0x05, 0x79, 0x01, 0x7f, 0x9e
    };
...
{
    ubyte expect[] = {
        0xd7, 0xb3, 0x77, 0xa4, 0xa5, 0xe5, 0xdb, 0x4f, 0x82, 0x50, 0xc2, 0xfc,
        0x72, 0x77, 0xd9, 0xc7, 0x98, 0x71, 0x17, 0xf9, 0x61, 0x6f, 0xa1, 0xdd,
        0x10, 0x43, 0x43, 0x3e, 0xd4, 0x0f, 0xdb, 0x47
    };
...
{
    ubyte expect[] = {
        0xeb, 0x72, 0x4a, 0xaf, 0xb8, 0xf1, 0xa1, 0x49, 0xee, 0xb0, 0x31, 0x63,
        0x6d, 0x6c, 0xac, 0xca, 0x6a, 0x35, 0x5a, 0xd9, 0x0c, 0x21, 0x7c, 0xe5,
        0x6e, 0x63, 0x2c, 0x75, 0x1e, 0x03, 0x81, 0xa4
    };
...
{
    ubyte expect[] = {
        0x0b, 0x8d, 0x19, 0xbc, 0xf6, 0x23, 0xc9, 0x81, 0x9c, 0xbb, 0xe2, 0xa2,
        0x02, 0x8f, 0x90, 0x62, 0x66, 0x58, 0x2e, 0xa5, 0xe7, 0x04, 0xd0, 0xf1,
        0xa5, 0x94, 0x39, 0x1e, 0xb7, 0x22, 0x48, 0xb6
    };
...
{
    ubyte expect[] = {
        0xc3, 0xf9, 0x97, 0x73, 0xb9, 0x1d, 0x8d, 0xc2, 0xff, 0xd2, 0x1e, 0x04,
        0x92, 0xf6, 0xbc, 0xd7, 0x2f, 0x01, 0x19, 0xfd, 0x16, 0xae, 0xcc, 0x6a,
        0x7b, 0xc9, 0xba, 0x5f, 0xa1, 0x79, 0x33, 0x20
    };
...
{
    ubyte expect[] = {
        0x5f, 0x2b, 0x2a, 0x8e, 0xde, 0xfb, 0x8f, 0x6a, 0xf9, 0x52, 0xd1, 0xbf,
        0xeb, 0x27, 0xbb, 0x0e, 0x7b, 0x04, 0x1c, 0xc7, 0x34, 0x89, 0x37, 0x69,
        0xd4, 0xf6, 0x25, 0x38, 0x02, 0xe4, 0x12, 0x4d
    };
