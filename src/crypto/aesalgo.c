...
#include "../crypto/hw_accel.h"
    ...
    (!defined(__ENABLE_DIGICERT_SMALL_AES__)) && \
    (!defined(__DISABLE_AES_SW_CIPHERS__))
...
#include "../crypto/hw_offload/cvmx.h"
#include "../crypto/hw_offload/cvmx-key.h"
...
#define toUbyte4( in) (((ubyte4)(in)[0] << 24) ^ ((ubyte4)(in)[1] << 16) ^ ((ubyte4)(in)[2] << 8) ^ ((ubyte4)(in)[3]))
#define fromUbyte4( out, in)        \
  {                                 \
    (out)[0] = (ubyte)((in) >> 24); \
    (out)[1] = (ubyte)((in) >> 16); \
    (out)[2] = (ubyte)((in) >> 8);  \
...
static const ubyte4 gRoundConst[] =
{
  0x01000000LU, 0x02000000LU, 0x04000000LU, 0x08000000LU,
  0x10000000LU, 0x20000000LU, 0x40000000LU, 0x80000000LU,
  0x1B000000LU, 0x36000000LU
};

/*----------------------------------------------------------------------------*/

/** 
 * Compute the encryption round keys from the cipher key.
 * Will return the number of rounds, 10, 12, or 14, based on the key size.
 */
MOC_EXTERN sbyte4 aesSwKeySetupEnc (
  ubyte4 pRoundKey[/*4*(numRounds + 1)*/],
  ...
  )
{
  ...
  {
    printf("aesKeySetupEnc: DIGI_MALLOC() failed\n");
    return 0;
  }

  DIGI_MEMSET((ubyte *)ctx, 0x00, sizeof(cavmCipherContext));
  DIGI_MEMCPY(&(ctx->key[0]), pCipherKey, keyBits);
...
#if defined(__ENABLE_DIGICERT_AES_NI_RUNTIME_CHECK__)
  if (check_for_aes_instructions())
  {
    /* We have AESNI instructions */
    return aesNiKeySetupEnc(pRoundKey, (ubyte *)pCipherKey, keyBits);
  }
  else
#endif
  {
    ...
    pRoundKey[0] = toUbyte4(pCipherKey);
    pRoundKey[1] = toUbyte4(pCipherKey + 4);
    pRoundKey[2] = toUbyte4(pCipherKey + 8);
    pRoundKey[3] = toUbyte4(pCipherKey + 12);
    ...
    if (128 == keyBits)
    {
      ...
      {
        temp = pRoundKey[3];
        pRoundKey[4] = pRoundKey[0] ^ (gTe4[(temp >> 16) & 0xff] & 0xff000000L) ^ (gTe4[(temp >> 8) & 0xff] & 0x00ff0000L) ^
                       (gTe4[(temp)&0xff] & 0x0000ff00L) ^ (gTe4[(temp >> 24)] & 0x000000ffL) ^ gRoundConst[i];
    ...
    pRoundKey[4] = toUbyte4(pCipherKey + 16);
    pRoundKey[5] = toUbyte4(pCipherKey + 20);
    ...
    if (192 == keyBits)
    {
      ...
      {
        temp = pRoundKey[5];
        pRoundKey[6] = pRoundKey[0] ^ (gTe4[(temp >> 16) & 0xff] & 0xff000000L) ^ (gTe4[(temp >> 8) & 0xff] & 0x00ff0000L) ^
                       (gTe4[(temp)&0xff] & 0x0000ff00L) ^ (gTe4[(temp >> 24)] & 0x000000ffL) ^ gRoundConst[i];
    ...
    pRoundKey[6] = toUbyte4(pCipherKey + 24);
    pRoundKey[7] = toUbyte4(pCipherKey + 28);
    ...
    if (256 == keyBits)
    {
      ...
      {
        temp = pRoundKey[7];
        pRoundKey[8] = pRoundKey[0] ^ (gTe4[(temp >> 16) & 0xff] & 0xff000000L) ^ (gTe4[(temp >> 8) & 0xff] & 0x00ff0000L) ^
                       (gTe4[(temp)&0xff] & 0x0000ff00L) ^ (gTe4[(temp >> 24)] & 0x000000ffL) ^ gRoundConst[i];
        ...
        temp = pRoundKey[11];
        pRoundKey[12] = pRoundKey[4] ^ (gTe4[(temp >> 24)] & 0xff000000L) ^ (gTe4[(temp >> 16) & 0xff] & 0x00ff0000L) ^
                        (gTe4[(temp >> 8) & 0xff] & 0x0000ff00L) ^ (gTe4[(temp)&0xff] & 0x000000ffL);
...
/** 
 * Compute the decryption round keys from the cipher key.
 * Will return the number of rounds, 10, 12, or 14, based on the key size.
 */
  ...
  )
{
  ...
  if (OK != DIGI_MALLOC((void **)&ctx, sizeof(cavmCipherContext)))
  {
    printf("aesKeySetupDec: DIGI_MALLOC() failed");
    return 0;
  }
  ...
  {
    /* We have AESNI instructions */
    return aesNiKeySetupDec(pRoundKey, (ubyte *)pCipherKey, keyBits);
  }
  else
#endif
  {
    sbyte4 numRounds = 0, i = 0, j = 0;
    ubyte4 temp = 0;

    /* First expand as if it's an encryption key */
    numRounds = aesSwKeySetupEnc(pRoundKey, pCipherKey, keyBits);

    /* invert the order */
    for (i = 0, j = 4 * numRounds; i < j; i += 4, j -= 4)
    {
      temp = pRoundKey[i];
      pRoundKey[i] = pRoundKey[j];
      pRoundKey[j] = temp;
      temp = pRoundKey[i + 1];
      pRoundKey[i + 1] = pRoundKey[j + 1];
      pRoundKey[j + 1] = temp;
      temp = pRoundKey[i + 2];
      pRoundKey[i + 2] = pRoundKey[j + 2];
      pRoundKey[j + 2] = temp;
      temp = pRoundKey[i + 3];
      pRoundKey[i + 3] = pRoundKey[j + 3];
      pRoundKey[j + 3] = temp;
    }

    /* the inverse MixColumn transform gets applied to all round keys except for the first and last */
    for (i = 1; i < numRounds; ++i)
    {
      pRoundKey += 4;
      pRoundKey[0] = gTd0[gTe4[(pRoundKey[0] >> 24)] & 0xff] ^ gTd1[gTe4[(pRoundKey[0] >> 16) & 0xff] & 0xff] ^
                     gTd2[gTe4[(pRoundKey[0] >> 8) & 0xff] & 0xff] ^ gTd3[gTe4[(pRoundKey[0]) & 0xff] & 0xff];
      pRoundKey[1] = gTd0[gTe4[(pRoundKey[1] >> 24)] & 0xff] ^ gTd1[gTe4[(pRoundKey[1] >> 16) & 0xff] & 0xff] ^
                     gTd2[gTe4[(pRoundKey[1] >> 8) & 0xff] & 0xff] ^ gTd3[gTe4[(pRoundKey[1]) & 0xff] & 0xff];
      pRoundKey[2] = gTd0[gTe4[(pRoundKey[2] >> 24)] & 0xff] ^ gTd1[gTe4[(pRoundKey[2] >> 16) & 0xff] & 0xff] ^
                     gTd2[gTe4[(pRoundKey[2] >> 8) & 0xff] & 0xff] ^ gTd3[gTe4[(pRoundKey[2]) & 0xff] & 0xff];
      pRoundKey[3] = gTd0[gTe4[(pRoundKey[3] >> 24)] & 0xff] ^ gTd1[gTe4[(pRoundKey[3] >> 16) & 0xff] & 0xff] ^
                     gTd2[gTe4[(pRoundKey[3] >> 8) & 0xff] & 0xff] ^ gTd3[gTe4[(pRoundKey[3]) & 0xff] & 0xff];
    }
  ...
  )
{
  ...
  {
    /* We have AESNI instructions */
    aesNiEncrypt(pRoundKey, numRounds, (ubyte *)pPlain, pCipher, 1);
    return;
  }
  else
#endif
  {
    ubyte4 s0 = 0, s1 = 0, s2 = 0, s3 = 0, t0 = 0, t1 = 0, t2 = 0, t3 = 0;
    sbyte4 r = 0;

    /* xor (ie field addition) initial round key */
    s0 = toUbyte4(pPlain) ^ pRoundKey[0];
    s1 = toUbyte4(pPlain + 4) ^ pRoundKey[1];
    s2 = toUbyte4(pPlain + 8) ^ pRoundKey[2];
    s3 = toUbyte4(pPlain + 12) ^ pRoundKey[3];
   
    /* we'll do 2 rounds at a time for all but the last round,
       start with r = numRounds/2, ie 5, 6, or 7. */ 
    r = numRounds >> 1;

    while (TRUE)
    {
      t0 = gTe0[(s0 >> 24)] ^ gTe1[(s1 >> 16) & 0xff] ^ gTe2[(s2 >> 8) & 0xff] ^ gTe3[(s3)&0xff] ^ pRoundKey[4];
      t1 = gTe0[(s1 >> 24)] ^ gTe1[(s2 >> 16) & 0xff] ^ gTe2[(s3 >> 8) & 0xff] ^ gTe3[(s0)&0xff] ^ pRoundKey[5];
      t2 = gTe0[(s2 >> 24)] ^ gTe1[(s3 >> 16) & 0xff] ^ gTe2[(s0 >> 8) & 0xff] ^ gTe3[(s1)&0xff] ^ pRoundKey[6];
      t3 = gTe0[(s3 >> 24)] ^ gTe1[(s0 >> 16) & 0xff] ^ gTe2[(s1 >> 8) & 0xff] ^ gTe3[(s2)&0xff] ^ pRoundKey[7];

      pRoundKey += 8;

      if ( !(--r) )  /* break and do the last round separate */
      {
        break;
      }

      s0 = gTe0[(t0 >> 24)] ^ gTe1[(t1 >> 16) & 0xff] ^ gTe2[(t2 >> 8) & 0xff] ^ gTe3[(t3)&0xff] ^ pRoundKey[0];
      s1 = gTe0[(t1 >> 24)] ^ gTe1[(t2 >> 16) & 0xff] ^ gTe2[(t3 >> 8) & 0xff] ^ gTe3[(t0)&0xff] ^ pRoundKey[1];
      s2 = gTe0[(t2 >> 24)] ^ gTe1[(t3 >> 16) & 0xff] ^ gTe2[(t0 >> 8) & 0xff] ^ gTe3[(t1)&0xff] ^ pRoundKey[2];
      s3 = gTe0[(t3 >> 24)] ^ gTe1[(t0 >> 16) & 0xff] ^ gTe2[(t1 >> 8) & 0xff] ^ gTe3[(t2)&0xff] ^ pRoundKey[3];
    }

    s0 = (gTe4[(t0 >> 24)] & 0xff000000L) ^ (gTe4[(t1 >> 16) & 0xff] & 0x00ff0000L) ^
         (gTe4[(t2 >> 8) & 0xff] & 0x0000ff00L) ^ (gTe4[(t3)&0xff] & 0x000000ffL) ^ pRoundKey[0];
    s1 = (gTe4[(t1 >> 24)] & 0xff000000L) ^ (gTe4[(t2 >> 16) & 0xff] & 0x00ff0000L) ^
         (gTe4[(t3 >> 8) & 0xff] & 0x0000ff00L) ^ (gTe4[(t0)&0xff] & 0x000000ffL) ^ pRoundKey[1];
    s2 = (gTe4[(t2 >> 24)] & 0xff000000L) ^ (gTe4[(t3 >> 16) & 0xff] & 0x00ff0000L) ^
         (gTe4[(t0 >> 8) & 0xff] & 0x0000ff00L) ^ (gTe4[(t1)&0xff] & 0x000000ffL) ^ pRoundKey[2];
    s3 = (gTe4[(t3 >> 24)] & 0xff000000L) ^ (gTe4[(t0 >> 16) & 0xff] & 0x00ff0000L) ^
         (gTe4[(t1 >> 8) & 0xff] & 0x0000ff00L) ^ (gTe4[(t2)&0xff] & 0x000000ffL) ^ pRoundKey[3];

    fromUbyte4(pCipher, s0);
    fromUbyte4(pCipher + 4, s1);
    fromUbyte4(pCipher + 8, s2);
    fromUbyte4(pCipher + 12, s3);
  }
#endif /*    __ENABLE_LLA_CAVIUM_HWXL__  */
  ...
  )
{
  ...
  {
    /* We have AESNI instructions */
    aesNiDecrypt(pRoundKey, numRounds, (ubyte *)pCipher, pPlain, 1);
    return;
  }
  else
#endif
  {
    ...
    /* xor (ie field addition) initial round key */
    s0 = toUbyte4(pCipher) ^ pRoundKey[0];
    s1 = toUbyte4(pCipher + 4) ^ pRoundKey[1];
    s2 = toUbyte4(pCipher + 8) ^ pRoundKey[2];
    s3 = toUbyte4(pCipher + 12) ^ pRoundKey[3];
    ...
    while (TRUE)
    {
      t0 = gTd0[(s0 >> 24)] ^ gTd1[(s3 >> 16) & 0xff] ^ gTd2[(s2 >> 8) & 0xff] ^ gTd3[(s1)&0xff] ^ pRoundKey[4];
      t1 = gTd0[(s1 >> 24)] ^ gTd1[(s0 >> 16) & 0xff] ^ gTd2[(s3 >> 8) & 0xff] ^ gTd3[(s2)&0xff] ^ pRoundKey[5];
      t2 = gTd0[(s2 >> 24)] ^ gTd1[(s1 >> 16) & 0xff] ^ gTd2[(s0 >> 8) & 0xff] ^ gTd3[(s3)&0xff] ^ pRoundKey[6];
      t3 = gTd0[(s3 >> 24)] ^ gTd1[(s2 >> 16) & 0xff] ^ gTd2[(s1 >> 8) & 0xff] ^ gTd3[(s0)&0xff] ^ pRoundKey[7];
      ...
      s0 = gTd0[(t0 >> 24)] ^ gTd1[(t3 >> 16) & 0xff] ^ gTd2[(t2 >> 8) & 0xff] ^ gTd3[(t1)&0xff] ^ pRoundKey[0];
      s1 = gTd0[(t1 >> 24)] ^ gTd1[(t0 >> 16) & 0xff] ^ gTd2[(t3 >> 8) & 0xff] ^ gTd3[(t2)&0xff] ^ pRoundKey[1];
      s2 = gTd0[(t2 >> 24)] ^ gTd1[(t1 >> 16) & 0xff] ^ gTd2[(t0 >> 8) & 0xff] ^ gTd3[(t3)&0xff] ^ pRoundKey[2];
      s3 = gTd0[(t3 >> 24)] ^ gTd1[(t2 >> 16) & 0xff] ^ gTd2[(t1 >> 8) & 0xff] ^ gTd3[(t0)&0xff] ^ pRoundKey[3];
    }

    s0 = (Td4[(t0 >> 24)] & 0xff000000L) ^ (Td4[(t3 >> 16) & 0xff] & 0x00ff0000L) ^
         (Td4[(t2 >> 8) & 0xff] & 0x0000ff00L) ^ (Td4[(t1)&0xff] & 0x000000ffL) ^ pRoundKey[0];
    s1 = (Td4[(t1 >> 24)] & 0xff000000L) ^ (Td4[(t0 >> 16) & 0xff] & 0x00ff0000L) ^
         (Td4[(t3 >> 8) & 0xff] & 0x0000ff00L) ^ (Td4[(t2)&0xff] & 0x000000ffL) ^ pRoundKey[1];
    s2 = (Td4[(t2 >> 24)] & 0xff000000L) ^ (Td4[(t1 >> 16) & 0xff] & 0x00ff0000L) ^
         (Td4[(t0 >> 8) & 0xff] & 0x0000ff00L) ^ (Td4[(t3)&0xff] & 0x000000ffL) ^ pRoundKey[2];
    s3 = (Td4[(t3 >> 24)] & 0xff000000L) ^ (Td4[(t2 >> 16) & 0xff] & 0x00ff0000L) ^
         (Td4[(t1 >> 8) & 0xff] & 0x0000ff00L) ^ (Td4[(t0)&0xff] & 0x000000ffL) ^ pRoundKey[3];

    fromUbyte4(pPlain, s0);
    fromUbyte4(pPlain + 4, s1);
    fromUbyte4(pPlain + 8, s2);
    fromUbyte4(pPlain + 12, s3);
  }
#endif /* __ENABLE_LLA_CAVIUM_HWXL__ */
...
/* If the AES-Ni build flags are not defined then these functions act as
 * as wrappers to the software implementation of AES. If they are defined then
 * aesalgo_intel_ni.c will implement new wrappers of the same name to the
 * Ni implementation. Wrappers are necessary so the software operators can
 * call directly to the software implementation even if AES-Ni is enabled, it
 * is how simultaneous implementations are supported. */

/*----------------------------------------------------------------------------*/

/* Wrapper for encryption key setup */
...
{
  return aesSwKeySetupEnc(pRoundKey, pCipherKey, keyBits);
}

/*----------------------------------------------------------------------------*/

/* Wrapper for decryption key setup */
...
{
  return aesSwKeySetupDec(pRoundKey, pCipherKey, keyBits);
}
...
{
  aesSwEncrypt(pRoundKey, numRounds, pPlain, pCipher);
}
...
{
  aesSwDecrypt(pRoundKey, numRounds, pCipher, pPlain);
}

#endif /* ifndef __ENABLE_DIGICERT_AES_NI__ etc. */
#endif /* ifndef __DISABLE_AES_CIPHERS__ etc. */
