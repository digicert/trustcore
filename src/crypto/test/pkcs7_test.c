...
static int SignAndVerifyData(int hint, const char* selfCertFile, const char* keyblobFile)
{
    MSTATUS status = OK;
    ...
    UNITTEST_STATUS_GOTO(hint, PKCS7_VerifySignedData(MOC_RSA(hwAccelCtx) pTemp, cs, NULL,
...
int pkcs7_test_SignAndVerifyData()
{
    int retVal = 0;
...
static int signAndVerify(ubyte* contentType)
{
    ...
    PKCS7_SignData(MOC_ASYM(hwAccelCtx) 0, pContentInfo, pSignedData,
    ...
    /* verify the signature */
    if (OK > (status = PKCS7_VerifySignedData(MOC_RSA(hwAccelCtx) pTemp, cs, NULL,
                                              NULL, myValCertFun, NULL, 0,
                                              &numKnownSigners)))
    {
        goto exit;
...
int pkcs7_test_SignAndVerifyData2()
{
    int retVal = 0;
...
/* sign some data and then verify */
int pkcs7_test_SignAndVerifyDetachedSignature()
{
    MSTATUS status = OK;
    ...
    /* verify the signature */
    if (OK > (status = PKCS7_VerifySignedData(MOC_RSA(hwAccelCtx) pTemp, cs,
 ...
 */
#define num 2
int pkcs7_test_SignDataDegenerate()
{
    MSTATUS status = OK;
...
static int verifySignedData(const char *signedFile)
{
    ...
    if (OK > (status = PKCS7_VerifySignedDataV3(MOC_RSA(hwAccelCtx) pSignedData, cs,
...
int pkcs7_test_VerifySignedData()
{
    int retVal = 0;
...
*/
int pkcs7_test_VerifyDetachedSignature()
{
    ...
    if (OK > (status = PKCS7_VerifySignedData(MOC_RSA(hwAccelCtx) pSignedData, cs,
                                              NULL, NULL, myValCertFun,
 ...
 */
int pkcs7_test_VerifyDegenerateSignedData()
{
    MSTATUS status = OK;
...
int pkcs7_test_DecryptEnvelopedData()
{
    int retVal = 0;
                           ...
                           const char* certFileName, const char* keyFileName)
{
    ...
    retVal += UNITTEST_STATUS(hint, PKCS7_EnvelopData(MOC_HW(hwAccelCtx) pContentInfo, pEnvelopedData,
                                                      &pCertRoot, &cs1, 1,
                                                      aes128CBC_OID,
                                                      RANDOM_rngFun,
                                                      g_pRandomContext,
...
int pkcs7_test_EnvelopeAndDecryptData()
{
    int retVal = 0;
    hwAccelDescr hwAccelCtx;
                              ...
                              AsymmetricKey* pAsymKey)
{
    ...
    status = CRYPTO_deserializeAsymKey(MOC_ASYM(0) pKeyBin, keyLen, NULL, pAsymKey);
...
static int pkcs7_test_decrypt(char *pEnv)
{
    ...
    retVal += UNITTEST_STATUS(__MOC_LINE__, PKCS7_DecryptEnvelopedData(MOC_HW(hwAccelCtx)
                                                                       pEnvelopedData,
                                                                       cs,
                                                                       NULL,
                                                                       getFixedKeyFun,
...
int pkcs7_test_VerifyOpenSSLSign()
{
    int retVal = 0;
