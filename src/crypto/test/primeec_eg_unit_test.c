/*
 * primeec_eg_unit_test.c
 *
 * unit test for primeec_eg.c
 *
...
#include "primeec_eg_data_192_inc.h"
#endif

#ifndef __DISABLE_DIGICERT_ECC_P224__
#include "primeec_eg_data_224_inc.h"
#endif

#ifndef __DISABLE_DIGICERT_ECC_P256__
#include "primeec_eg_data_256_inc.h"
#endif

#ifndef __DISABLE_DIGICERT_ECC_P384__
#include "primeec_eg_data_384_inc.h"
#endif

#ifndef __DISABLE_DIGICERT_ECC_P521__
#include "primeec_eg_data_521_inc.h"
#endif
...
static int testOneShotEncrypt(ECCKey *pPublicKey, ubyte *pPlainText, ubyte4 plainLen, ubyte *pExpectedCipher, ubyte4 expectedCipherLen)
{
    ...
    status = ECEG_encrypt(pPublicKey, &rngCallback, NULL, pPlainText, plainLen, &pResult, &resultLen, NULL);
...
static int testEvpEncrypt(ECCKey *pPublicKey, ubyte *pPlainText, ubyte *pExpectedCipher, ubyte4 *plainIndices, ubyte4* cipherIndices)
{
    ...
    status = ECEG_init(&ctx, pPublicKey, MOCANA_ECEG_ENCRYPT, &rngCallback, NULL, NULL);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if(OK != status)
        goto exit;
    ...
    for (i = 0; i < ECEG_TEST_MAX_UPDATE_CALLS; ++i)
    {
        ...
        {
            status = ECEG_update(&ctx, pPlainText + plainIndices[i], plainIndices[i+1] - plainIndices[i], pResultPtr, sizeof(pResultBuffer) - totalBytesWritten, &bytesWritten, NULL);
            UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
            if(OK != status)
                goto exit;
...
static int testOneShotDecrypt(ECCKey *pPrivateKey, ubyte *pCipherText, ubyte4 cipherLen, ubyte *pExpectedPlain)
{
    ...
    status = ECEG_decrypt(pPrivateKey, pCipherText, cipherLen, &pResult, &resultLen, NULL);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if(OK != status)
        goto exit;
...
static int testEvpDecrypt(ECCKey *pPrivateKey, ubyte *pCipherText, ubyte *pExpectedPlain, ubyte4 *cipherIndices, ubyte4* plainIndices)
{
    ...
    status = ECEG_init(&ctx, pPrivateKey, MOCANA_ECEG_DECRYPT, NULL, NULL, NULL);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if(OK != status)
        goto exit;
    ...
    for (i = 0; i < ECEG_TEST_MAX_UPDATE_CALLS; ++i)
    {
        ...
        {
            status = ECEG_update(&ctx, pCipherText + cipherIndices[i], cipherIndices[i+1] - cipherIndices[i], pResultPtr, sizeof(pResultBuffer) - totalBytesWritten, &bytesWritten, NULL);
            UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
            if(OK != status)
                goto exit;
...
static int knownAnswerTest(TestVector *pTestVector, PEllipticCurvePtr pEC, ubyte4 pIndices[][4][4], ubyte4 numEVPtests)
{
    MSTATUS status = OK;
    int retVal = 0;
    ubyte4 coordLen = (pEC->pPF->numBits+7)/8; /* in bytes */
    int i;
    ...
    /* Set the keys */
    status = EC_newKey(pEC, &pPublicKey);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_setKeyParameters(pPublicKey, pPublicKeyBytes, pubLen, NULL, 0);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_newKey(pEC, &pPrivateKey);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_setKeyParameters(pPrivateKey, pPublicKeyBytes, pubLen, pPrivateKeyBytes, privLen);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
...
exit:
    ...
    if (NULL != pPublicKey)
    {
        status = EC_deleteKey(&pPublicKey);
        UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    }
    if (NULL != pPrivateKey)
    {
        status = EC_deleteKey(&pPrivateKey);
        UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    }
...
static int knownAnswerTestPKCS(PEllipticCurvePtr pEC, TestVectorPKCS *pTestVector)
{
    MSTATUS status = OK;
    int retVal = 0;
    sbyte4 compare;
    ...
    paddingLen = nonceLen - (pEC->pPF->n)*8; /* last (pEC->pPF->n) * sizeof(pf_unit) is the ElGamal nonce */
#else
    paddingLen = nonceLen - (pEC->pPF->n)*4;
#endif
    ...
    /* Set the nonce */
    status = copyRNGdata(pNonce + paddingLen, nonceLen - paddingLen, pEC->pPF->n);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    ...
    /* Set the keys */
    status = EC_newKey(pEC, &pPub);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_setKeyParameters(pPub, pPublicKey, pubLen, NULL, 0);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_newKey(pEC, &pPriv);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    
    status = EC_setKeyParameters(pPriv, pPublicKey, pubLen, pPrivateKey, privLen);
    UNITTEST_VECTOR_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    ...
    status = ECEG_encryptPKCSv1p5(pPub, rngCallback, NULL, pPlainText, plainLen, pGenCipherText, NULL);
    UNITTEST_VECTOR_STATUS(gCurrentVector, status);
    if(OK != status)
        goto exit;
    ...
    status = ECEG_decryptPKCSv1p5(pPriv, pCipherText, cipherLen, pGenPlainText, NULL);
    UNITTEST_VECTOR_STATUS(gCurrentVector, status);
    if(OK != status)
        goto exit;
...
exit:
    ...
    if (NULL != pPub)
    {
        status = EC_deleteKey(&pPub);
        UNITTEST_VECTOR_STATUS(gCurrentVector, status);
    }
    if (NULL != pPriv)
    {
        status = EC_deleteKey(&pPriv);
        UNITTEST_VECTOR_STATUS(gCurrentVector, status);
    }
...
static int testErrorCases(PEllipticCurvePtr pEC)
{
    MSTATUS status;
    int retVal = 0;
    int plainBlockSize = (pEC->pPF->numBits+7)/8 - MOCANA_ECEG_CTR_LEN;
    int cipherBlockSize = 4*((pEC->pPF->numBits+7)/8);
    ...
    /* Null params */
    status = ECEG_encrypt(NULL, &rngCallback, NULL, pInput, plainBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encrypt(pKey, NULL, NULL, pInput, plainBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encrypt(pKey, &rngCallback, NULL, NULL, plainBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encrypt(pKey, &rngCallback, NULL, pInput, plainBlockSize, NULL, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encrypt(pKey, &rngCallback, NULL, pInput, plainBlockSize, &pResult, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);

    /* bad key */
    pKey->privateKey = TRUE;
    status = ECEG_encrypt(pKey, &rngCallback, NULL, pInput, plainBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    
    status = ECEG_encrypt(&unallocKey, &rngCallback, NULL, pInput, plainBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);
    
    /* bad inputLen */
    pKey->privateKey = FALSE;
    status = ECEG_encrypt(pKey, &rngCallback, NULL, pInput, 0, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_INPUT_LEN);
    
    status = ECEG_encrypt(pKey, &rngCallback, NULL, pInput, plainBlockSize - 1, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_INPUT_LEN);
    ...
    /* Null params */
    pKey->privateKey = TRUE;
    status = ECEG_decrypt(NULL, pInput, cipherBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_decrypt(pKey, NULL, cipherBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_decrypt(pKey, pInput, cipherBlockSize, NULL, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_decrypt(pKey, pInput, cipherBlockSize, &pResult, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    /* bad key */
    pKey->privateKey = FALSE;
    status = ECEG_decrypt(pKey, pInput, cipherBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    
    unallocKey.privateKey = TRUE;
    status = ECEG_decrypt(&unallocKey, pInput, cipherBlockSize, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);
    
    /* bad inputLen (no curve has 0, 95, or 265 ciphertext blocksize) */
    pKey->privateKey = TRUE;
    status = ECEG_decrypt(pKey, pInput, 0, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_INPUT_LEN);
    
    status = ECEG_decrypt(pKey, pInput, cipherBlockSize - 1, &pResult, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_INPUT_LEN);
    ...
    /* Null params */
    status = ECEG_init(NULL, pKey, MOCANA_ECEG_ENCRYPT, &rngCallback, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_init(&ctx, NULL, MOCANA_ECEG_ENCRYPT, &rngCallback, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_init(&ctx, pKey, MOCANA_ECEG_ENCRYPT, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_init(NULL, pKey, MOCANA_ECEG_DECRYPT, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_init(&ctx, NULL, MOCANA_ECEG_DECRYPT, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    /* bad direction */
    status = ECEG_init(&ctx, pKey, 2, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_INVALID_ARG);
    
    /* bad key */
    pKey->privateKey = FALSE;
    status = ECEG_init(&ctx, pKey, MOCANA_ECEG_DECRYPT, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    
    pKey->privateKey = TRUE;
    status = ECEG_init(&ctx, pKey, MOCANA_ECEG_ENCRYPT, &rngCallback, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    
    unallocKey.privateKey = FALSE;
    status = ECEG_init(&ctx, &unallocKey, MOCANA_ECEG_ENCRYPT, &rngCallback, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);
    
    unallocKey.privateKey = TRUE;
    status = ECEG_init(&ctx, &unallocKey, MOCANA_ECEG_DECRYPT, NULL, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);
    
    /* test update and final Now on uninitialized ctx */
    status = ECEG_update(&ctx, pInput, plainBlockSize, pResultBuffer, cipherBlockSize, &resultLen, NULL);
    ...
    pKey->privateKey = FALSE;
    status = ECEG_init(&ctx, pKey, MOCANA_ECEG_ENCRYPT, &badRNGCallback, NULL, NULL);
    retVal += UNITTEST_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
    ...
    /* Null params */
    status = ECEG_update(NULL, pInput, plainBlockSize, pResultBuffer, cipherBlockSize, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_update(&ctx, NULL, plainBlockSize, pResultBuffer, cipherBlockSize, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_update(&ctx, pInput, plainBlockSize, NULL, cipherBlockSize, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_update(&ctx, pInput, plainBlockSize, pResultBuffer, cipherBlockSize, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    /* result buffer too small */
    status = ECEG_update(&ctx, pInput, plainBlockSize, pResultBuffer, 0, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_BUFFER_TOO_SMALL);
    
    status = ECEG_update(&ctx, pInput, plainBlockSize, pResultBuffer, cipherBlockSize - 1, &resultLen, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_BUFFER_TOO_SMALL);
    
    /* Test update with no data, which is an ok no-op (but not tested above) */
    status = ECEG_update(&ctx, NULL, 0, NULL, 0, &resultLen, NULL);
    retVal += UNITTEST_STATUS(__MOC_LINE__, status);
    
    /* Properly call update with one byte for tests of ECEG_final */
    status = ECEG_update(&ctx, pInput, 1, NULL, 0, &resultLen, NULL);
    retVal += UNITTEST_STATUS(__MOC_LINE__, status);
    if (OK != status)
        goto exit;
...
static int testErrorCasesPKCS(PEllipticCurvePtr pEC)
{
    MSTATUS status;
    int retVal = 0;
    ...
    ubyte4 plainLen = ((pEC->pPF->numBits+7)/8) - 15; /* minus 4 byte counter and 11 pkcs v1.5 padding bytes */
    ubyte4 cipherLen = 4*((pEC->pPF->numBits+7)/8);
    ...
    /* null params */
    status = ECEG_encryptPKCSv1p5(NULL, rngCallback, NULL, pPlain, plainLen, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encryptPKCSv1p5(pKey, NULL, NULL, pPlain, plainLen, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encryptPKCSv1p5(pKey, rngCallback, NULL, NULL, plainLen, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_encryptPKCSv1p5(pKey, rngCallback, NULL, pPlain, plainLen, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    /* invalid key type */
    pKey->privateKey = TRUE;
    status = ECEG_encryptPKCSv1p5(pKey, rngCallback, NULL, pPlain, plainLen, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    pKey->privateKey = FALSE;
    
    /* unalloc key */
    status = ECEG_encryptPKCSv1p5(&unallocKey, rngCallback, NULL, pPlain, plainLen, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);

    /* invalid plaintext len */
    status = ECEG_encryptPKCSv1p5(pKey, rngCallback, NULL, pPlain, 0, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_PLAINTEXT_LEN);
    
    status = ECEG_encryptPKCSv1p5(pKey, rngCallback, NULL, pPlain, plainLen + 1, pCipher, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_PLAINTEXT_LEN);
    
    /* invalid rng */
    status = ECEG_encryptPKCSv1p5(pKey, badRNGCallback, NULL, pPlain, plainLen, pCipher, NULL);
    ...
    /* null params */
    pKey->privateKey = TRUE;
    status = ECEG_decryptPKCSv1p5(NULL, pCipher, cipherLen, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_decryptPKCSv1p5(pKey, NULL, cipherLen, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    status = ECEG_decryptPKCSv1p5(pKey, pCipher, cipherLen, NULL, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_NULL_POINTER);
    
    /* invalid key type */
    pKey->privateKey = FALSE;
    status = ECEG_decryptPKCSv1p5(pKey, pCipher, cipherLen, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_KEY_TYPE);
    pKey->privateKey = TRUE;
    
    /* unalloc key */
    unallocKey.privateKey = TRUE;
    status = ECEG_decryptPKCSv1p5(&unallocKey, pCipher, cipherLen, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_UNALLOCATED_KEY);
    
    /* invalid cipher len */
    status = ECEG_decryptPKCSv1p5(pKey, pCipher, 0, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_CIPHERTEXT_LEN);
    
    status = ECEG_decryptPKCSv1p5(pKey, pCipher, cipherLen - 1, pPlain, NULL);
    retVal += UNITTEST_INT(__MOC_LINE__, status, ERR_ECEG_INVALID_CIPHERTEXT_LEN);
...
static int testErrorCasesEnc(PEllipticCurvePtr pEC, TestVectorPKCS *pTestVector, MSTATUS expectedStatus)
{
    MSTATUS status;
    int retVal = 0;
    ubyte4 paddingLen;
    ...
    {
#ifdef __ENABLE_DIGICERT_64_BIT__
        paddingLen = nonceLen - 16*(pEC->pPF->n)*8; /* last 16*(pEC->pPF->n) * sizeof(pf_unit) is the ElGamal nonce */
#else
        paddingLen = nonceLen - 16*(pEC->pPF->n)*4;
#endif
    }
    else
    {
...
static int testErrorCasesDec(PEllipticCurvePtr pEC, TestVectorPKCS *pTestVector, MSTATUS expectedStatus)
{
    MSTATUS status;
    int retVal = 0;
...
int primeec_eg_unit_test_all()
{
    ...
    for (i = 0; i < COUNTOF(gTestVector_p192); ++i)
    {
        ...
        {
            case 1:
                retVal += knownAnswerTest(gTestVector_p192+i, EC_P192, gpIndicesOneBlock_p192, COUNTOF(gpIndicesOneBlock_p192));
                break;
                
            case 2:
                retVal += knownAnswerTest(gTestVector_p192+i, EC_P192, gpIndicesTwoBlocks_p192, COUNTOF(gpIndicesTwoBlocks_p192));
                break;
                
            case 4:
                retVal += knownAnswerTest(gTestVector_p192+i, EC_P192, gpIndicesFourBlocks_p192, COUNTOF(gpIndicesFourBlocks_p192));
                break;
        }
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
    ...
    {
        retVal += knownAnswerTestPKCS(EC_P192, gTestVector_p192PKCS+i);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
    
    retVal += testErrorCasesPKCS(EC_P192);
    ...
    for (i = 0; i < COUNTOF(gTestVector_p224); ++i)
    {
        ...
        {
            case 1:
                retVal += knownAnswerTest(gTestVector_p224+i, EC_P224, gpIndicesOneBlock_p224, COUNTOF(gpIndicesOneBlock_p224));
                break;
                
            case 2:
                retVal += knownAnswerTest(gTestVector_p224+i, EC_P224, gpIndicesTwoBlocks_p224, COUNTOF(gpIndicesTwoBlocks_p224));
                break;
                
            case 4:
                retVal += knownAnswerTest(gTestVector_p224+i, EC_P224, gpIndicesFourBlocks_p224, COUNTOF(gpIndicesFourBlocks_p224));
                break;
        }
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
    ...
    {
        retVal += knownAnswerTestPKCS(EC_P224, gTestVector_p224PKCS+i);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
    
    retVal += testErrorCasesPKCS(EC_P224);
    ...
    for (i = 0; i < COUNTOF(gTestVector_p256); ++i)
    {
        ...
        {
            case 1:
                retVal += knownAnswerTest(gTestVector_p256+i, EC_P256, gpIndicesOneBlock_p256, COUNTOF(gpIndicesOneBlock_p256));
                break;
                
            case 2:
                retVal += knownAnswerTest(gTestVector_p256+i, EC_P256, gpIndicesTwoBlocks_p256, COUNTOF(gpIndicesTwoBlocks_p256));
                break;
                
            case 4:
                retVal += knownAnswerTest(gTestVector_p256+i, EC_P256, gpIndicesFourBlocks_p256, COUNTOF(gpIndicesFourBlocks_p256));
                break;
        }
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
    ...
    {
        retVal += knownAnswerTestPKCS(EC_P256, gTestVector_p256PKCS+i);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
    
    retVal += testErrorCasesPKCS(EC_P256);
    ...
    for (i = 0; i < COUNTOF(gTestVector_p384); ++i)
    {
        ...
        {
            case 1:
                retVal += knownAnswerTest(gTestVector_p384+i, EC_P384, gpIndicesOneBlock_p384, COUNTOF(gpIndicesOneBlock_p384));
                break;
                
            case 2:
                retVal += knownAnswerTest(gTestVector_p384+i, EC_P384, gpIndicesTwoBlocks_p384, COUNTOF(gpIndicesTwoBlocks_p384));
                break;
                
            case 4:
                retVal += knownAnswerTest(gTestVector_p384+i, EC_P384, gpIndicesFourBlocks_p384, COUNTOF(gpIndicesFourBlocks_p384));
                break;
        }
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
    ...
    {
        retVal += knownAnswerTestPKCS(EC_P384, gTestVector_p384PKCS+i);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
    
    retVal += testErrorCasesPKCS(EC_P384);
    ...
    for (i = 0; i < COUNTOF(gTestVector_p521); ++i)
    {
        ...
        {
            case 1:
                retVal += knownAnswerTest(gTestVector_p521+i, EC_P521, gpIndicesOneBlock_p521, COUNTOF(gpIndicesOneBlock_p521));
                break;
                
            case 2:
                retVal += knownAnswerTest(gTestVector_p521+i, EC_P521, gpIndicesTwoBlocks_p521, COUNTOF(gpIndicesTwoBlocks_p521));
                break;
                
            case 4:
                retVal += knownAnswerTest(gTestVector_p521+i, EC_P521, gpIndicesFourBlocks_p521, COUNTOF(gpIndicesFourBlocks_p521));
                break;
        }
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
    ...
    {
        retVal += knownAnswerTestPKCS(EC_P521, gTestVector_p521PKCS+i);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
    
    retVal += testErrorCasesPKCS(EC_P521);

#ifdef __DEBUG_VECTORS__
    gCurrentVector = 0;
#endif
    retVal += testErrorCasesEnc(EC_P521, &gError_enc_p521[0], -1); /* our error code from the rng callback */

#ifdef __DEBUG_VECTORS__
    gCurrentVector = 1;
#endif
    retVal += testErrorCasesEnc(EC_P521, &gError_enc_p521[1], ERR_EC_INFINITE_RESULT);

#ifdef __DEBUG_VECTORS__
    gCurrentVector = 0;
#endif
    retVal += testErrorCasesDec(EC_P521, &gError_dec_p521[0], ERR_EC_INFINITE_RESULT);
    ...
    {
        retVal += testErrorCasesDec(EC_P521, gError_dec_p521 + i, ERR_ECEG_INVALID_PKCS1_V1P5);
#ifdef __DEBUG_VECTORS__
        gCurrentVector++;
#endif
    }
