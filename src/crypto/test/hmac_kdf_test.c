...
static int HmacKdfExtractTestErrors()
{
    ...
    status = HmacKdfExtract(NULL, NULL, 0, NULL, 0, NULL, 0);
    errorCount += UNITTEST_INT(ERR_NULL_POINTER, ERR_NULL_POINTER, status);

    status = HmacKdfExtract(&SHA1Suite, NULL, 0, NULL, 0, NULL, 0);
    errorCount += UNITTEST_INT(ERR_NULL_POINTER, ERR_NULL_POINTER, status);

    status = HmacKdfExtract(
        &SHA1Suite, NULL, 0, NULL, 0, pSmallOutput, sizeof(pSmallOutput));
    errorCount += UNITTEST_INT(ERR_BAD_LENGTH, ERR_BAD_LENGTH, status);
...
static int HmacKdfExpandTestErrors()
{
    ...
    status = HmacKdfExpand(NULL, NULL, 0, NULL, 0, NULL, 0, NULL, 0);
    errorCount += UNITTEST_INT(ERR_NULL_POINTER, ERR_NULL_POINTER, status);

    status = HmacKdfExpand(&SHA1Suite, NULL, 0, NULL, 0, NULL, 0, NULL, 0);
    errorCount += UNITTEST_INT(ERR_NULL_POINTER, ERR_NULL_POINTER, status);

    status = HmacKdfExpand(&SHA1Suite, pData, 0, NULL, 0, NULL, 0, NULL, 0);
    errorCount += UNITTEST_INT(ERR_NULL_POINTER, ERR_NULL_POINTER, status);

    status = HmacKdfExpand(&SHA1Suite, pData, 0, NULL, 0, NULL, 10, pData, 0);
...
exit:
    ...
    fstatus = DIGICERT_freeDigicert();
    UNITTEST_STATUS(OK, fstatus);
    if (OK == status)
        status = fstatus;
