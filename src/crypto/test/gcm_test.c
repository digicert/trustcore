...
static void gcm_speed_test_64k(MOC_SYM(hwAccelDescr hwAccelCtx) ubyte dummy)
{
    ubyte* buffer = 0;
    int i;
    ...
    if ( buffer)
    {
        ...
        pC = GCM_createCtx_64k(MOC_SYM(hwAccelCtx) zeroes, 16, TRUE);
        for ( i = 0; i < COUNTOF(sizes); ++i)
        {
            ...
            times(&tstart);
            counter = 0;
            GCM_init_64k(MOC_SYM(hwAccelCtx) pC, zeroes, 16, zeroes, 16);
            while( ALARM_OFF)
            {
                GCM_update_encrypt_64k(MOC_SYM(hwAccelCtx) pC, buffer, sizes[i]);
                counter++;
            }
            GCM_final_64k(pC, tag);
            times(&tend);
            diffTime = tend.tms_utime-tstart.tms_utime;
            diffTime /= sysconf(_SC_CLK_TCK);
            kbytes = sizes[i] * (counter / 1024.0);
            printf("\tGCM64k: %d blocks of %d bytes in %g seconds of CPU time\n", 
                   counter, sizes[i], diffTime);
            printf("GCM64k: %g kbytes/second (CPU time)(%d bytes block) (1 kbyte = 1024 bytes)\n",
                   kbytes/diffTime, sizes[i]);
            
        }
        GCM_deleteCtx_64k(MOC_SYM(hwAccelCtx) &pC);
        FREE(buffer);
    }    
}
...
static void gcm_speed_test_4k(MOC_SYM(hwAccelDescr hwAccelCtx) ubyte dummy)
{
    ubyte* buffer = 0;
    int i;
    ubyte4 sizes[] = { 16, 64, 256, 1024, 8192 };
    ...
    if ( buffer)
    {
        ...
        pC = GCM_createCtx_4k(MOC_SYM(hwAccelCtx) zeroes, 16, TRUE);
        for ( i = 0; i < COUNTOF(sizes); ++i)

        {
            ...
            times(&tstart);
            counter = 0;
            GCM_init_4k(MOC_SYM(hwAccelCtx) pC, zeroes, 16, zeroes, 16);
            while( ALARM_OFF)
            {
                GCM_update_encrypt_4k(MOC_SYM(hwAccelCtx) pC, buffer, sizes[i]);
                counter++;
            }
            GCM_final_4k(pC, tag);
            times(&tend);
            ...
            diffTime /= sysconf(_SC_CLK_TCK);
            kbytes = sizes[i] * (counter / 1024.0);
            printf("\tGCM4k: %d blocks of %d bytes in %g seconds of CPU time\n", 
                   counter, sizes[i], diffTime);
            printf("GCM4k: %g kbytes/second (CPU time)(%d bytes block) (1 kbyte = 1024 bytes)\n",
                   kbytes/diffTime, sizes[i]);
            
        }
        GCM_deleteCtx_4k(MOC_SYM(hwAccelCtx) &pC);
        FREE(buffer);
    }    
}
#endif
...
static void gcm_speed_test_256b(MOC_SYM(hwAccelDescr hwAccelCtx) ubyte dummy)
{
    ubyte* buffer = 0;
    int i;
    ubyte4 sizes[] = { 16, 64, 256, 1024, 8192 };
    ...
    if ( buffer)
    {
        ...
        pC = GCM_createCtx_256b(MOC_SYM(hwAccelCtx) zeroes, 16, TRUE);
        for ( i = 0; i < COUNTOF(sizes); ++i)
        {
            struct tms tstart, tend;
            double diffTime, kbytes;
            ...
            times(&tstart);
            counter = 0;
            GCM_init_256b(MOC_SYM(hwAccelCtx) pC, zeroes, 16, zeroes, 16);
            while( ALARM_OFF)
            {
                GCM_update_encrypt_256b(MOC_SYM(hwAccelCtx) pC, buffer, sizes[i]);
                counter++;
            }
            GCM_final_256b(pC, tag);
            times(&tend);
            diffTime = tend.tms_utime-tstart.tms_utime;
            diffTime /= sysconf(_SC_CLK_TCK);
            kbytes = sizes[i] * (counter / 1024.0);
            printf("\tGCM256b: %d blocks of %d bytes in %g seconds of CPU time\n", 
                   counter, sizes[i], diffTime);
            printf("GCM256b: %g kbytes/second (CPU time)(%d bytes block) (1 kbyte = 1024 bytes)\n",
                   kbytes/diffTime, sizes[i]);
            
        }
        GCM_deleteCtx_256b(MOC_SYM(hwAccelCtx) &pC);
        FREE(buffer);
    }    
}
#endif
...
int gcm_test_all_64k()
{
    int retVal = 0;
#ifdef __ENABLE_DIGICERT_GCM_64K__
    MSTATUS status;
    ubyte4 i,j;
    ubyte *key, *pt, *adata, *ct, *nonce, *etag;
    ubyte4 keyLen, ptLen, aLen, ctLen, nonceLen, etagLen;
    ...
    key = pt = adata = ct = nonce = 0;

    /* encrypt test */
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
    {
        GCM_test* pTest = gTests + i;

        keyLen = UNITTEST_UTILS_str_to_byteStr(pTest->key, &key);
        nonceLen = UNITTEST_UTILS_str_to_byteStr( pTest->nonce, &nonce);
        ctLen = UNITTEST_UTILS_str_to_byteStr( pTest->ct, &ct);
        etagLen = UNITTEST_UTILS_str_to_byteStr( pTest->tag, &etag);
        aLen = UNITTEST_UTILS_str_to_byteStr( pTest->adata, &adata);
            
        ptLen = UNITTEST_UTILS_str_to_byteStr( pTest->data, &pt);

        /* all in one shot */
        pC = GCM_createCtx_64k(MOC_SYM(hwAccelCtx) key, keyLen, TRUE);
        status = GCM_init_64k(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_encrypt_64k(MOC_SYM(hwAccelCtx) pC, pt, ptLen);
        retVal += UNITTEST_STATUS(i, status);
  
        status = GCM_final_64k( pC, tag);
        retVal += UNITTEST_STATUS(i, status);
        DIGI_MEMCMP( pt, ct, ctLen, &res);
        retVal += UNITTEST_INT( i, res, 0);

        DIGI_MEMCMP( etag, tag, AES_BLOCK_SIZE, &res);
        retVal += UNITTEST_INT( i, res, 0);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ptLen)
            {
                ...
                status = GCM_update_encrypt_64k(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ptLen -= num;
            }
    ...
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
        ...
        /* all in one shot */
        pC = GCM_createCtx_64k(MOC_SYM(hwAccelCtx) key, keyLen, FALSE);
        status = GCM_init_64k(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_decrypt_64k(MOC_SYM(hwAccelCtx) pC, ct, ctLen);
        retVal += UNITTEST_STATUS(i, status);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ctLen)
            {
                ...
                status = GCM_update_decrypt_64k(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ctLen -= num;
            }
    ...
    if ( 0 == retVal)
    {
        gcm_speed_test_64k(MOC_SYM(hwAccelCtx) NULL);
    }
#endif
...
int gcm_test_all_4k()
{
    ...
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
        ...
        /* all in one shot */
        pC = GCM_createCtx_4k(MOC_SYM(hwAccelCtx) key, keyLen, TRUE);

        status = GCM_init_4k(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_encrypt_4k(MOC_SYM(hwAccelCtx) pC, pt, ptLen);
        retVal += UNITTEST_STATUS(i, status);
  
        status = GCM_final_4k( pC, tag);
        retVal += UNITTEST_STATUS(i, status);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ptLen)
            {
                ...
                status = GCM_update_encrypt_4k(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ptLen -= num;
            }
    ...
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
        ...
        /* all in one shot */
        pC = GCM_createCtx_4k(MOC_SYM(hwAccelCtx) key, keyLen, FALSE);

        status = GCM_init_4k(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_decrypt_4k(MOC_SYM(hwAccelCtx) pC, ct, ctLen);
        retVal += UNITTEST_STATUS(i, status);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ctLen)
            {
                ...
                status = GCM_update_decrypt_4k(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ctLen -= num;
            }
    ...
    if ( 0 == retVal)
    {
        gcm_speed_test_4k(MOC_SYM(hwAccelCtx) NULL);
    }
#endif
...
int gcm_test_all_256b()
{
    ...
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
        ...
        /* all in one shot */
        pC = GCM_createCtx_256b(MOC_SYM(hwAccelCtx) key, keyLen, TRUE);

        status = GCM_init_256b(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_encrypt_256b(MOC_SYM(hwAccelCtx) pC, pt, ptLen);
        retVal += UNITTEST_STATUS(i, status);
  
        status = GCM_final_256b( pC, tag);
        retVal += UNITTEST_STATUS(i, status);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ptLen)
            {
                ...
                status = GCM_update_encrypt_256b(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ptLen -= num;
            }
        ...
        GCM_deleteCtx_256b(MOC_SYM(hwAccelCtx) (BulkCtx*)&pC);
        FREE( key);
        FREE( adata);
        FREE( nonce);
        FREE( ct);
    ...
    for (i = 0; i < COUNTOF(gTests); ++i)
    {
        ...
        /* all in one shot */
        pC = GCM_createCtx_256b(MOC_SYM(hwAccelCtx) key, keyLen, FALSE);

        status = GCM_init_256b(MOC_SYM(hwAccelCtx) pC, nonce, nonceLen, adata, aLen);
        retVal += UNITTEST_STATUS(i, status);
        
        status = GCM_update_decrypt_256b(MOC_SYM(hwAccelCtx) pC, ct, ctLen);
        retVal += UNITTEST_STATUS(i, status);
        ...
        for ( j = 1; j <= ctLen; ++j)
        {
            ...
            while (ctLen)
            {
                ...
                status = GCM_update_decrypt_256b(MOC_SYM(hwAccelCtx) pC, p, num);
                retVal += UNITTEST_STATUS(i, status);
                p += num;
                ctLen -= num;
            }
    ...
    if ( 0 == retVal)
    {
        gcm_speed_test_256b(MOC_SYM(hwAccelCtx)NULL);
    }
#endif
...
cases use the same notation asin Equations1and2, with the exception that Ni 
is used in place of 
Xi 
when GHASH is used to compute Y0, in order to distinguish that case from the later invocation 
