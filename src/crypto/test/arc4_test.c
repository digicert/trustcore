...
#ifdef __ARC4_HARDWARE_CIPHER__
#include "../arc4.h"

/* dummy functions for build only test */
static void prepare_key(ubyte *key_data_ptr, sbyte4 key_data_len, rc4_key *key){}
static void rc4(ubyte *buffer_ptr, sbyte4 buffer_len, rc4_key *key){}
#endif
...
int arc4_test_all()
{
    int i, retVal = 0;
    rc4_key key;
    const arc4_test* pBigTest;
    sbyte4 resCmp;

    for (i = 0; i < COUNTOF(gArc4Tests); ++i)
    {       

        prepare_key( (ubyte*) gArc4Tests[i].key, gArc4Tests[i].keyLen,
                     &key);
        
        DIGI_MEMCPY(gWorkBuffer, gArc4Tests[i].input, 
                   gArc4Tests[i].msgLen);
        rc4( gWorkBuffer, gArc4Tests[i].msgLen, &key);
        
        DIGI_MEMCMP( gWorkBuffer, (ubyte*) gArc4Tests[i].output, 
                    gArc4Tests[i].msgLen, &resCmp);
        retVal += UNITTEST_INT( i, resCmp, 0); 
    }

    pBigTest = gArc4Tests + COUNTOF(gArc4Tests) - 1;

    /* alignment tests */
    for ( i = 0; i < 32; ++i)
    {
        prepare_key( (ubyte*) pBigTest->key, pBigTest->keyLen, &key);
 
        DIGI_MEMCPY(gWorkBuffer + i, pBigTest->input, pBigTest->msgLen - i);

        rc4( gWorkBuffer + i, pBigTest->msgLen - i, &key);
        
        DIGI_MEMCMP( gWorkBuffer+i, (ubyte*) pBigTest->output, 
                    pBigTest->msgLen - i, &resCmp);
        retVal += UNITTEST_INT( 100 + i, resCmp, 0); 
    }
    ...
    for ( i = 0; i < 32; ++i)
    {
        ...
        DIGI_MEMCPY(gWorkBuffer, pBigTest->input, pBigTest->msgLen);

        msgLen = pBigTest->msgLen / 2 + i;

        rc4( gWorkBuffer, msgLen, &key);
        rc4( gWorkBuffer + msgLen, pBigTest->msgLen - msgLen, &key); 
        
        DIGI_MEMCMP( gWorkBuffer, (ubyte*) pBigTest->output, 
                   pBigTest->msgLen, &resCmp);
        retVal += UNITTEST_INT( 100 + i, resCmp, 0); 
    }
    ...
    if (0 == retVal)
    {
        ...
        printf("\tARC4: %d kbytes in %g seconds of CPU time\n", 
               counter, diffTime);
        printf("ARC4: %g kbytes/second (CPU time) (1 kbyte = 1024 bytes)\n", 
               counter/diffTime);
    }
#endif
