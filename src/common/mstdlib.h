...
#define DIGI_MALLOC( x, y) DIGI_MALLOC_TRACK( x, y, (ubyte *) __func__, __LINE__)
#define DIGI_CALLOC( x, y, z) DIGI_CALLOC_TRACK( x, y, z, (ubyte *) __func__, __LINE__)
#define DIGI_MALLOC_ALIGN( x, y, z) DIGI_MALLOC_ALIGN_TRACK( x, y, z, (ubyte *) __func__, __LINE__)
...
#define DIGI_MALLOC_MEMCPY( x, y, z, w) DIGI_MALLOC_MEMCPY_TRACK( x, y, z, w, (ubyte *) __func__, __LINE__)
...
MOC_EXTERN ubyte4  DIGI_NTOHL(const ubyte *v);
MOC_EXTERN ubyte2  DIGI_NTOHS(const ubyte *v);
MOC_EXTERN void    DIGI_HTONL(ubyte n[4], ubyte4 h);
MOC_EXTERN void    DIGI_HTONS(ubyte n[2], ubyte2 h);

/* REVIEW -- prototypes should use void* */
MOC_EXTERN MSTATUS DIGI_MEMMOVE(ubyte *pDest, const ubyte *pSrc, sbyte4 len);
MOC_EXTERN MSTATUS DIGI_MEMCPY(void *pDest, const void *pSrc, sbyte4 len);
MOC_EXTERN MSTATUS DIGI_MEMCMP(const ubyte *pSrc1, const ubyte *pSrc2, usize len, sbyte4 *pResult);
MOC_EXTERN MSTATUS DIGI_MEMSET(ubyte *pDest, ubyte value, usize len);
MOC_EXTERN MSTATUS DIGI_XORCPY(void *pDst, const void *pSrc, ubyte4 numBytes);
MOC_EXTERN MSTATUS DIGI_CTIME_MATCH(const void *a, const void *b, ubyte4 len,
                                   intBoolean *pDiffer);

MOC_EXTERN ubyte   returnHexDigit(ubyte4 digit);
MOC_EXTERN sbyte   MTOLOWER(sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISSPACE( sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISLWS( sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISXDIGIT( sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISLOWER( sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISDIGIT( sbyte c);
MOC_EXTERN byteBoolean   DIGI_ISASCII( sbyte c);

MOC_EXTERN sbyte4  DIGI_STRCMP(const sbyte *pString1, const sbyte *pString2);
MOC_EXTERN sbyte4  DIGI_STRNCMP(const sbyte *pString1, const sbyte *pString2, ubyte4 n);
MOC_EXTERN sbyte4  DIGI_STRNICMP(const sbyte *pString1, const sbyte* pString2, ubyte4 n);
MOC_EXTERN ubyte4  DIGI_STRLEN(const sbyte *s);
MOC_EXTERN ubyte4  DIGI_STRCBCPY( sbyte* dest, ubyte4 destSize, const sbyte* src);
MOC_EXTERN sbyte*  DIGI_STRCHR(sbyte *src, sbyte ch, ubyte4 len);
MOC_EXTERN ubyte4  DIGI_STRCAT( sbyte* dest, const sbyte* addsrc);
MOC_EXTERN ubyte4  DIGI_BITLENGTH( ubyte4 w);
MOC_EXTERN ubyte4  DIGI_BITCOUNT( ubyte4 v);

struct TimeDate;
MOC_EXTERN sbyte4 DIGI_cmpTimeDate(const struct TimeDate* first,
...
MOC_EXTERN sbyte4  DIGI_ATOL(const sbyte *s, const sbyte **stop);

/* convert a string to a hex byte arrey. pOut must have enough space */
MOC_EXTERN MSTATUS DIGI_ATOH(ubyte *pHexString, ubyte4 hexStrLen, ubyte *pOut);
...
/* write a integer to a string using decimal base. Returns the
    ...
    NOT NUL terminated */
MOC_EXTERN sbyte* DIGI_LTOA(sbyte4 value, sbyte *buff, ubyte4 bufSize);

/* 0 = Sunday, ..... 6 = Saturday */
MOC_EXTERN sbyte4 DIGI_DAYOFWEEK( sbyte4 day, sbyte4 month /*1-12*/, sbyte4 year /* 1752- */);
...
#define UNITS_MALLOC(a,b)   DIGI_MALLOC_ALIGN((a),(b),16)
#define UNITS_FREE(a)       DIGI_FREE((a))

#else /* not __ALTIVEC__ */

#define NEED_MALLOC_ALIGN   0
#define UNITS_MALLOC(a,b)   DIGI_MALLOC((a),(b))
#define UNITS_FREE(a)       DIGI_FREE((a))
 ...
 * and cnverts them to calls to DIGI_MALLOC and DIGI_FREE.
 */
...
MOC_EXTERN MSTATUS MEM_PROFILE_ADD_SUFFIX(DIGI_MALLOC)(void **ppPtr, ubyte4 bufSize MEM_PROFILE_TRACK_DECL);
...
/* DIGI_MALLOC_ALIGN is the same as DIGI_MALLOC, except this call will guarantee
 ...
 * it is the same as DIGI_MALLOC.
 * At the moment, the only supported value for alignment is 16.
 */
MOC_EXTERN MSTATUS MEM_PROFILE_ADD_SUFFIX(DIGI_MALLOC_ALIGN)(void **ppPtr, ubyte4 bufSize, ubyte4 alignment MEM_PROFILE_TRACK_DECL);
/* Allocate memory and memset it to 0.
 * This will allocate count * size bytes.
 */
MOC_EXTERN MSTATUS MEM_PROFILE_ADD_SUFFIX(DIGI_CALLOC)(void **ppPtr, ubyte4 count, ubyte4 size MEM_PROFILE_TRACK_DECL);
MOC_EXTERN MSTATUS DIGI_FREE(void **ppPtr);

MOC_EXTERN MSTATUS shredMemory(ubyte **ppMemToShred, ubyte4 memToShredLen, byteBoolean freeMemory);

MOC_EXTERN MSTATUS DIGI_Defragment(uintptr *pAllocationList, ubyte4 listLen);

/* DIGI_MEMSET_FREE is a simple macro wrapper that calls shredMemory. It is
...
MOC_EXTERN MSTATUS DIGI_convertHexString(const char *src, ubyte *outbuf,
        ubyte4 outbuflen);
