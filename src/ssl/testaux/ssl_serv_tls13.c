...
static sbyte4  ssls_maxEarlyDataSize  = 5000;
static sbyte4  ssls_recvEarlyDataSize = 5000;
...
static CertificateInfo gCertificateInfos[] =
{
    ...
    CERTIFICATE_INFO_PEM( "ecc_384_mldsa65_", "BU", 384),
                         ...
                         certDistinguishedName* pCertNames, sbyte4 identity)
{
    ...
    retVal = OK;
    if ( OK > DIGICERT_readFile( pCI->certFileName,
                                &pCI->certDesc.pCertificate,
                                &pCI->certDesc.certLength) ||
       OK > DIGICERT_readFile( pCI->certKeyFileName,
    ...
    if ( pCI->caCertFileName) /* it's a chain */
    {
        ...
        certificates[1].data = pParentCI->certDesc.pCertificate;
        certificates[1].length = pParentCI->certDesc.certLength;
#endif
...
typedef struct SSL_SERV_TLS13_tls13PSKList
{
    ubyte *pPsk;
    ubyte4 pskLen;
    ubyte *pServerName;
    ...
    ubyte *pIdentity;
    ubyte4 identityLen;
    struct SSL_SERV_TLS13_tls13PSKList *pNext;
} SSL_SERV_TLS13_tls13PSKList;

SSL_SERV_TLS13_tls13PSKList *pSessionListHeader = NULL;
static int   count               = 0;
...
{
    SSL_SERV_TLS13_tls13PSKList *pNewSessNode;
    SSL_SERV_TLS13_tls13PSKList *pSessList = pSessionListHeader;
    MSTATUS status = OK;

    /* Allocate memory and copy the data */
    if (OK > (status = DIGI_MALLOC((void **)&pNewSessNode, sizeof(SSL_SERV_TLS13_tls13PSKList))))
    {
        goto exit;
    }

    if (OK > (status = DIGI_MALLOC((void **)&pNewSessNode->pPsk, pskLen)))
    {
        goto exit;
    }

    if (OK > (status = DIGI_MEMCPY(pNewSessNode->pPsk, pPsk, pskLen)))
    {
        goto exit;
    }

    pNewSessNode->pskLen = pskLen;

    if (OK > (status = DIGI_MALLOC((void **)&pNewSessNode->pIdentity, identityLengthPSK)))
    {
        goto exit;
    }

    if (OK > (status = DIGI_MEMCPY(pNewSessNode->pIdentity, pIdentityPSK, identityLengthPSK)))
    {
        goto exit;
    }
    pNewSessNode->identityLen = identityLengthPSK;
    pNewSessNode->pNext = NULL;

    /* Add a new node to the bottom of a list */
    while ( (NULL != pSessList ) && (NULL != pSessList->pNext))
    {
...
{
    SSL_SERV_TLS13_tls13PSKList *pSessList = pSessionListHeader;
    SSL_SERV_TLS13_tls13PSKList *pSessFind = NULL;
    MSTATUS status = OK;
    sbyte4 cmpRes = 0;
    ...
    while ( NULL != pSessList )
    {
        ...
        if(pSessFind->identityLen == identityLengthPSK )
        {
            DIGI_MEMCMP(pIdentityPSK, pSessFind->pIdentity, identityLengthPSK, &cmpRes);
            if (0 == cmpRes)
            {
                *ppPsk = pSessFind->pPsk;
                *pPskLen = pSessFind->pskLen;
                break;
            }
        }
        pSessList = pSessList->pNext;
...
#if defined(__ENABLE_DIGICERT_OCSP_CLIENT__) && defined(__TEST_OCSP_TLS13__)

typedef struct
{
    byteBoolean isMutualAuth;
    MSTATUS ocspStatus;
} MAuthOcspResult;

static sbyte4
SSL_EXAMPLE_funcPtrCertStatusCallback(sbyte4 connectionInstance, const ubyte *pCert,
                ubyte4 certLen, ubyte *pOcspResp, ubyte4 ocspRespLen, sbyte4 ocspStatus)
{
    MAuthOcspResult *pOcspResult = NULL;

    printf("\nOCSP Stapling status: %d\n", ocspStatus);


    SSL_getCookie(connectionInstance, (void **) &pOcspResult);
    if (NULL != pOcspResult)
        pOcspResult->ocspStatus = ocspStatus;
...
static MSTATUS
SSL_EXAMPLE_funcPtrCertCallback(sbyte4 connectionInstance,
                        struct certChain *pCertChain, MSTATUS validationStatus)
{
    MAuthOcspResult *pOcspResult = NULL;

    SSL_getCookie(connectionInstance, (void **) &pOcspResult);
    if (NULL != pOcspResult)
        pOcspResult->isMutualAuth = TRUE;
...
SSL_SERV_initUpcalls()
{
    ...
    SSL_sslSettings()->sslTimeOutHello                  = 0;
    SSL_sslSettings()->sslTimeOutReceive                = 0;
    ...
    SSL_sslSettings()->funcPtrServerSavePSK    = SSL_EXAMPLE_funcPtrSavePSK;
    SSL_sslSettings()->funcPtrLookupPSKParams  = SSL_EXAMPLE_funcPtrLookupPSKParams;
    SSL_sslSettings()->funcPtrServerDeletePSK  = SSL_EXAMPLE_funcPtrDeletePSK;

#if defined(__ENABLE_DIGICERT_OCSP_CLIENT__) && defined(__TEST_OCSP_TLS13__)
    SSL_sslSettings()->funcPtrSingleCertStatusCallback = SSL_EXAMPLE_funcPtrCertStatusCallback;
#endif
}
...
SSL_SERV_handleConnection( void* arg)
{
    ...
    MAuthOcspResult ocspResult;

    ocspResult.isMutualAuth = FALSE;
    ocspResult.ocspStatus = ERR_OCSP;
#endif

#if defined(__SSLSERV_OCSP_CLIENT__)
    ...
    if (0 < setSupportedGroup)
    {
        if (OK > SSL_setCipherAlgorithm(connectionInstance , &setSupportedGroup , 1 , TLS13_supportedGroups))
        {
            //goto exit;
        }
        printf("\n------server is %d supported group--------\n" , setSupportedGroup);
    }

    /* ------------------- psk test case 1--------------------------*/
    if (0 < setNumTickets)
    {
        if (OK > SSL_ioctl(connectionInstance , SSL_SET_NUM_TICKETS , setNumTickets))
        {
            //goto exit;
        }
        printf("\n------server is %d SSL_SET_NUM_TICKETS--------\n" , setNumTickets);
    }
    ...
    {
        if (OK > SSL_setMaxEarlyDataSize(connectionInstance, ssls_maxEarlyDataSize))
        {
            DEBUG_ERROR(DEBUG_SSL_EXAMPLE, "SSL_SERV_handleConnection: SSL_setMaxEarlyDataSize failed. ", connectionInstance);
        }
    }

    if (ssls_recvEarlyDataSize > 0 )
    {
        if (OK > SSL_setRecvEarlyDataSize(connectionInstance, ssls_recvEarlyDataSize))
        {
            DEBUG_ERROR(DEBUG_SSL_EXAMPLE, "SSL_SERV_handleConnection: SSL_setRecvEarlyDataSize failed. ", connectionInstance);
        }
    }
#endif
...
#if defined(__SSLSERV_OCSP_CLIENT__)
#if defined(__TEST_OCSP_TLS13__)
    ...
    if (OK > (status = SSL_setCertifcateStatusRequestExtensions(connectionInstance, pTrustedResponderCertsPath, trustedRespondercertCount, NULL, 0)))
    {
        DEBUG_ERROR(DEBUG_SSL_EXAMPLE, (sbyte *)"startHttpsThread: SSL_setCertifcateStatusRequestExtensions Failed ", status);
        SSL_closeConnection(connectionInstance);
        goto exit;
    }

    if (OK > (status = SSL_setCookie(connectionInstance, (void *) &ocspResult)))
    {
        DEBUG_ERROR(DEBUG_SSL_EXAMPLE, (sbyte *)"startHttpsThread: SSL_setCookie Failed ", status);
        SSL_closeConnection(connectionInstance);
        goto exit;
    }

    if (OK > (status = SSL_setCertAndStatusCallback(connectionInstance, SSL_EXAMPLE_funcPtrCertCallback)))
    {
        DEBUG_ERROR(DEBUG_SSL_EXAMPLE, (sbyte *)"startHttpsThread: SSL_setCertAndStatusCallback Failed ", status);
        SSL_closeConnection(connectionInstance);
        goto exit;
    }
#else
...
#if defined(__SSLSERV_OCSP_CLIENT__) && defined(__TEST_OCSP_TLS13__)
    if ( (TRUE == ocspResult.isMutualAuth) && (OK != ocspResult.ocspStatus) )
    {
        DEBUG_ERROR(DEBUG_SSL_EXAMPLE, (sbyte *)"startHttpsThread: Client side OCSP Failed with status = ", ocspResult.ocspStatus);
    }
#endif

#if defined(__ENABLE_DIGICERT_TLS13_PSK__)
    if(0 < setKeyupdateTests)
    {
        ...
        /* wait  one second , client request for key update */
        RTOS_sleepMS(10);
        ...
        printf("\n----------sendkeyupdate test 3, value 0,  ---------\n");
        /* ------------------- keyupdate test case 3--------------------------*/
        /* update key request */
        SSL_sendKeyUpdateRequest(connectionInstance, 0);

        if (0 > ( status = SSL_SERV_doRequest(connectionInstance, &stopRequest)))
            DEBUG_ERROR(DEBUG_SSL_EXAMPLE, "SSL_SERV_handleConnection: SSL_SERV_getRequest failed. ", status);

        RTOS_sleepMS(10);
        printf("\n----------sendkeyupdate test 3, value 1,  ---------\n");
        /* ------------------- keyupdate test case 4--------------------------*/
        /* update key request */
        SSL_sendKeyUpdateRequest(connectionInstance, 1);
...
#ifdef __SSLSERV_MUTUAL_AUTH_SUPPORT__
    if( 0 < setPosthandshakeAuthTests)
    {
        /* ------------------- posthand shake auth test case 1--------------------------*/
        if (OK > (status = SSL_sendPosthandshakeAuthCertificateRequest(connectionInstance)))
            DEBUG_ERROR(DEBUG_SSL_EXAMPLE, "SSL_SERV_handleConnection: SSL_sendPosthandshakeAuthCertificateRequest failed. ", status);
        /* to handle the io operation */
        if (0 > ( status = SSL_SERV_doRequest(connectionInstance, &stopRequest)))
            DEBUG_ERROR(DEBUG_SSL_EXAMPLE, "SSL_SERV_handleConnection: SSL_SERV_getRequest failed. ", status);
        setPosthandshakeAuthTests = 0;
    }
#endif
...
SSL_SERV_startOCSPServers()
{
    ...
    ubyte4  issuerLen = 0;
#if defined(__TEST_OCSP_TLS13__)
    ubyte*  pOcspIssuer = NULL;
    ubyte4  ocspIssuerLen = 0;
    AsymmetricKey asymKey = { 0 };
    ...
    if (OK > (status = DIGICERT_readFile("ocsp_test_certs/RSAChild1Key.pem",&pDerKey, &derKeyLen)))
        goto exit;

    if (OK > (status = DIGICERT_readFile("ocsp_test_certs/RSACA.der", &pOcspIssuer, &ocspIssuerLen)))
        goto exit;

    if (OK > (status = CERT_STORE_addTrustPoint(pSslOcspCertStore, pOcspIssuer, ocspIssuerLen)))
        goto exit;

    if (OK > (status = CRYPTO_deserializeAsymKey(pDerKey, derKeyLen, NULL, &asymKey)))
        goto exit;

    if (OK > (status = CRYPTO_serializeAsymKey(&asymKey, mocanaBlobVersion2, &pKeyBlob, &keyBlobLen)))
        goto exit;
...
static MSTATUS
SSL_SERV_TLS13_allocHashPtrElement(void *pHashCookie, hashTablePtrElement **ppRetNewHashElement)
{
    MSTATUS status = OK;
...
static MSTATUS
SSL_SERV_TLS13_freeHashPtrElement(void *pHashCookie, hashTablePtrElement *pFreeHashElement)
{
    MSTATUS status = OK;
                          ...
                          void *pKey, TapOperation op, ubyte getContext)
{
    ...
    if (NULL == g_pTapCtxHashTable)
    {
        ...
        if (OK > (status = HASH_TABLE_createPtrsTable(&g_pTapCtxHashTable, (1 << count) - 1, NULL, SSL_SERV_TLS13_allocHashPtrElement, SSL_SERV_TLS13_freeHashPtrElement)))
            goto exit;
    }
...
#elif defined(__ENABLE_DIGICERT_TLS13_PSK__)
    SSL_SERV_startServer(1490);
