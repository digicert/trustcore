 ...
 * and multiple TAPContexts being created. The hash table implementation stores
 * the contexts corresponding to the key, and frees all of them at the end.
 */
...
static char * 	       ssls_ServerCert2    = NULL;
static char * 	       ssls_ServerBlob2    = NULL;
...
static tls13PSK externalPsk = {
    ...
    .pskTLS13IdentityLength = sizeof("External TLS 1.3 PSK"),
...
{
    printf("Early data received is %s\n", (char*)pData);
    return 0;
}
...
SSL_EXAMPLE_sendBigResponseB(sbyte4 connectionInstance, sbyte* fileName, sbyte* httpVer)
{
    ...
    {
        (void) sprintf((char *)body, "%s|<--- body length %d    ", body, body_len);
        for (j = 0; j < 30; j++)
            (void) sprintf((char *)body, "%s@@@@@@@@@@@@@@@@@@@ %d ", body, DIGI_STRLEN((const sbyte *)body));
        (void) sprintf((char *)body, "%s\n", body);
    }

    (void) sprintf((char *)body, "%s</body></html>", body);
...
startHttpsThread(void* hsocketClient)
{
    ...
    sbyte4  status = OK;
    sbyte4  socketClient = (sbyte4)((usize) hsocketClient);
    ...
    if (0 > (connectionInstance = SSL_acceptConnection(socketClient, pSslCertStore)))
    {
        ...
        if (OK > (status = DIGICERT_readFile("ca.der",&issuerCertData,&issuerCertLen)))
            goto exit;
...
#if ((defined(__ENABLE_DIGICERT_TLS13__)) && (defined(__ENABLE_DIGICERT_TLS13_PSK__)) && (__ENABLE_DIGICERT_TLS13_0RTT__))
...
initUpcallsAndCertStores()
{
    ...
    if(NULL != fullpath)
    {
        ...
        certificate.length = certAndKey.certLen;
        certificate.data = certAndKey.pCert;
...
load_certs:
    if (count == 1)
        SSL_CERT_UTILS_getFullPath(ssls_KeyStore, ssls_ServerCert2, &fullpath);
    else
        SSL_CERT_UTILS_getFullPath(ssls_KeyStore, ssls_ServerCert, &fullpath);
    if(NULL != fullpath)
    {
    ...
    if (count == 1)
        SSL_CERT_UTILS_getFullPath(ssls_KeyStore, ssls_ServerBlob2, &fullpath);
    else
...
exit1:
    ...
    if (OK > (status = CERT_STORE_addIdentityWithCertificateChain(
    		pSslCertStore, &certificate, 1, certDesc.pKeyBlob, certDesc.keyBlobLength))) {
        if (count == 1)
            goto load_cacerts;
        else
            goto exit;
...
load_cacerts:
    if (ssls_CACert)
    {
        ...
        if (NULL != fullpath)
        {
            if (OK > (status = DIGICERT_readFile(fullpath, &CACertData, &CACertLength)))
            {
                myPrintStringError("initUpcallsAndCertStores::DIGICERT_readFile()::file ", fullpath);
                goto exit;
            }
...
uninitUpcallsAndCertStores()
{
    ...
    if (NULL != ssls_ServerCert2)
    {
        DIGI_FREE((void **)&ssls_ServerCert2);
    }
    if (NULL != ssls_ServerBlob2)
    {
        DIGI_FREE((void **)&ssls_ServerBlob2);
    }
#ifdef __ENABLE_DIGICERT_SSL_MUTUAL_AUTH_SUPPORT_EXAMPLE__
    if (NULL != ssls_CACert)
    {
...
/*	}  */
	...
	for (i = 1; i < argc; i++)
	{
		...
		else if (DIGI_STRCMP((const sbyte *)argv[i], (const sbyte *)"-ssl_server_cert2") == 0)
		{
			{
				if (++i < argc)
				{
					setStringParameter(&ssls_ServerCert2, argv[i]);
					serverCertSet = 1;
				}
				continue;
			}
		}
        else if (DIGI_STRCMP((const sbyte *)argv[i], (const sbyte *)"-ssl_server_keyblob2") == 0)
		{
			{
				if (++i < argc)
				{
					setStringParameter(&ssls_ServerBlob2, argv[i]);
					serverBlobSet = 1;
				}
				continue;
			}
