...
extern MSTATUS
HPUX_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "HPUX_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "HPUX_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    saServer.sin_len = sizeof(saServer);
    saServer.sin_addr.s_addr = inet_addr(INADDR_ANY);
    saServer.sin_port = htons(portNumber);

    if (0 != (nRet = bind(newSocket, (struct sockaddr*)&saServer, sizeof(saServer))))
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "HPUX_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "HPUX_TCP_listenSocket: bind() socket : ", newSocket);
        status = ERR_TCP_LISTEN_BIND_ERROR;
        goto error_cleanup;
    }

    if (0 != (nRet = listen(newSocket, SOMAXCONN)))
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "HPUX_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
HPUX_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
} /* HPUX_TCP_acceptSocket */


/*------------------------------------------------------------------*/

extern MSTATUS
HPUX_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
extern MSTATUS
HPUX_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
HPUX_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* HPUX_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
HPUX_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
                      ubyte4 *pNumBytesWritten)
{
    ...
    retValue = send(socket, (const char *)pBuffer, numBytesToWrite, 0);
...
extern MSTATUS
HPUX_TCP_getPeerName(TCP_SOCKET socket, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
