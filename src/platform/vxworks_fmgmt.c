 ...
 * vxworks_fmgmt.c
 *
 * VXWorks File Management Abstraction Layer
 *
...
typedef struct
{
    sbyte *pPath;
    DIR *pDir;
} VxDirDescriptorWrapper;

extern intBoolean VXWORKS_pathExists (const sbyte *pFilePath, FileDescriptorInfo *pFileInfo)
{
...
extern MSTATUS VXWORKS_rename (const sbyte *pOldName, sbyte *pNewName)
{
...
extern MSTATUS VXWORKS_mkdir (const sbyte *pDirectoryName, ubyte4 mode)
{
    MSTATUS status = ERR_NULL_POINTER;
#ifdef __RTOS_VXWORKS_6__
...
#ifdef __RTOS_VXWORKS_6__
    exists = VXWORKS_pathExists(pDirectoryName, NULL);
    if (TRUE == exists)
    {
        return ERR_DIR_EXISTS;
    }

    if (0 == mkdir (pDirectoryName))
    {
        return OK;
    }
...
static MSTATUS recursiveDelete (sbyte *pPath)
{
    ...
    status = VXWORKS_remove (pPath, FALSE);
    if (OK == status)
    {
        goto exit;
    }
    ...
    {
        status = VXWORKS_getFirstFile (pPath, &pDir, &dirEntry);
        if (OK != status)
            goto exit;
        ...
        else
        {
            ...
            do
            {
                ...
                status = VXWORKS_getNextFile (pDir, &dirEntry);
                if (OK != status)
                    goto exit;
...
extern MSTATUS VXWORKS_remove (const sbyte *pFilePath, intBoolean recursive)
{
    MSTATUS status = ERR_NULL_POINTER;
    ubyte4 tflags;
...
extern MSTATUS VXWORKS_changeCWD (const sbyte *pNewCwd)
{
    MSTATUS status = ERR_NULL_POINTER;
    if (NULL == pNewCwd)
        goto exit;
...
extern MSTATUS VXWORKS_getCWD (sbyte *pCwd, ubyte4 cwdLength)
{
    MSTATUS status = ERR_NULL_POINTER;
...
extern MSTATUS VXWORKS_ftell (FileDescriptor pFileCtx, ubyte4 *pOffset)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
    sbyte4 offset;
...
extern MSTATUS VXWORKS_getDirectoryPath (const sbyte *pFilePath, sbyte *pDirectoryPath, ubyte4 directoryPathLength)
{
    MSTATUS status;
...
extern MSTATUS VXWORKS_getDirectoryPathAlloc (const sbyte *pFilePath, sbyte **ppDirectoryPath)
{
    MSTATUS status = ERR_NULL_POINTER;
...
extern MSTATUS VXWORKS_getFullPath (const sbyte *pRelativePath, sbyte *pAbsolutePath, ubyte4 absolutePathLength)
{
    MSTATUS status = ERR_NULL_POINTER;
...
extern MSTATUS VXWORKS_getFullPathAlloc (const sbyte *pRelativePath, sbyte **ppAbsolutePath)
{
    MSTATUS status = ERR_NULL_POINTER;
...
extern MSTATUS VXWORKS_getNextFile (DirectoryDescriptor pDirCtx, DirectoryEntry *pFileCtx)
{
    MSTATUS status = ERR_NULL_POINTER;
    struct dirent *pDirEntry = NULL;
    DirectoryEntry *pNewDirEntryCtx = NULL;
    DIR *pDir = NULL;
    VxDirDescriptorWrapper *pWrapper = NULL;
    ...
    pWrapper = (VxDirDescriptorWrapper *)pDirCtx;
    ...
    /* Get the filetype */
    if (TRUE != VXWORKS_pathExists(pFullPath, &fileInfo))
    {
...
extern MSTATUS VXWORKS_getFirstFile (const sbyte *pDirPath, DirectoryDescriptor *ppNewDirCtx, DirectoryEntry *pFirstFile)
{
    MSTATUS status = ERR_NULL_POINTER;
    DIR *pDir = NULL;
    VxDirDescriptorWrapper *pWrapper = NULL;
    ...
    *ppNewDirCtx = NULL;
    ...
    if (NULL != pDir)
    {
        status = DIGI_MALLOC((void **)&pWrapper, sizeof(VxDirDescriptorWrapper));
        if (OK != status)
            goto exit;

        /* For vxworks we need to keep a copy of the path to use for a stat later as the filetype
         * is not in the struct dirent. */
        status = DIGI_MALLOC_MEMCPY((void **)&pWrapper->pPath, DIGI_STRLEN(pDirPath) + 1, pDirPath, DIGI_STRLEN(pDirPath) + 1);
        if (OK != status)
        {
        ...
        *ppNewDirCtx = (DirectoryDescriptor) pWrapper;
        ...
        status = VXWORKS_getNextFile ((DirectoryDescriptor) pWrapper, pFirstFile);
        goto exit;
    }
...
extern MSTATUS VXWORKS_closeDir (DirectoryDescriptor *ppDirCtx)
{
    MSTATUS status = ERR_NULL_POINTER;
    DIR *pDir = NULL;
    VxDirDescriptorWrapper *pWrapper = NULL;
    ...
    pWrapper = (VxDirDescriptorWrapper *)(*ppDirCtx);
    if (NULL != pWrapper)
    {
...
extern MSTATUS VXWORKS_fopen (const sbyte *pFileName, const sbyte *pMode, FileDescriptor *ppNewFileCtx)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
...
extern MSTATUS VXWORKS_fclose (FileDescriptor *ppFileCtx)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
...
extern MSTATUS VXWORKS_fread (ubyte *pBuffer, ubyte4 itemSize, ubyte4 numOfItems, FileDescriptor pFileCtx, ubyte4 *pBytesRead)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
    ubyte4 readCount;
...
extern MSTATUS VXWORKS_fwrite (const ubyte *pBuffer, ubyte4 itemSize, ubyte4 numOfItems, FileDescriptor pFileCtx, ubyte4 *pBytesWrote)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
    ubyte4 writeCount;
...
extern MSTATUS VXWORKS_fseek (FileDescriptor pFileCtx, sbyte8 offset, ubyte4 m_whence)
#else
extern MSTATUS VXWORKS_fseek (FileDescriptor pFileCtx, sbyte4 offset, ubyte4 m_whence)
#endif
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
...
extern MSTATUS VXWORKS_fflush (FileDescriptor pFileCtx)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
...
extern MSTATUS VXWORKS_fprintf (FileDescriptor pFileCtx, const sbyte *pFormat, ...)
{
    MSTATUS status = ERR_NULL_POINTER;
    FILE *pFile;
    va_list args;
...
extern MSTATUS VXWORKS_getEnvironmentVariableValue (const sbyte *pVariableName, sbyte *pValueBuffer, ubyte4 valueBufferLength)
{
    MSTATUS status = ERR_NULL_POINTER;
    sbyte *pValue;
    ubyte4 valueLength;
...
extern MSTATUS VXWORKS_getEnvironmentVariableValueAlloc (const sbyte *pVariableName, sbyte **ppValueBuffer)
{
    MSTATUS status = ERR_NULL_POINTER;
    sbyte *pValue;
...
extern sbyte* VXWORKS_fgets (sbyte *pString, ubyte4 stringLen, FileDescriptor pFileCtx)
{
...
extern MSTATUS VXWORKS_getProcessPath (sbyte *pDirectoryPath, ubyte4 directoryPathLength, ubyte4 *pBytesRead)
{
    return ERR_NOT_IMPLEMENTED;
}

extern MSTATUS VXWORKS_getProcessPathAlloc (sbyte **ppDirectoryPath)
{
    return ERR_NOT_IMPLEMENTED;
}
#endif /* __VXWORKS_FMGMT__ */
