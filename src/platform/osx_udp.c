...
extern MSTATUS
OSX_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    MSTATUS status = OK;
...
extern MSTATUS
OSX_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    ...
    if (NULL != pHostName)
    {
        ...
        if (NULL != (pHostAddr = gethostbyname((const char*) pHostName)))
        {
...
static MSTATUS
OSX_UDP_bindConnect(void **ppRetUdpDescr,
                    ...
                    intBoolean isNonBlocking, intBoolean connected)
{
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate OSX_UDP_interface */
    if (NULL == (pUdpIf = (OSX_UDP_interface*) MALLOC(sizeof(OSX_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(OSX_UDP_interface));
...
extern MSTATUS
OSX_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return OSX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
OSX_UDP_simpleBind(void **ppRetUdpDescr,
                   MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                   intBoolean isNonBlocking)
{
    return OSX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
OSX_UDP_unbind(void **ppReleaseUdpDescr)
{
    OSX_UDP_interface*  pUdpIf;
    MSTATUS             status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((OSX_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate OSX_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
OSX_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    OSX_UDP_interface*  pUdpIf = (OSX_UDP_interface *)pUdpDescr;
    MSTATUS             status = OK;
...
extern MSTATUS
OSX_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
OSX_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    OSX_UDP_interface   *pUdpIf = (OSX_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
OSX_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    OSX_UDP_interface*  pUdpIf = (OSX_UDP_interface *)pUdpDescr;
    ssize_t             result;
    MSTATUS             status = OK;
...
extern MSTATUS
OSX_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
                 ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    ...
    *pRetDataLength = 0;
    ...
    result = (int)recvfrom(pUdpIf->udpFd, (char *)pBuf, bufSize, 0,
...
extern MSTATUS
OSX_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    OSX_UDP_interface*      pUdpIf = (OSX_UDP_interface *)pUdpDescr;
...
extern MSTATUS
OSX_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr,
                   ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        OSX_UDP_interface *pUdpIf = (OSX_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
        {
            FD_SET(pUdpIf->udpFd, pSocketList);
            if (fdMax < pUdpIf->udpFd) fdMax = pUdpIf->udpFd;
        }
    }
    ...
    ret = select(fdMax + 1, pSocketList, NULL, NULL, &timeout);
    if (0 == ret) /* timed out */
    {
