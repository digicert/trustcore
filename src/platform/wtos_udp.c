...
extern MSTATUS
WTOS_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
/*  MSTATUS status = OK;
...
extern MSTATUS
WTOS_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
extern MSTATUS
WTOS_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    WTOS_UDP_interface*  pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate WTOS_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(WTOS_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(WTOS_UDP_interface));
...
extern MSTATUS
WTOS_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return WTOS_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
WTOS_UDP_simpleBind(void **ppRetUdpDescr,
                     MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                     intBoolean isNonBlocking)
{
    return WTOS_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
WTOS_UDP_unbind(void **ppReleaseUdpDescr)
{
    WTOS_UDP_interface*  pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((WTOS_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate WTOS_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
WTOS_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    WTOS_UDP_interface*    pUdpIf = (WTOS_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
WTOS_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
WTOS_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    WTOS_UDP_interface*  pUdpIf = (WTOS_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
WTOS_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    WTOS_UDP_interface*    pUdpIf = (WTOS_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
WTOS_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
WTOS_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    WTOS_UDP_interface*    pUdpIf = (WTOS_UDP_interface *)pUdpDescr;
...
extern MSTATUS
WTOS_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        WTOS_UDP_interface *pUdpIf = (WTOS_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
