...
extern MSTATUS
SYMBIAN_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
/*  MSTATUS status = OK;
...
extern MSTATUS
SYMBIAN_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
extern MSTATUS
SYMBIAN_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    SYMBIAN_UDP_interface*  pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate SYMBIAN_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(SYMBIAN_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(SYMBIAN_UDP_interface));
...
extern MSTATUS
SYMBIAN_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return SYMBIAN_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
SYMBIAN_UDP_simpleBind(void **ppRetUdpDescr,
                     MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                     intBoolean isNonBlocking)
{
    return SYMBIAN_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
SYMBIAN_UDP_unbind(void **ppReleaseUdpDescr)
{
    SYMBIAN_UDP_interface*  pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((SYMBIAN_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate SYMBIAN_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
SYMBIAN_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    SYMBIAN_UDP_interface*    pUdpIf = (SYMBIAN_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
SYMBIAN_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
SYMBIAN_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    SYMBIAN_UDP_interface*  pUdpIf = (SYMBIAN_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
SYMBIAN_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    SYMBIAN_UDP_interface*    pUdpIf = (SYMBIAN_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
SYMBIAN_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
SYMBIAN_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    SYMBIAN_UDP_interface*    pUdpIf = (SYMBIAN_UDP_interface *)pUdpDescr;
...
extern MSTATUS
SYMBIAN_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        SYMBIAN_UDP_interface *pUdpIf = (SYMBIAN_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
