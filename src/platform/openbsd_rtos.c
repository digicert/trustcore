 ...
 * openbsd_rtos.c
 *
 * OpenBSD RTOS Abstraction Layer
 *
...
extern MSTATUS
OPENBSD_mutexCreate(RTOS_MUTEX* pMutex, enum mutexTypes mutexType, int mutexCount)
{
...
extern MSTATUS
OPENBSD_mutexWait(RTOS_MUTEX mutex)
{
...
extern MSTATUS
OPENBSD_mutexRelease(RTOS_MUTEX mutex)
{
    pthread_mutex_t* pPthreadMutex = (pthread_mutex_t *)mutex;
...
extern MSTATUS
OPENBSD_mutexFree(RTOS_MUTEX* pMutex)
{
    pthread_mutex_t* pPthreadMutex;
...
extern ubyte4
OPENBSD_deltaMS(const moctime_t* origin, moctime_t* current)
{
    struct timeval tval;
    ubyte4 retVal = 0;
...
extern ubyte4
OPENBSD_deltaConstMS(const moctime_t* origin, const moctime_t* current)
{
    struct timeval diff;
...
extern moctime_t *
OPENBSD_timerAddMS(moctime_t* pTimer, ubyte4 addNumMS)
{
    ubyte4 sec;
...
extern void
OPENBSD_sleepMS(ubyte4 sleepTimeInMS)
{
...
extern MSTATUS
OPENBSD_createThread(void (*threadEntry)(void*), void* context, ubyte4 threadType, RTOS_THREAD *pRetTid)
{
    pthread_t   tid;
    int         ret;
    MSTATUS     status  = OK;
    ...
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "OPENBSD_createThread: pthread_create error ", ret);
...
extern void
OPENBSD_destroyThread(RTOS_THREAD tid)
{
