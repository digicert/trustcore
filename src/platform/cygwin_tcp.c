...
extern MSTATUS
CYGWIN_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*)"CYGWIN_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*)"CYGWIN_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "CYGWIN_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "CYGWIN_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "CYGWIN_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
CYGWIN_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
extern MSTATUS
CYGWIN_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
    ...
    server.sin_addr.s_addr = inet_addr((const char *)pIpAddress);
...
extern MSTATUS
CYGWIN_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
CYGWIN_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* CYGWIN_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
CYGWIN_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
...
extern MSTATUS
CYGWIN_TCP_getPeerName(TCP_SOCKET socket, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
