...
extern MSTATUS
OPENBSD_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS_S *pRetIpAddress)
{
...
extern MSTATUS
OPENBSD_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS_S *pRetIpAddress)
{
...
static MSTATUS
OPENBSD_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    OPENBSD_UDP_interface*  pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate OPENBSD_UDP_interface */
    if (NULL == (pUdpIf = (OPENBSD_UDP_interface*) MALLOC(sizeof(OPENBSD_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(OPENBSD_UDP_interface));
...
extern MSTATUS
OPENBSD_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return OPENBSD_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
OPENBSD_UDP_simpleBind(void **ppRetUdpDescr,
                   MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                   intBoolean isNonBlocking)
{
    return OPENBSD_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
OPENBSD_UDP_unbind(void **ppReleaseUdpDescr)
{
    OPENBSD_UDP_interface*  pUdpIf;
    MSTATUS             status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((OPENBSD_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate OPENBSD_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
OPENBSD_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    OPENBSD_UDP_interface*  pUdpIf = (OPENBSD_UDP_interface *)pUdpDescr;
    MSTATUS             status = OK;
...
extern MSTATUS
OPENBSD_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
OPENBSD_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    OPENBSD_UDP_interface*  pUdpIf = (OPENBSD_UDP_interface *)pUdpDescr;
    ssize_t             result;
    MSTATUS             status = OK;
...
extern MSTATUS
OPENBSD_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS_S *pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
OPENBSD_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
    OPENBSD_UDP_interface*    pUdpIf = (OPENBSD_UDP_interface *)pUdpDescr;
...
extern MSTATUS
OPENBSD_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr,
                    ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        OPENBSD_UDP_interface *pUdpIf = (OPENBSD_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
        {
