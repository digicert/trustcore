 ...
 * integrity_udp.c
 *
 * GHS Integrity UDP Abstraction Layer
 *
...
extern MSTATUS
INTEGRITY_UDP_getInterfaceAddress(sbyte *pHostName,
...
*/
    ...
    status = INTEGRITY_UDP_getAddressOfHost(pHostName, pRetIpAddress);
...
extern MSTATUS
INTEGRITY_UDP_getAddressOfHost(sbyte *pHostName,
                            MOC_IP_ADDRESS_S *pRetIpAddress)
{
...
#ifdef __ENABLE_DIGICERT_IPV6__
    /* check IPv6 colon notation */
    else if (0 < inet_pton(AF_INET6, (const char *)pHostName,
                           (void *) pRetIpAddress->uin.addr6))
    {
        pRetIpAddress->family = AF_INET6;
        status = OK;
    }
...
static MSTATUS
INTEGRITY_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    INTEGRITY_UDP_interface *pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate INTEGRITY_UDP_interface */
    if (NULL == (pUdpIf = (INTEGRITY_UDP_interface*)
                            MALLOC(sizeof(INTEGRITY_UDP_interface))))
    {
    ...
    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(INTEGRITY_UDP_interface));
...
extern MSTATUS
INTEGRITY_UDP_connect(void **ppRetUdpDescr,
                MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                intBoolean isNonBlocking)
{
    return INTEGRITY_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
INTEGRITY_UDP_simpleBind(void **ppRetUdpDescr,
                    MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                    intBoolean isNonBlocking)
{
    return INTEGRITY_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
INTEGRITY_UDP_unbind(void **ppReleaseUdpDescr)
{
    INTEGRITY_UDP_interface *pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((INTEGRITY_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate INTEGRITY_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 < pUdpIf->udpFd)
            close(pUdpIf->udpFd);
...
extern MSTATUS
INTEGRITY_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    INTEGRITY_UDP_interface *pUdpIf = (INTEGRITY_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
INTEGRITY_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress,
...
extern MSTATUS
INTEGRITY_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    INTEGRITY_UDP_interface *pUdpIf = (INTEGRITY_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
INTEGRITY_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize,
...
extern MSTATUS
INTEGRITY_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS_S *pPeerAddress,
...
extern MSTATUS
INTEGRITY_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo,
...
extern MSTATUS
INTEGRITY_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr,
                    ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        INTEGRITY_UDP_interface *pUdpIf = (INTEGRITY_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
        {
