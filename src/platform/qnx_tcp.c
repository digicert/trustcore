...
extern MSTATUS
QNX_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*)"QNX_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*)"QNX_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, (sbyte*)"QNX_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, (sbyte*)"QNX_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, (sbyte*)"QNX_TCP_listenSocket: listen() error: ", nRet);
    ...
    *listenSocket = newSocket;
    ...
    printf(" New Listen Socket Created ...\n");
    goto exit;
...
extern MSTATUS
QNX_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
} /* QNX_TCP_acceptSocket */


/*------------------------------------------------------------------*/

extern MSTATUS
QNX_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
extern MSTATUS
QNX_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
QNX_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* QNX_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
QNX_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
...
extern MSTATUS
QNX_TCP_getPeerName(TCP_SOCKET socket, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
...
/* Dummy stub */
extern MSTATUS
QNX_TCP_getHostByName(char* pDomainName, char* pIpAddress)
{
    return OK;
}
