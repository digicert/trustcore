...
/* duplicate symbols if you leave this defined */
...
extern MSTATUS
OSX_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*) "OSX_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, (sbyte*) "OSX_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "OSX_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "OSX_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "OSX_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
OSX_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
    ...
    {
    printf("accept error: %d and errno %d\n",newClientSocket,errno);
...
extern MSTATUS
OSX_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
extern MSTATUS
OSX_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
OSX_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
                     ubyte4 maxBytesToRead, ubyte4 *pNumBytesRead, ubyte4 msTimeout)
{
    ...
    *pNumBytesRead = 0;
    ...
    retValue = (int) recv(socket, pBuffer, maxBytesToRead, 0);
...
} /* OSX_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
OSX_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
                      ubyte4 *pNumBytesWritten)
{
    ...
    retValue = (int) send((TCP_SOCKET)socket, (const char *)pBuffer, numBytesToWrite, 0);
...
extern MSTATUS
OSX_TCP_getPeerName(TCP_SOCKET socket, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
    ...
    *pRetPortNo = htons(myAddress.SIN_PORT);
    ...
    pRetAddr->uin.addr = htonl((ubyte4)myAddress.SIN_ADDR.S_ADDR);
#else
