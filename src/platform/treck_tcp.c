...
extern MSTATUS
TRECK_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "TRECK_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "TRECK_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "TRECK_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "TRECK_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "TRECK_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
TRECK_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
extern MSTATUS
TRECK_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
extern MSTATUS
TRECK_TCP_closeSocket(TCP_SOCKET socket)
{
    tfClose(socket);
    return OK;
}
...
extern MSTATUS
TRECK_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
                     ubyte4 maxBytesToRead, ubyte4 *pNumBytesRead, ubyte4 msTimeout)
{
    ...
    if (TCP_NO_TIMEOUT != msTimeout)
    {
        ...
        if (0 == select(1 + socket, pSocketList, NULL, NULL, &timeout))
        {
...
} /* TRECK_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
TRECK_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
                      ubyte4 *pNumBytesWritten)
{
    ...
    if (0 > retValue)
    {
        if (TM_EWOULDBLOCK != tfGetSocketError(socket))
        {
