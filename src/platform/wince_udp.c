...
extern MSTATUS
WINCE_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    ...
    if (NULL == pHostName)
    {
        if (SOCKET_ERROR == gethostname((char *)myHostName, sizeof(myHostName)))
        {
            goto exit;
        }
...
extern MSTATUS
WINCE_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
static MSTATUS
WINCE_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    WINCE_UDP_interface*    pUdpIf = NULL;
    unsigned long           flags;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate WINCE_UDP_interface */
    if (NULL == (pUdpIf = (WINCE_UDP_interface*)MALLOC(sizeof(WINCE_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(WINCE_UDP_interface));
...
extern MSTATUS
WINCE_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return WINCE_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
WINCE_UDP_simpleBind(void **ppRetUdpDescr,
                        MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                        intBoolean isNonBlocking)
{
    return WINCE_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
WINCE_UDP_unbind(void **ppReleaseUdpDescr)
{
    WINCE_UDP_interface*    pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((WINCE_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate WINCE_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            closesocket(pUdpIf->udpFd);
...
extern MSTATUS
WINCE_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    WINCE_UDP_interface*    pUdpIf = (WINCE_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
WINCE_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
WINCE_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    WINCE_UDP_interface*   pUdpIf = (WINCE_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
WINCE_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    WINCE_UDP_interface*    pUdpIf = (WINCE_UDP_interface *)pUdpDescr;
    int                     result;
    MSTATUS                 status = OK;
...
extern MSTATUS
WINCE_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
WINCE_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    WINCE_UDP_interface*    pUdpIf = (WINCE_UDP_interface *)pUdpDescr;
#ifdef __ENABLE_DIGICERT_IPV6__
...
extern MSTATUS
WINCE_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        WINCE_UDP_interface *pUdpIf = (WINCE_UDP_interface *) ppUdpDescr[i];
