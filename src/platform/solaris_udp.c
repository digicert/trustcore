...
extern MSTATUS
SOLARIS_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    MSTATUS status = OK;
...
extern MSTATUS
SOLARIS_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
static MSTATUS
SOLARIS_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    SOLARIS_UDP_interface*   pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate SOLARIS_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(SOLARIS_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(SOLARIS_UDP_interface));
...
extern MSTATUS
SOLARIS_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return SOLARIS_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
SOLARIS_UDP_simpleBind(void **ppRetUdpDescr,
                   MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                   intBoolean isNonBlocking)
{
    return SOLARIS_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
SOLARIS_UDP_unbind(void **ppReleaseUdpDescr)
{
    SOLARIS_UDP_interface*  pUdpIf;
    MSTATUS             status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((SOLARIS_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate SOLARIS_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
SOLARIS_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    SOLARIS_UDP_interface*  pUdpIf = (SOLARIS_UDP_interface *)pUdpDescr;
    MSTATUS             status = OK;
...
extern MSTATUS
SOLARIS_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
SOLARIS_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    SOLARIS_UDP_interface*  pUdpIf = (SOLARIS_UDP_interface *)pUdpDescr;
    ssize_t             result;
    MSTATUS             status = OK;
...
extern MSTATUS
SOLARIS_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
SOLARIS_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    SOLARIS_UDP_interface*  pUdpIf = (SOLARIS_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
SOLARIS_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS * pRetAddr)
{
    SOLARIS_UDP_interface*    pUdpIf = (SOLARIS_UDP_interface *)pUdpDescr;
