...
extern MSTATUS
WTOS_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "WTOS_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "WTOS_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "WTOS_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "WTOS_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "WTOS_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
WTOS_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
} /* WTOS_TCP_acceptSocket */


/*------------------------------------------------------------------*/

extern MSTATUS
WTOS_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
    MSTATUS             status = OK;
#if 0
    ...
    server.sin_addr.s_addr = inet_addr(pIpAddress);

    if (0 != connect(*pConnectSocket, (struct sockaddr *)&server, sizeof(server)))
        status = ERR_TCP_CONNECT_ERROR;

#else
    if ((*pConnectSocket = tcp_client(pIpAddress, portNo, 0, 0)) < 0 )
        status = ERR_TCP_CONNECT_ERROR;
...
extern MSTATUS
WTOS_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
WTOS_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* WTOS_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
WTOS_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
...
extern MSTATUS
WTOS_TCP_getPeerName(TCP_SOCKET socket, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
