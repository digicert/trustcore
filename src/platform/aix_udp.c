...
extern MSTATUS
AIX_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS_S *pRetIpAddress)
{
...
extern MSTATUS
AIX_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS_S *pRetIpAddress)
{
...
extern MSTATUS
AIX_UDP_bindConnect(void **ppRetUdpDescr,
                      ...
                      intBoolean isNonBlocking, intBoolean connected)
{
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate AIX_UDP_interface */
    if (NULL == (pUdpIf = (AIX_UDP_interface*) MALLOC(sizeof(AIX_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(AIX_UDP_interface));
...
extern MSTATUS
AIX_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return AIX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
AIX_UDP_simpleBind(void **ppRetUdpDescr,
                     MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                     intBoolean isNonBlocking)
{
    return AIX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
AIX_UDP_unbind(void **ppReleaseUdpDescr)
{
    AIX_UDP_interface*    pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((AIX_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate AIX_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
AIX_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    AIX_UDP_interface*    pUdpIf = (AIX_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
AIX_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
AIX_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    AIX_UDP_interface*    pUdpIf = (AIX_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
AIX_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    AIX_UDP_interface*    pUdpIf = (AIX_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
AIX_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS_S *pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
AIX_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS_S *pRetAddr)
{
    AIX_UDP_interface*    pUdpIf = (AIX_UDP_interface *)pUdpDescr;
...
extern MSTATUS
AIX_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        AIX_UDP_interface *pUdpIf = (AIX_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
        {
