...
static MSTATUS
set_sock_buf(int fd, int bufType, int size)
{
    MSTATUS status = OK;

    if (0 > (status = setsockopt(fd, SOL_SOCKET, bufType, &size, sizeof(size))))
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "TCP set_sock_buf setsockopt error ", status);
...
extern MSTATUS
SOLARIS_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "SOLARIS_TCP_listenSocket: Could not create listen socket");
        status = ERR_TCP_LISTEN_SOCKET_ERROR;
        goto exit;
    }

    if (0 > setsockopt(newSocket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(int)))
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "SOLARIS_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
        status = ERR_TCP_SOCKOPT_ERROR;
        goto error_cleanup;
    }

    if (OK > (status = set_no_delay(newSocket)))
        goto exit;

    if (OK > (status = set_sock_buf(newSocket, SO_SNDBUF, 1024 * 1024)))
        goto exit;

    if (OK > (status = set_sock_buf(newSocket, SO_RCVBUF, 32 * 1024)))
        goto exit;
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "SOLARIS_TCP_listenSocket: bind() error: ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "SOLARIS_TCP_listenSocket: bind() socket: ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "SOLARIS_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
SOLARIS_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
extern MSTATUS
SOLARIS_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
#ifdef __ENABLE_DIGICERT_SOLARIS_IPV6__
    inet_pton(AF_INET, pIpAddress, &server.sin_addr);
#else
...
extern MSTATUS
SOLARIS_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
SOLARIS_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* SOLARIS_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
SOLARIS_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
