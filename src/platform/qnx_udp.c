...
/* Some functions that we need are not defined when _KERNEL is presetn */
#ifdef _KERNEL
...
extern MSTATUS
QNX_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
/*  MSTATUS status = OK;
...
extern MSTATUS
QNX_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
extern MSTATUS
QNX_UDP_bindConnect(void **ppRetUdpDescr,
                      ...
                      intBoolean isNonBlocking, intBoolean connected)
{
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate QNX_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(QNX_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(QNX_UDP_interface));
...
extern MSTATUS
QNX_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return QNX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
QNX_UDP_simpleBind(void **ppRetUdpDescr,
                     MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                     intBoolean isNonBlocking)
{
    return QNX_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
QNX_UDP_unbind(void **ppReleaseUdpDescr)
{
    QNX_UDP_interface*  pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((QNX_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate QNX_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
QNX_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    QNX_UDP_interface*    pUdpIf = (QNX_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
QNX_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
QNX_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    QNX_UDP_interface*  pUdpIf = (QNX_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
QNX_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    QNX_UDP_interface*    pUdpIf = (QNX_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
QNX_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
QNX_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    QNX_UDP_interface*    pUdpIf = (QNX_UDP_interface *)pUdpDescr;
...
extern MSTATUS
QNX_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        QNX_UDP_interface *pUdpIf = (QNX_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
