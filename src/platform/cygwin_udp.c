...
extern MSTATUS
CYGWIN_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    MSTATUS status = OK;
...
extern MSTATUS
CYGWIN_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    ...
    if (NULL != pHostName)
    {
        ...
        if (NULL != (pHostAddr = (struct hostent *)gethostbyname((const char *)pHostName)))
        {
...
extern MSTATUS
CYGWIN_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    CYGWIN_UDP_interface*  pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate CYGWIN_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(CYGWIN_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(CYGWIN_UDP_interface));
...
extern MSTATUS
CYGWIN_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return CYGWIN_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
CYGWIN_UDP_simpleBind(void **ppRetUdpDescr,
                     MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                     intBoolean isNonBlocking)
{
    return CYGWIN_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
CYGWIN_UDP_unbind(void **ppReleaseUdpDescr)
{
    CYGWIN_UDP_interface*  pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((CYGWIN_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate CYGWIN_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
CYGWIN_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    CYGWIN_UDP_interface*    pUdpIf = (CYGWIN_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
CYGWIN_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
CYGWIN_UDP_getFd(void *pUdpDescr, sbyte4 *fd)
{
    CYGWIN_UDP_interface*  pUdpIf = (CYGWIN_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
CYGWIN_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    CYGWIN_UDP_interface*    pUdpIf = (CYGWIN_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
CYGWIN_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
CYGWIN_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS * pRetAddr)
{
    CYGWIN_UDP_interface*    pUdpIf = (CYGWIN_UDP_interface *)pUdpDescr;
    ...
    *pRetPortNo = ntohs(myAddress.sin_port);
    *pRetAddr   = ntohl(myAddress.sin_addr.s_addr);
...
extern MSTATUS
CYGWIN_UDP_selReadAvl(void *ppUdpDescr[], sbyte4 numUdpDescr, ubyte4 msTimeout)
{
    MSTATUS         status = OK;
    ...
    {
        CYGWIN_UDP_interface *pUdpIf = (CYGWIN_UDP_interface *) ppUdpDescr[i];
        if (NULL != pUdpIf)
        {
