...
extern MSTATUS
OSE_TCP_listenSocket(TCP_SOCKET *listenSocket, ubyte2 portNumber)
{
    ...
    if (0 > newSocket)
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "OSE_TCP_listenSocket: Could not create listen socket");
    ...
    {
        DEBUG_PRINTNL(DEBUG_PLATFORM, "OSE_TCP_listenSocket: setsockopt(SO_REUSEADDR) error");
    ...
    if (0 > nRet)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "OSE_TCP_listenSocket: bind() error : ", nRet);
        DEBUG_ERROR(DEBUG_PLATFORM, "OSE_TCP_listenSocket: bind() socket : ", newSocket);
    ...
    if (nRet != 0)
    {
        DEBUG_ERROR(DEBUG_PLATFORM, "OSE_TCP_listenSocket: listen() error: ", nRet);
...
extern MSTATUS
OSE_TCP_acceptSocket(TCP_SOCKET *clientSocket, TCP_SOCKET listenSocket, intBoolean *isBreakSignalRequest)
{
...
extern MSTATUS
OSE_TCP_connectSocket(TCP_SOCKET *pConnectSocket, sbyte *pIpAddress, ubyte2 portNo)
{
...
extern MSTATUS
OSE_TCP_closeSocket(TCP_SOCKET socket)
{
    close(socket);
    return OK;
}
...
extern MSTATUS
OSE_TCP_readSocketAvailable(TCP_SOCKET socket, sbyte *pBuffer,
...
} /* OSE_TCP_readSocketAvailable */


/*------------------------------------------------------------------*/

extern MSTATUS
OSE_TCP_writeSocket(TCP_SOCKET socket, sbyte *pBuffer, ubyte4 numBytesToWrite,
                    ubyte4 *pNumBytesWritten)
{
    ...
    *pNumBytesWritten = 0;
    ...
    retValue = inet_send((TCP_SOCKET)socket, (const void *)pBuffer, (int)numBytesToWrite, 0);
