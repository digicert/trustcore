...
extern MSTATUS
FREEBSD_UDP_getInterfaceAddress(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
    MSTATUS status = OK;
...
extern MSTATUS
FREEBSD_UDP_getAddressOfHost(sbyte *pHostName, MOC_IP_ADDRESS *pRetIpAddress)
{
...
static MSTATUS
FREEBSD_UDP_bindConnect(void **ppRetUdpDescr,
...
{
    FREEBSD_UDP_interface*  pUdpIf = NULL;
    ...
    *ppRetUdpDescr = NULL;
    ...
    /* allocate FREEBSD_UDP_interface */
    if (NULL == (pUdpIf = MALLOC(sizeof(FREEBSD_UDP_interface))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    DIGI_MEMSET((ubyte *)pUdpIf, 0x00, sizeof(FREEBSD_UDP_interface));
...
extern MSTATUS
FREEBSD_UDP_connect(void **ppRetUdpDescr,
                  MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                  MOC_IP_ADDRESS dstAddress, ubyte2 dstPortNo,
                  intBoolean isNonBlocking)
{
    return FREEBSD_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
FREEBSD_UDP_simpleBind(void **ppRetUdpDescr,
                   MOC_IP_ADDRESS srcAddress, ubyte2 srcPortNo,
                   intBoolean isNonBlocking)
{
    return FREEBSD_UDP_bindConnect(ppRetUdpDescr, srcAddress, srcPortNo,
...
extern MSTATUS
FREEBSD_UDP_unbind(void **ppReleaseUdpDescr)
{
    FREEBSD_UDP_interface*  pUdpIf;
    MSTATUS                 status = ERR_NULL_POINTER;

    if (NULL == ppReleaseUdpDescr)
        goto exit;

    pUdpIf = *((FREEBSD_UDP_interface **)ppReleaseUdpDescr);

    /* de-allocate FREEBSD_UDP_interface */
    if (NULL != pUdpIf)
    {
        if (0 != pUdpIf)
            close(pUdpIf->udpFd);
...
extern MSTATUS
FREEBSD_UDP_send(void *pUdpDescr, ubyte *pData, ubyte4 dataLength)
{
    FREEBSD_UDP_interface*  pUdpIf = (FREEBSD_UDP_interface *)pUdpDescr;
    MSTATUS                 status = OK;
...
extern MSTATUS
FREEBSD_UDP_sendTo(void *pUdpDescr, MOC_IP_ADDRESS peerAddress, ubyte2 peerPortNo,
...
extern MSTATUS
FREEBSD_UDP_recv(void *pUdpDescr, ubyte *pBuf, ubyte4 bufSize, ubyte4 *pRetDataLength)
{
    FREEBSD_UDP_interface*  pUdpIf = (FREEBSD_UDP_interface *)pUdpDescr;
    ssize_t                 result;
    MSTATUS                 status = OK;
...
extern MSTATUS
FREEBSD_UDP_recvFrom(void *pUdpDescr, MOC_IP_ADDRESS* pPeerAddress, ubyte2* pPeerPortNo,
...
extern MSTATUS
FREEBSD_UDP_getSrcPortAddr(void *pUdpDescr, ubyte2 *pRetPortNo, MOC_IP_ADDRESS *pRetAddr)
{
    FREEBSD_UDP_interface*  pUdpIf = (FREEBSD_UDP_interface *)pUdpDescr;
