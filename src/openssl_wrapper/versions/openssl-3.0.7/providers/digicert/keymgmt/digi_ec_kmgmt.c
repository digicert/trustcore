 ...
 * digi_ec_kmgmt.c
 *
 * EC keygen implementations for OSSL 3.0 provider ADAPTED FROM OPENSSL code
 *
...
static int digiprov_ec_gen_set_params(void *genctx, const OSSL_PARAM params[]);
int moc_ecc_keygen(EC_KEY *pEcKey);
int moc_get_ecc_ex_app_data();

static
const char *digiprov_ec_query_operation_name(int operation_id)
{
    switch (operation_id) {
 ...
 */
static ossl_inline
int digiprov_key_to_params(const EC_KEY *eckey, OSSL_PARAM_BLD *tmpl,
...
static ossl_inline
int digiprov_otherparams_to_params(const EC_KEY *ec, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[])
{
...
static
void *digiprov_ec_newdata(void *provctx)
{
...
static
void digiprov_ec_freedata(void *keydata)
{
    EC_KEY_free(keydata);
}

static
int digiprov_ec_has(const void *keydata, int selection)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)
    {
        ...
        if (NULL == EC_KEY_get0_private_key(ec) && NULL == EC_KEY_get_ex_data(ec, moc_get_ecc_ex_app_data()))
            ok = 0;
    }
...
static int digiprov_ec_match(const void *keydata1, const void *keydata2, int selection)
{
...
static
int digiprov_common_import(void *keydata, int selection, const OSSL_PARAM params[], int sm2_wanted)
{
    EC_KEY *ec = keydata;
    int ok = 1;
...
static
int digiprov_ec_import(void *keydata, int selection, const OSSL_PARAM params[])
{
    return digiprov_common_import(keydata, selection, params, 0);
}

static
int digiprov_ec_export(void *keydata, int selection, OSSL_CALLBACK *param_cb, void *cbarg)
{
    EC_KEY *ec = keydata;
    ...
    if (!digiprov_is_running() || ec == NULL)
        return 0;
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        ...
        ok = ok && digiprov_key_to_params(ec, tmpl, NULL, include_private, &pub_key);
    }
    if ((selection & OSSL_KEYMGMT_SELECT_OTHER_PARAMETERS) != 0)
        ok = ok && digiprov_otherparams_to_params(ec, tmpl, NULL);
    ...
    OPENSSL_free(pub_key);  /* Allocated by Openssl EC point conversion, free with OPENSSL */
    OPENSSL_free(genbuf);
    BN_CTX_end(bnctx);
    BN_CTX_free(bnctx);
    return ok;
 ...
 */
#include "digi_ec_kmgmt_imexport.inc"

static ossl_inline
const OSSL_PARAM *digiprov_ec_imexport_types(int selection)
{
    int type_select = 0;
...
static
const OSSL_PARAM *digiprov_ec_import_types(int selection)
{
    return digiprov_ec_imexport_types(selection);
}

static
const OSSL_PARAM *digiprov_ec_export_types(int selection)
{
    return digiprov_ec_imexport_types(selection);
}

static
int digiprov_common_get_params(void *key, OSSL_PARAM params[], int sm2)
{
    int ret = 0;
    EC_KEY *eck = key;
    const EC_GROUP *ecg = NULL;
    ...
    ret = ossl_ec_group_todata(ecg, NULL, params, libctx, propq, bnctx, &genbuf)
          && digiprov_key_to_params(eck, NULL, params, 1, &pub_key)
          && digiprov_otherparams_to_params(eck, NULL, params);
err:
    OPENSSL_free(genbuf); /* Allocated by OPENSSL EC conversions, free with OPENSSL */
...
static
int digiprov_ec_get_params(void *key, OSSL_PARAM params[])
{
    return digiprov_common_get_params(key, params, 0);
}
...
static
const OSSL_PARAM *digiprov_ec_gettable_params(void *provctx)
{
...
static
const OSSL_PARAM *digiprov_ec_settable_params(void *provctx)
{
    return ec_known_settable_params;
}

static
int digiprov_ec_set_params(void *key, const OSSL_PARAM params[])
{
    EC_KEY *eck = key;
    const OSSL_PARAM *p;
...
static
int digiprov_ec_validate(const void *keydata, int selection, int checktype)
{
...
static void *digiprov_ec_gen_init(void *provctx, int selection,
                                  const OSSL_PARAM params[])
{
    ...
    if (!digiprov_ec_gen_set_params(gctx, params)) 
    {
...
static int digiprov_ec_gen_set_group(void *genctx, const EC_GROUP *src)
{
...
static int digiprov_ec_gen_set_template(void *genctx, void *templ)
{
    struct ec_gen_ctx *gctx = genctx;
    EC_KEY *ec = templ;
    ...
    if ((ec_group = EC_KEY_get0_group(ec)) == NULL)
        return 0;
    return digiprov_ec_gen_set_group(gctx, ec_group);
}
...
if (p != NULL) {                                                               \
    if (p->data_type != OSSL_PARAM_UTF8_STRING)                                \
        goto err;                                                              \
    (void) DIGI_FREE((void **) &val);                                           \
    status = digiprov_strdup((void **) &val, p->data);                         \
    if (OK != status)                                                          \
        goto err;                                                              \
}
...
if (p != NULL) {                                                               \
    ...
    (void) DIGI_FREE((void **) &val);                                           \
    len = p->data_size;                                                        \
    status = DIGI_MALLOC_MEMCPY((void **) &val, p->data_size, p->data, p->data_size);\
    if (OK != status)                                                          \
        goto err;                                                              \
}
...
static int digiprov_ec_gen_set_group_from_params(struct ec_gen_ctx *gctx)
{
    int ret = 0;
    OSSL_PARAM_BLD *bld;
    OSSL_PARAM *params = NULL;
...
static const OSSL_PARAM *digiprov_ec_gen_settable_params(ossl_unused void *genctx,
...
static int digiprov_ec_gen_assign_group(EC_KEY *ec, EC_GROUP *group)
{
    if (group == NULL) {
        ERR_raise(ERR_LIB_PROV, PROV_R_NO_PARAMETERS_SET);
        return 0;
 ...
 */
static void *digiprov_ec_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
    struct ec_gen_ctx *gctx = genctx;
    EC_KEY *ec = NULL;
    int ret = 0;
    ...
    if (gctx->gen_group == NULL) {
        if (!digiprov_ec_gen_set_group_from_params(gctx))
            goto err;
    } else {
        if (gctx->encoding != NULL) {
    ...
    ret = digiprov_ec_gen_assign_group(ec, gctx->gen_group);

    /* Whether you want it or not, you get a keypair, not just one half */
    if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0)
        ret = ret && moc_ecc_keygen(ec);
...
static void digiprov_ec_gen_cleanup(void *genctx)
{
    struct ec_gen_ctx *gctx = genctx;
    ...
    (void) DIGI_FREE((void **) &gctx->group_name);
    (void) DIGI_FREE((void **) &gctx->field_type);
    (void) DIGI_FREE((void **) &gctx->pt_format);
    (void) DIGI_FREE((void **) &gctx->encoding);
    (void) DIGI_FREE((void **) &gctx->seed);
    (void) DIGI_FREE((void **) &gctx->gen);
    (void) DIGI_FREE((void **) &gctx);
}

static void *digiprov_common_load(const void *reference, size_t reference_sz, int sm2_wanted)
{
    EC_KEY *ec = NULL;

    if (digiprov_is_running() && reference_sz == sizeof(ec))
    {
        /* The contents of the reference is the address to our object */
        ec = *(EC_KEY **)reference;

        /* sm2 not supported at this time 
...
static void *digiprov_ec_load(const void *reference, size_t reference_sz)
{
    return digiprov_common_load(reference, reference_sz, 0);
}

static void *digiprov_ec_dup(const void *keydata_from, int selection)
{
    if (!digiprov_is_running())
        return NULL;
...
const OSSL_DISPATCH digiprov_ec_keymgmt_functions[] = {
    { OSSL_FUNC_KEYMGMT_NEW,             (void (*)(void))digiprov_ec_newdata },
    { OSSL_FUNC_KEYMGMT_GEN_INIT,        (void (*)(void))digiprov_ec_gen_init },
    { OSSL_FUNC_KEYMGMT_GEN_SET_TEMPLATE,(void (*)(void))digiprov_ec_gen_set_template },
    { OSSL_FUNC_KEYMGMT_GEN_SET_PARAMS,  (void (*)(void))digiprov_ec_gen_set_params },
    { OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS,(void (*)(void))digiprov_ec_gen_settable_params },
    { OSSL_FUNC_KEYMGMT_GEN,             (void (*)(void))digiprov_ec_gen },
    { OSSL_FUNC_KEYMGMT_GEN_CLEANUP,     (void (*)(void))digiprov_ec_gen_cleanup },
    { OSSL_FUNC_KEYMGMT_LOAD,            (void (*)(void))digiprov_ec_load },
    { OSSL_FUNC_KEYMGMT_FREE,            (void (*)(void))digiprov_ec_freedata },
    { OSSL_FUNC_KEYMGMT_GET_PARAMS,      (void (*)(void))digiprov_ec_get_params },
    { OSSL_FUNC_KEYMGMT_GETTABLE_PARAMS, (void (*)(void))digiprov_ec_gettable_params },
    { OSSL_FUNC_KEYMGMT_SET_PARAMS,      (void (*)(void))digiprov_ec_set_params },
    { OSSL_FUNC_KEYMGMT_SETTABLE_PARAMS, (void (*)(void))digiprov_ec_settable_params },
    { OSSL_FUNC_KEYMGMT_HAS,             (void (*)(void))digiprov_ec_has },
    { OSSL_FUNC_KEYMGMT_MATCH,           (void (*)(void))digiprov_ec_match },
    { OSSL_FUNC_KEYMGMT_VALIDATE,        (void (*)(void))digiprov_ec_validate },
    { OSSL_FUNC_KEYMGMT_IMPORT,          (void (*)(void))digiprov_ec_import },
    { OSSL_FUNC_KEYMGMT_IMPORT_TYPES,    (void (*)(void))digiprov_ec_import_types },
    { OSSL_FUNC_KEYMGMT_EXPORT,          (void (*)(void))digiprov_ec_export },
    { OSSL_FUNC_KEYMGMT_EXPORT_TYPES,    (void (*)(void))digiprov_ec_export_types },
    { OSSL_FUNC_KEYMGMT_QUERY_OPERATION_NAME, (void (*)(void))digiprov_ec_query_operation_name },
    { OSSL_FUNC_KEYMGMT_DUP,                  (void (*)(void))digiprov_ec_dup },
    { 0, NULL }
};
