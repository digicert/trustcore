 ...
 * digi_dh_exch.c
 *
 * DH implementations for OSSL 3.5 provider ADAPTED FROM OPENSSL CODE
 *
...
static int digiprov_dh_init(void *vpdhctx, void *vdh, const OSSL_PARAM params[])
{
    ...
    pdhctx->dh = vdh;
    pdhctx->kdf_type = PROV_DH_KDF_NONE;
    if (!digiprov_dh_set_ctx_params(pdhctx, params))
        return 0;

    /* OpenSSL 3.5 has FIPS handling here. dh key check is performed only when FIPS_MODULE is enabled */
