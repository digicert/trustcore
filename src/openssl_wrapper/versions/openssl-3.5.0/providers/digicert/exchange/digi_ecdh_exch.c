 ...
 * digi_ecdh_exch.c
 *
 * ECDH implementations for OSSL 3.5 provider ADAPTED FROM OPENSSL code
 *
...
static int digiprov_ecdh_init(void *vpecdhctx, void *vecdh, const OSSL_PARAM params[])
{
    ...
    if (!digiprov_ecdh_set_ctx_params(pecdhctx, params))
        return 0;

    /* OpenSSL 3.5 has FIPS handling here. EC key check is performed only when FIPS_MODULE is enabled */
    return 1;
}
...
static int digiprov_ecdh_set_ctx_params(void *vpecdhctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        /* OpenSSL 3.5 has FIPS handling here. KDF digest check is performed only when FIPS_MODULE is enabled */
    }
                                                  ...
                                                  size_t *psecretlen, size_t outlen)
{
    ...
    /* OpenSSL 3.5 has FIPS handling here. */
