 ...
 * digi_x963_kdf.c
 *
 * ANSI_X963-KDF implementation for OSSL 3.5 provider. ADAPTED from OPENSSL code
 *
...
/* Concatenate multiple INFO parameters into a single buffer */
static int digiprov_concat_info_params(const OSSL_PARAM *params, unsigned char **out, size_t *out_len)
{
    MSTATUS status = OK;
    const OSSL_PARAM *p;
    unsigned char *new_info = NULL;
    size_t total_info_len = 0;
    size_t offset = 0;
    ...
    if (NULL == p)
        return 1;
    for (; NULL != p; p = OSSL_PARAM_locate_const(p + 1, OSSL_KDF_PARAM_INFO))
    {
        if (NULL == p->data || 0 == p->data_size)
            continue;

        if (OSSL_PARAM_OCTET_STRING != p->data_type)
            return 0;

        if ((size_t)-1 - total_info_len < p->data_size)
            return 0;
        total_info_len += p->data_size;
    }

    status = DIGI_CALLOC((void **)&new_info, 1, total_info_len);
    if (OK != status)
    {
        return 0;
    }
    ...
    for (; NULL != p; p = OSSL_PARAM_locate_const(p + 1, OSSL_KDF_PARAM_INFO))
    {
        if (NULL != p->data && 0 < p->data_size)
        {
            (void) DIGI_MEMCPY(new_info + offset, p->data, p->data_size);
            offset += p->data_size;
        }
    }
    ...
    if (*out != NULL)
    {
        (void) DIGI_MEMSET_FREE(out, *out_len);
    }
    *out = new_info;
    *out_len = total_info_len;
...
static int digiprov_sskdf_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    /* Handle multiple INFO parameters by concatenating them */
    if (!digiprov_concat_info_params(params, &ctx->info, &ctx->info_len))
        return 0;
