 ...
 * digi_hmac_kdf.c
 *
 * HMAC-KDF implementation for OSSL 3.5 provider. ADAPTED from OPENSSL code
 *
...
static int digiprov_hkdf_common_set_ctx_params(DP_HKDF_CTX *ctx, const OSSL_PARAM params[])
{
    ...
    if (NULL != p)
    {
        ...
        digest_name = (const char *)p->data;

        /* Check for XOF digests */
        if (OPENSSL_strcasecmp(digest_name, "SHAKE-128") == 0 ||
            OPENSSL_strcasecmp(digest_name, "SHAKE-256") == 0 ||
            OPENSSL_strcasecmp(digest_name, "SHAKE128") == 0 ||
            OPENSSL_strcasecmp(digest_name, "SHAKE256") == 0) {
            ERR_raise(ERR_LIB_PROV, PROV_R_XOF_DIGESTS_NOT_ALLOWED);
            return 0;
        }

        status = digiprov_get_digest_data(digest_name, &ctx->pBHAlgo, &digestLen, NULL);
        if (OK != status)
        {
