 ...
 * digi_pqc_keymgmt.c
 *
 * PQC keygen implementations for OSSL 3.5 provider
 *
...
extern int digiprov_pqc_key_up_ref(DP_PQC_KEY *pKey)
{
    ...
    REF_PRINT_COUNT("PQC_KEY", i, pKey);
    REF_ASSERT_ISNT(i < 2);
    return ((i > 1) ? 1 : 0);
}
...
static DP_PQC_KEY *digiprov_pqc_new_key(OSSL_LIB_CTX *libctx, size_t cid, size_t secSize, const char *propq)
{
    ...
    if (!CRYPTO_NEW_REF(&pRet->references, 1))
        goto err;
...
err:
    ...
    CRYPTO_FREE_REF(&pRet->references);
...
extern void digiprov_pqc_key_free(DP_PQC_KEY *pKey)
{
    ...
    CRYPTO_DOWN_REF(&pKey->references, &i);
    REF_PRINT_COUNT("PQC_KEY", i, pKey);
    if (i > 0)
        return;
    REF_ASSERT_ISNT(i < 0);
    ...
    CRYPTO_FREE_REF(&pKey->references);
