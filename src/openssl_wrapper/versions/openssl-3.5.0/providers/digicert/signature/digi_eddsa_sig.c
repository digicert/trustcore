 ...
 * digi_eddsa_sig.c
 *
 * EDDSA implementations for OSSL 3.5 provider Adapted from OPENSSL code
 *
...
static int digiprov_eddsa_set_ctx_params(void *vpeddsactx, const OSSL_PARAM params[])
{
    DP_EDDSA_CTX *peddsactx = (DP_EDDSA_CTX *)vpeddsactx;
    const OSSL_PARAM *p;
    char instanceName[OSSL_MAX_NAME_SIZE] = "";
    char *pInstanceName = instanceName;
    ...
    /*
     * The "instance" parameter specifies EdDSA variants (Ed25519, Ed25519ctx, Ed25519ph, etc.)
     * The DigiCert provider currently only supports the standard Ed25519 and Ed448 variants.
     * We accept the parameter but only allow the standard variant names.
     */
    p = OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_INSTANCE);
    if (p != NULL)
    {
        if (!OSSL_PARAM_get_utf8_string(p, &pInstanceName, sizeof(instanceName)))
            return 0;

        if (OPENSSL_strcasecmp(pInstanceName, "Ed25519") != 0
            && OPENSSL_strcasecmp(pInstanceName, "Ed448") != 0)
        {
            ERR_raise(ERR_LIB_PROV, PROV_R_NOT_SUPPORTED);
            return 0;
        }
        /* Instance matches key type, nothing to do */
    }
...
static const OSSL_PARAM *digiprov_eddsa_settable_ctx_params(ossl_unused void *vpeddsactx,
                                                            ossl_unused void *provctx)
{
...
const OSSL_DISPATCH digiprov_ed25519_functions[] =
{
    ...
    { OSSL_FUNC_SIGNATURE_SET_CTX_PARAMS,      (void (*)(void))digiprov_eddsa_set_ctx_params },
    { OSSL_FUNC_SIGNATURE_SETTABLE_CTX_PARAMS, (void (*)(void))digiprov_eddsa_settable_ctx_params },
    { 0, NULL }
};
