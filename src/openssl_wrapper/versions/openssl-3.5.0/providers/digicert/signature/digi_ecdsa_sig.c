 ...
 * digi_ecdsa_sig.c
 *
 * ECDSA implementations for OSSL 3.5 provider ADAPTED from OPENSSL code
 *
...
static int digiprov_ecdsa_signverify_init(void *vctx, void *ec, const OSSL_PARAM params[], int operation)
{
    ...
    /* OpenSSL 3.5 has FIPS handling here. Key check is performed only if FIPS_MODULE_MODE is enabled */
                                   ...
                                   const char *mdprops)
{
    ...
    /* OpenSSL 3.5 has FIPS handling here: ossl_fips_ind_digest_sign_check*/
