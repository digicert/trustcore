 ...
 * digi_rsa_sig.c
 *
 * RSA s/v implementations for OSSL 3.5 provider ADAPTED FROM OPENSSL code
 *
...
static int digiprov_rsa_setup_md(DP_RSA_CTX *ctx, const char *mdname, const char *mdprops)
{
    ...
    if (mdname != NULL) 
    {
        ...
        if (!digiprov_rsa_check_padding(ctx, mdname, NULL, md_nid))
            goto err; 
...
static void *digiprov_rsa_newctx(void *provctx, const char *propq)
{
    ...
    pCtx->saltlen = RSA_PSS_SALTLEN_AUTO_DIGEST_MAX;
    pCtx->min_saltlen = -1;
...
static int digiprov_rsa_sv_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[], int operation)
{
    ...
    /* OpenSSL 3.5 has FIPS handling here. RSA key check is performed only when FIPS_MODULE is enabled */
