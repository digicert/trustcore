 ...
 * digi_dsa_sig.c
 *
 * DSA implementations for OSSL 3.5 provider ADAPTED FROM OPENSSL CODE
 *
...
static int digiprov_dsa_get_ctx_params(void *vpdsactx, OSSL_PARAM *params)
{
    ...
    if (p != NULL
        && !OSSL_PARAM_set_octet_string(p, pdsactx->aid_len == 0 ? NULL : pdsactx->aid_buf, pdsactx->aid_len))
        return 0;
