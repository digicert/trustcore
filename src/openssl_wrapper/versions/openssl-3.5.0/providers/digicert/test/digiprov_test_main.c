 ...
 *
 * Test of the digicert OSSL 3.5 provider
 *
...
test_start:
    ...
    {
        tret = ret;
        /* OpenSSL 3.5.0 has removed support for 1024-bit DSA key sizes */
        /* ret += test_dsa_sign(pLibCtx, 1024, 160, "SHA-1"); not supported */
        ret += test_dsa_sign(pLibCtx, 2048, 224, "SHA-224");
        ret += test_dsa_sign(pLibCtx, 2048, 256, "SHA-256");
        ret += test_dsa_sign(pLibCtx, 3072, 256, "SHA-256");
        /* DSA 3072 is supported, but currently disabled as these test cases increase CI execution time */
     /* ret += test_dsa_sign(pLibCtx, 3072, 256, "SHA-384");
        ret += test_dsa_sign(pLibCtx, 3072, 256, "SHA-512"); */
