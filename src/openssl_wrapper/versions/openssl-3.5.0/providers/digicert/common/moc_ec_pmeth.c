...
void DIGI_registerECMeth(EVP_PKEY_METHOD **ec_pmeth)
{
...
static MSTATUS DIGI_EVP_convertEdDigiToOssl(ECCKey *pMocKey, ECX_KEY **ppOsslKey)
{
    MSTATUS status;
...
static MSTATUS DIGI_EVP_convertEdOsslToDigi(EVP_PKEY *pOsslKey, ECCKey **ppMocKey)
{
    MSTATUS status;
    ubyte4 curveId;
    ECCKey *pNewKey = NULL;
...
static int moc_pkey_ecx_keygen(EVP_PKEY_CTX *pCtx, EVP_PKEY *pPkey)
{
    ...
    status = DIGI_EVP_convertEdDigiToOssl(pNewKey, &pOsslKey);
    if (OK != status)
        goto exit;
    ...
    EVP_PKEY_CTX *pCtx, unsigned char *pKey, size_t *pKeyLen)
{
    ...
    status = DIGI_EVP_convertEdOsslToDigi(pCtx->pkey, &pMocKey);
    if (OK != status)
        goto exit;
    ...
    const unsigned char *pMsg, size_t msgLen)
{
    ...
    status = DIGI_EVP_convertEdOsslToDigi(
...
void DIGI_registerEcx25519Meth(EVP_PKEY_METHOD **ec_pmeth)
{
...
void DIGI_registerEcx448Meth(EVP_PKEY_METHOD **ec_pmeth)
{
...
void DIGI_registerEd25519Meth(EVP_PKEY_METHOD **ec_pmeth)
{
...
void DIGI_registerEd448Meth(EVP_PKEY_METHOD **ec_pmeth)
{
