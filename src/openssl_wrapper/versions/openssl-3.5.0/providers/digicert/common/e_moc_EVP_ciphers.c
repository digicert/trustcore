...
#define VERSION_1_1_0_OR_1_1_1C_OR_3_0_OR_3_5
#else
...
static int DIGI_EVP_engineInit(ENGINE *e);
static int DIGI_EVP_engineFinish(ENGINE *e);
static int DIGI_EVP_engineDestroy(ENGINE *e);
BIGNUM *DIGI_EVP_vlong2BN(vlong *v);
#if defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE__) && !defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) && !defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__)
 ...
 */
#if defined(VERSION_1_1_0_OR_1_1_1C_OR_3_0_OR_3_5)
...
#endif /* VERSION_1_1_0_OR_1_1_1C_OR_3_0_OR_3_5 */
...
#error "Mocana EVP with OpenSSL 1.1.x and 3.x must be built with Crypto Interface enabled"
#endif
...
#endif /* __ENABLE_DIGICERT_OPENSSL_LIB_3_5__ */
...
int DIGI_EVP_ENGINE_unregister()
{
    ENGINE *pEngine = ENGINE_by_id("mocana");
    if (NULL == pEngine)
    {
...
int DIGI_EVP_setFipsMode(int fipsMode)
{
    ...
    else
    {
        ...
        if (0 == fipsMode)
        {
            if (0 == DIGI_EVP_ENGINE_unregister())
            {
                return 0;
            }
        }
    ...
    EVP_CIPHER_CTX *pCtx, EVP_CIPHER_CTX *pCtxCopy)
{
    ...
    /* Copy the outer EVP_CIPHER_CTX fields */
    pCtxCopy->encrypt = pCtx->encrypt;
    pCtxCopy->buf_len = pCtx->buf_len;
    pCtxCopy->num = pCtx->num;
    DIGI_MEMCPY(pCtxCopy->oiv, pCtx->oiv, sizeof(pCtx->oiv));
    DIGI_MEMCPY(pCtxCopy->iv, pCtx->iv, sizeof(pCtx->iv));
    DIGI_MEMCPY(pCtxCopy->buf, pCtx->buf, sizeof(pCtx->buf));
...
static MSTATUS DIGI_EVP_AES_GCM_setIv(
...
static int DIGI_EVP_AES_GCM_TLS_doCipher(
    ...
    size_t inLen)
{
    ...
    {
        return DIGI_EVP_AES_GCM_TLS_doCipher(pCtx, pOut, pIn, inLen);
    }
    ...
    EVP_CIPHER_CTX *pCtx, int type, int arg, void *pPtr)
{
    ...
    switch (type)
    {
        ...
        case EVP_CTRL_GCM_IV_GEN:
            ...
            status = DIGI_EVP_AES_GCM_setIv(pGCtx, pGCtx->pIv, pGCtx->ivLen);
            if (OK != status)
            {
                return 0;
            }
...
static int DIGI_EVP_doCustomCipher(
    EVP_CIPHER_CTX *pCtx,
    unsigned char *pOut,
...
#if defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) || defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__)
static int DIGI_EVP_doAESWrapCipherEx(EVP_CIPHER_CTX *ctx, unsigned char *out,
...
int DIGI_EVP_doAESWrapCipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
    ...
    if (FALSE == pad)
    {
        return DIGI_EVP_doAESWrapCipherEx(ctx, out, in, inlen, transform);
    }
...
#endif /* if defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) || defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__) */
...
static int moc_evp_cipher_nids[] = {
    ...
    NID_des_ede3_ecb,
    NID_des_ede3_cbc
#if (defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) || defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__)) && !defined(__DISABLE_3DES_TWO_KEY_CIPHER__)
    ,
    NID_des_ede_ecb,
    NID_des_ede_cbc
#endif
    ...
    NID_id_aes192_wrap,
    NID_id_aes256_wrap
#if defined(__ENABLE_BLOWFISH_CIPHERS__) && (defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) || defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__))
    ,
    NID_bf_cbc
#endif
};
...
/* TODO unify ifdefs */
#endif /* defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__) / defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) */
...
#else /* if defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_0__) || defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__) */
static const EVP_CIPHER _s_hidden_aeswrap_128 = {
	NID_id_aes128_wrap,
	16,
	16,
...
int DIGI_EVP_RC5_ctrl(EVP_CIPHER_CTX *pCtx, int type, int arg, void *pPtr)
{
    ...
    switch (type)
    {
        ...
        case EVP_CTRL_COPY:
        {
            EVP_CIPHER_CTX *pCtxOut = (EVP_CIPHER_CTX *) pPtr;
            MOC_EVP_RC5_CIPHER_CTX *pMocCtxOut = (MOC_EVP_RC5_CIPHER_CTX *) DIGI_EVP_CIPHER_CTX_getCipherData(pCtxOut);
            MSTATUS status = OK;

            if (NULL == pMocCtxOut)
                return 0;

            DIGI_MEMCPY(pMocCtxOut, pMocRC5Ctx, sizeof(MOC_EVP_RC5_CIPHER_CTX));
            pMocCtxOut->pKey = NULL;
            pMocCtxOut->pEncrData = NULL;

            if (NULL != pMocRC5Ctx->pKey)
            {
                status = DIGI_MALLOC((void **)&(pMocCtxOut->pKey), EVP_CIPHER_CTX_key_length(pCtxOut));
                if (OK != status)
                    return 0;

                DIGI_MEMCPY(pMocCtxOut->pKey, pMocRC5Ctx->pKey, EVP_CIPHER_CTX_key_length(pCtxOut));
            }

            if (NULL != pMocRC5Ctx->pEncrData)
            {
#ifdef __ENABLE_DIGICERT_CRYPTO_INTERFACE__
                status = CRYPTO_INTERFACE_MocRC5CloneCtx(MOC_SYM(hwAccelCtx) pMocRC5Ctx->pEncrData, &(pMocCtxOut->pEncrData));
#else
                status = MocRC5CloneCtx(MOC_SYM(hwAccelCtx) pMocRC5Ctx->pEncrData, &(pMocCtxOut->pEncrData));
#endif
                if (OK != status)
                    return 0;
            }
...
int DIGI_EVP_CHACHAPOLY_ctrl(EVP_CIPHER_CTX *pCtx, int type, int arg, void *pPtr)
{
    ...
    switch (type)
    {
        ...
        {
            EVP_CIPHER_CTX *pCtxOut = (EVP_CIPHER_CTX *) pPtr;
            MOC_EVP_CHACHAPOLY_CIPHER_CTX *pMocCtxOut = (MOC_EVP_CHACHAPOLY_CIPHER_CTX *) DIGI_EVP_CIPHER_CTX_getCipherData(pCtxOut);
            MSTATUS status;

            if (NULL == pMocCtxOut)
                return 0;

            DIGI_MEMCPY(pMocCtxOut, pMocCtx, sizeof(MOC_EVP_CHACHAPOLY_CIPHER_CTX));
            pMocCtxOut->pEncrData = NULL;

            /* Clone the ChaCha20Poly1305 context */
            if (NULL != pMocCtx->pEncrData)
            {
#if defined(__ENABLE_DIGICERT_CRYPTO_INTERFACE__)
                status = CRYPTO_INTERFACE_ChaCha20Poly1305_cloneCtx(MOC_SYM(hwAccelCtx) pMocCtx->pEncrData, &(pMocCtxOut->pEncrData));
#else
                status = ChaCha20Poly1305_cloneCtx(MOC_SYM(hwAccelCtx) pMocCtx->pEncrData, &(pMocCtxOut->pEncrData));
#endif
                if (OK != status)
                    return 0;
            }
...
MSTATUS DIGI_EVP_convertSha3TypeOsslToDigi(int nid, ubyte4 *pMode)
{
    MSTATUS status;
...
int moc_sha3_init(EVP_MD_CTX *pEvpCtx)
{
    ...
    status = DIGI_EVP_convertSha3TypeOsslToDigi(pEvpCtx->digest->type, &mode);
    if (OK != status)
        goto exit;
...
int moc_sha3_copy(EVP_MD_CTX *pDest, const EVP_MD_CTX *pSrc)
{
    ...
    status = DIGI_EVP_convertSha3TypeOsslToDigi(pSrc->digest->type, &mode);
    if (OK != status)
    {
...
#endif /* __ENABLE_DIGICERT_OPENSSL_LIB_1_1_1C__ OR __ENABLE_DIGICERT_OPENSSL_LIB_3_0__ || __ENABLE_DIGICERT_OPENSSL_LIB_3_5__ */

static int DIGI_EVP_digests(ENGINE *e, const EVP_MD **digest,
			   const int **nids, int nid)
{
		 ...
		 unsigned char *to, RSA *rsa, int padding)
{
    ...
    /* Validate that input < modulus n */
    {
        BIGNUM *f = BN_new();
        if (f == NULL)
        {
            RSAerr(VERSION_RSA_F_RSA_PRIVATE_DECRYPT, ERR_R_BN_LIB);
            return rval;
        }
...
exit2:
        ...
        case RSA_NO_PADDING:
            {
#ifdef __ENABLE_DIGICERT_CRYPTO_INTERFACE__
            /* For RSA_NO_PADDING, input must be left-padded to modulus size */
            buf = OPENSSL_malloc(num);
            if (NULL == buf)
                break;
            ...
            if (flen < num)
            {
                DIGI_MEMSET(buf, 0, num - flen);
                DIGI_MEMCPY(buf + num - flen, from, flen);
            }
            else
            {
                DIGI_MEMCPY(buf, from, num);
            }

            if (OK > (status = CRYPTO_INTERFACE_RSA_applyPrivateKey(
                    pRSAKey, RANDOM_FUN, RANDOM_CTX, buf,
                    num, &pOutput, &pVlongQueue, asymKey.type)))
            {
                OPENSSL_free(buf);
                break;
	        ...
	        if (OK > (status = VLONG_vlongFromByteString(buf, num, &pEncrypted, &pVlongQueue)))
            {
                OPENSSL_free(buf);
	            break;
            }
...
#ifdef __ENABLE_DIGICERT_CRYPTO_INTERFACE__
static ECCKey *DIGI_EVP_opensslEC2DIGI(
    EC_KEY *pEcKey
    )
{
    MSTATUS status = OK;
...
#else
static ECCKey*
DIGI_EVP_opensslEC2DIGI(EC_KEY *eckey)
{
    ECCKey    * pNewKey = 0;
    MSTATUS	status = OK;
    unsigned char     * to;
    ...
    )
{
    ...
    {
        keyType = akt_ecc;
        pKey = DIGI_EVP_opensslEC2DIGI(pEcKey);
        if (NULL == pKey)
            return NULL;
    }
               ...
               EC_KEY *eckey)
{
    ...
    if (NULL == (pECCKey = DIGI_EVP_opensslEC2DIGI(eckey))) {
        return NULL;
    }
...
static DSAKey *
DIGI_EVP_opensslDSA2DIGI(DSA *dsa)
{
#ifdef __ENABLE_DIGICERT_CRYPTO_INTERFACE__
...
DSA_SIG *moc_dsa_do_sign(const unsigned char *dgst, int dlen, DSA *dsa)
{
    ...
    if (NULL == (pDSAKey = DIGI_EVP_opensslDSA2DIGI(dsa)))
	return NULL;
...
DIGI_EVP_opensslDH2DIGI(const BIGNUM *pub_key, DH *dh, vlong **ppVlongQueue)
{
...
static int moc_compute_dh_key(unsigned char *key, const BIGNUM *pub_key, DH *dh)
{
    ...
    if (NULL == (pDHCtx = DIGI_EVP_opensslDH2DIGI(pub_key, dh, &pVlongQueue)))
	goto exit;
...
#endif /* if defined(__ENABLE_DIGICERT_OPENSSL_LIB_3_5__) */
                           ...
                           const int **nids, int nid)
{
    ...
    {
        DIGI_registerEcx25519Meth(pmeth);
        return 1;
    }
    else
#endif
...
exit:
    ...
    return ret;
     ...
     defined(VERSION_1_1_0_OR_1_1_1C_OR_3_0_OR_3_5)
...
#endif /* !OPENSSL_NO_EC && VERSION_1_1_0_OR_1_1_1C_OR_3_0_OR_3_5 */
...
static int bind_moc_evp(ENGINE *e)
{
    ...
    if (!ENGINE_set_id(e, engine_moc_evp_id)
        ...
        || !ENGINE_set_destroy_function(e, DIGI_EVP_engineDestroy)
        || !ENGINE_set_init_function(e, DIGI_EVP_engineInit)
        || !ENGINE_set_finish_function(e, DIGI_EVP_engineFinish)) {
    ...
    if (!ENGINE_set_digests(e, DIGI_EVP_digests))
    {
        MOC_EVPerr(MOC_EVP_F_BIND, MOC_EVP_R_INIT_FAILED);
        return 0;
    }
