 ...
 *
 * AES keywrap implementations for OSSL 3.5 provider ADAPTED FROM OpenSSL code
 *
...
static void *digiprov_aes_wrap_dupctx(void *vctx, unsigned int mode)
{
    MSTATUS status = OK;
    DP_CIPHER_CTX *ctx = (DP_CIPHER_CTX *)vctx;
    DP_CIPHER_CTX *dupctx = NULL;
    ...
    *dupctx = *ctx;
    ...
    if (!digiprov_aes_set_mode(dupctx, dupctx->keylen * 8, dupctx->ivlen * 8, mode))
    {
        digiprov_aes_wrap_freectx(dupctx);
        return NULL;
    }

    /* Copy the nested EVP context state */
    if (!DIGI_EVP_CIPHER_copyCtx(ctx->pEvpCtx, dupctx->pEvpCtx))
    {
        digiprov_aes_wrap_freectx(dupctx);
        return NULL;
    }
...
#define IMPLEMENT_cipher(mode, fname, UCMODE, flags, kbits, blkbits, ivbits)   \
    ...
    static OSSL_FUNC_cipher_dupctx_fn aes_##kbits##fname##_dupctx;             \
    static void *aes_##kbits##fname##_dupctx(void *vctx)                       \
    {                                                                          \
        return digiprov_aes_##mode##_dupctx(vctx, EVP_CIPH_##UCMODE##_MODE);  \
    }                                                                          \
    const OSSL_DISPATCH digiprov_##aes##kbits##fname##_functions[] = {             \
        { OSSL_FUNC_CIPHER_NEWCTX,                                             \
            (void (*)(void))aes_##kbits##fname##_newctx },                     \
        { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))aes_##kbits##fname##_dupctx }, \
