 ...
 * digi_ciphercommon.c
 *
 * Provider for OSSL 3.5 Adapted from OpenSSL provider code.
 *
                                        ...
                                        const OSSL_PARAM params[], int enc)
{
            ...
            || ctx->mode == EVP_CIPH_OFB_MODE))
    {
        ...
        /* Also reset the working IV in the underlying Mocana context so that
         * algorithms like RFC 3211 key-unwrap (used by CMS PWRI) that call
         * EVP_DecryptInit_ex(ctx, NULL, NULL, NULL, NULL) to restore the
         * original IV before a final decrypt pass get the correct IV. Without
         * this, DIGI_EVP_doCipher would use the stale wIv from the previous
         * decrypt operation, producing garbage and failing the integrity check. */
        if (NULL != ctx->pEvpCtx && !ctx->isRc5)
        {
            MOC_EVP_CIPHER_CTX *pMocCtx = (MOC_EVP_CIPHER_CTX *)ctx->pEvpCtx->cipher_data;
            if (pMocCtx != NULL)
                DIGI_MEMCPY(pMocCtx->wIv, ctx->oiv, ctx->ivlen);
        }
    }
...
int digiprov_cipher_newevp(EVP_CIPHER_CTX **ppCtx)
{
    ...
    pCipher->ctx_size = sizeof(MOC_EVP_CIPHER_CTX);
...
void *digiprov_cipher_generic_dupctx(void *vctx)
{
    ...
    *pRet = *pShell;
    ...
    if (pShell->pEvpCtx && pShell->pEvpCtx->cipher)
    {
        nid = EVP_CIPHER_get_nid(pShell->pEvpCtx->cipher);

        /* Check if this is an AES cipher that uses the generic digiprov_aes_set_mode.
         * We check both NID ranges (to identify AES ciphers) AND mode.
         * Note: AES NIDs are not contiguous, they span multiple ranges. */
        is_aes_cipher = ((nid >= NID_aes_128_ecb && nid <= NID_aes_256_cfb128) ||  /* 418-429: ECB/CBC/OFB/CFB */
                             (nid >= NID_id_aes128_wrap && nid <= NID_id_aes256_wrap) ||  /* 788-790: WRAP */
                             (nid >= NID_aes_128_ctr && nid <= NID_aes_256_ctr) ||  /* 904-906: CTR */
                             (nid >= NID_aes_128_xts && nid <= NID_aes_256_xts));   /* 913-914: XTS */

        is_supported_mode = (pRet->mode == EVP_CIPH_ECB_MODE ||
                                 pRet->mode == EVP_CIPH_CBC_MODE ||
                                 pRet->mode == EVP_CIPH_OFB_MODE ||
                                 pRet->mode == EVP_CIPH_CFB_MODE ||
                                 pRet->mode == EVP_CIPH_CTR_MODE ||
                                 pRet->mode == EVP_CIPH_XTS_MODE ||
                                 pRet->mode == EVP_CIPH_WRAP_MODE);

        if (is_aes_cipher && is_supported_mode)
        {
            if (!digiprov_aes_set_mode(pRet, pRet->keylen * 8, pRet->ivlen * 8, pRet->mode))
            {
