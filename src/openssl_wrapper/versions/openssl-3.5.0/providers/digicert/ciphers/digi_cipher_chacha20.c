...
static void *digiprov_chacha20_dupctx(void *vctx)
{
    MSTATUS status = OK;
    DP_CIPHER_CTX *ctx = (DP_CIPHER_CTX *)vctx;
    ...
    *dupctx = *ctx;
    ...
    dupctx->pEvpCtx->key_len = CHACHA20_KEYLEN;
    dupctx->pEvpCtx->iv_len = CHACHA20_IVLEN;

    if (dupctx->pEvpCtx->cipher != NULL)
        ((EVP_CIPHER *)dupctx->pEvpCtx->cipher)->nid = NID_chacha20;

    /* Copy the nested EVP context state */
    if (!DIGI_EVP_CIPHER_copyCtx(ctx->pEvpCtx, dupctx->pEvpCtx))
    {
        digiprov_chacha20_freectx(dupctx);
        return NULL;
    }
...
const OSSL_DISPATCH digiprov_chacha20_functions[] =
{
    ...
    { OSSL_FUNC_CIPHER_DUPCTX,              (void (*)(void))digiprov_chacha20_dupctx },
