 ...
 *
 * AES-GCM implementations for OSSL 3.5 provider ADAPTED FROM OpenSSL code
 *
...
static void *digiprov_aes_gcm_dupctx(void *vctx)
{
    MSTATUS status = OK;
    DP_GCM_CTX *ctx = (DP_GCM_CTX *)vctx;
    DP_GCM_CTX *dupctx = NULL;
    ...
    status = DIGI_CALLOC((void **)&dupctx, 1, sizeof(*dupctx));
    if (OK != status)
        return NULL;

    *dupctx = *ctx;

    /* Set pEvpCtx to NULL to avoid double-free */
    dupctx->pEvpCtx = NULL;

    if (!digiprov_cipher_newevp(&dupctx->pEvpCtx))
    {
        DIGI_MEMSET_FREE((ubyte **)&dupctx, sizeof(*dupctx));
        return NULL;
    }

    /* Initialize the cipher mode on the duplicate context */
    if (!digiprov_gcm_set_mode(dupctx->pEvpCtx, dupctx->keylen * 8, 96))
    {
        digiprov_aes_gcm_freectx(dupctx);
        return NULL;
    }

    dupctx->pEvpCtx->encrypt = ctx->pEvpCtx->encrypt;
    dupctx->pEvpCtx->buf_len = ctx->pEvpCtx->buf_len;
    dupctx->pEvpCtx->num = ctx->pEvpCtx->num;
    DIGI_MEMCPY(dupctx->pEvpCtx->oiv, ctx->pEvpCtx->oiv, sizeof(ctx->pEvpCtx->oiv));
    DIGI_MEMCPY(dupctx->pEvpCtx->iv, ctx->pEvpCtx->iv, sizeof(ctx->pEvpCtx->iv));
    DIGI_MEMCPY(dupctx->pEvpCtx->buf, ctx->pEvpCtx->buf, sizeof(ctx->pEvpCtx->buf));

    /* Copy the internal GCM state (cipher_data, IV, AAD, GCM context) */
    if (!DIGI_EVP_AES_GCM_cipherCtxCtrl(ctx->pEvpCtx, EVP_CTRL_COPY, 0, dupctx->pEvpCtx))
    {
        digiprov_aes_gcm_freectx(dupctx);
        return NULL;
    }
