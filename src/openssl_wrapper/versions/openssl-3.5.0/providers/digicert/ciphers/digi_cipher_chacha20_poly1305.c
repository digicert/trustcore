...
static void *digiprov_chacha20_poly1305_dupctx(void *vctx)
{
    MSTATUS status = OK;
    DP_CHACHAPOLY_CTX *ctx = (DP_CHACHAPOLY_CTX *)vctx;
    DP_CHACHAPOLY_CTX *dupctx = NULL;
    ...
    *dupctx = *ctx;
    ...
    /* Set ChaCha20 parameters on the duplicate EVP context */
    dupctx->pEvpCtx->key_len = CHACHA20_POLY1305_KEYLEN;
    dupctx->pEvpCtx->iv_len = CHACHA20_POLY1305_IVLEN;

    if (dupctx->pEvpCtx->cipher != NULL)
        ((EVP_CIPHER *)dupctx->pEvpCtx->cipher)->nid = NID_chacha20_poly1305;

    /* Copy the nested EVP context state using ChaCha20-Poly1305's own copy handler */
    if (!DIGI_EVP_CHACHAPOLY_ctrl(ctx->pEvpCtx, EVP_CTRL_COPY, 0, dupctx->pEvpCtx))
    {
        digiprov_chacha20_poly1305_freectx(dupctx);
        return NULL;
    }
...
const OSSL_DISPATCH digiprov_chacha20_poly1305_functions[] =
{
    ...
    { OSSL_FUNC_CIPHER_DUPCTX,              (void (*)(void))digiprov_chacha20_poly1305_dupctx },
