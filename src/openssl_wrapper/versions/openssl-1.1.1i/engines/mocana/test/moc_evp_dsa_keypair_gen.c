/*
 * moc_evp_dsa_keypair_gen.c
 *
 * Test code for DSA key pair generation
 *
...
int main(int argc, char *argv[])
{
	...
	if(NULL == (pctx = EVP_PKEY_CTX_new_id(EVP_PKEY_DSA, NULL))) 
		printf("Error in DSA key generation\n");
	...
	if(NULL == (kctx = EVP_PKEY_CTX_new(params, NULL))) 
		printf("Error in DSA key generation 1 \n");

	/* Generate the key */
	if(1 != EVP_PKEY_keygen_init(kctx)) 
		printf("Error in DSA key generation 2\n");

	if (1 != EVP_PKEY_keygen(kctx, &pkey)) 
		printf("Error in DSA key generation 3\n");
	/* Test */
