 ...
 * moc_evp_3des_test.c
 *
 * Test program for 3DES EVP cipher implementation
 *
...
static int test_mocana_3des_cbc(ENGINE *e)
{
    EVP_CIPHER_CTX *ctx;
    const EVP_CIPHER *cipher = EVP_des_ede3_cbc();
    unsigned char key[32] = "0011223300112ee30011223300112233";
    unsigned char iv[8] = "00112233";
    unsigned char text[32] = "The eagle flies at midnight.1234";
    

    unsigned char ebuf[40];
    ...
    } else if (!EVP_CipherUpdate(ctx, ebuf, &encl, text, 32)) {
...
static int test_mocana_3des_ecb(ENGINE *e)
{
    EVP_CIPHER_CTX *ctx;
...
int main(int argc, char **argv)
{
    ...
    if (test_mocana_3des_cbc(e) == 0) {
        ret = 1;
        goto end;
    } else {
		if (verbose) printf("3des_cbc PASSED\n");
    }
    if (test_mocana_3des_ecb(e) == 0) {
        ret = 1;
        goto end;
    } else {
		if (verbose) printf("3des_ecb PASSED\n");
    }
    printf("PASS\n");
end:
    ENGINE_free(e);
