...
static int test_mocana_des_cbc(ENGINE *e)
{
    EVP_CIPHER_CTX *ctx;
...
static int test_mocana_des_ecb(ENGINE *e)
{
    EVP_CIPHER_CTX *ctx;
...
int main(int argc, char **argv)
{
    ...
    if (test_mocana_des_cbc(e) == 0) {
        ret = 1;
		goto end;
    } else {
	    if (verbose) printf("des_cbc PASSED\n");
    }
    if (test_mocana_des_ecb(e) == 0) {
        ret = 1;
        goto end;
    } else {
	    if (verbose) printf("des_ecb PASSED\n");
    }
    printf("PASS\n");
end :
    ENGINE_free(e);
