...
setDefaultArguments()
{
    ...
    const char defaultCaFile[] = "ClientRSACert.pem";
    const char defaultKeyFile[]  = "RSACertCAKey.pem";
    const char defaultCertFile[]   = "RSACertCA.pem";
...
int main(int argc, char *argv[])
{
  ...
  * ---------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------- *
   * Set required bio controls                                  *
   * ---------------------------------------------------------- */

  BIO_ctrl (bio, BIO_CTRL_DGRAM_MTU_DISCOVER, 0, NULL);

  /* ---------------------------------------------------------- *
   * Set DTLS required options                                  *
   * ---------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------- *
   * Listen for client hello messages                           *
   * ---------------------------------------------------------- */

  client_addr.sin_family = AF_INET;
  while (1 > (ret=DTLSv1_listen(ssl, (BIO_ADDR *)&client_addr)))
  {
    if (ret < 0) /* fatal error occured */
    {
      BIO_printf(outbio, "Error: DTLSv1_listen failed. %s %d\n",strerror(errno), errno);
      goto EXIT;
    }

    ret = getnameinfo((struct sockaddr *)&client_addr, sizeof(struct sockaddr), addrstr, 32, NULL, 0, NI_NUMERICHOST);
    if (ret < 0)
    {
        BIO_printf(outbio, "getnameinfo failed: %s\n", gai_strerror(ret));
        goto EXIT;
    }

    BIO_printf(outbio, "Error: DTLSv1_failed or no cookie from %s\n",addrstr);
  }
   ...
   * ---------------------------------------------------------- */
    ...
    {
        printf("Error: SSL_do_handshake failed %d\n", SSL_get_error(ssl, ret));
        BIO_printf(outbio, "Error: Could not build a SSL session. %s %d\n",strerror(errno), errno);
        goto EXIT;
    }
    else
    {
...
WRITE:
    sprintf((char*)body, "<html><head><title>Good Bye!</title></head>"
                    "<body>Good Bye!</body></html>");

    sprintf((char*)reply,
        "HTTP/1.0 200 0\r\nContent-type: text/html\r\nContent-length: %d\r\n\r\n%s\r\n\r\n",
        (int)strlen(body), body);
    retval = SSL_write(ssl,reply,strlen(reply));
