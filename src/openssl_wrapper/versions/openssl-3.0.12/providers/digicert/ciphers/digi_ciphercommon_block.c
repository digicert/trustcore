/*
 * digi_ciphercommon_block.c
 *
 * Common block handling code for block ciphers. ADAPTED FROM OPENSSL code.
 *
 ...
 */
size_t digiprov_cipher_fillblock(unsigned char *buf, size_t *buflen,
                                 size_t blocksize,
 ...
 */
int digiprov_cipher_trailingdata(unsigned char *buf, size_t *buflen, size_t blocksize,
                                 const unsigned char **in, size_t *inlen)
{
    if (*inlen == 0)
        return 1;
...
/*-
 * digiprov_cipher_tlsunpadblock removes the CBC padding from the decrypted, TLS, CBC
