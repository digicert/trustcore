...
extern MSTATUS
SSHC_OUT_MESG_sendMessageSize(sshClientContext *pContextSSH,
...
extern MSTATUS
SSHC_OUT_MESG_sendMessage(sshClientContext *pContextSSH,
    ...
    if (NULL != OUTBOUND_CIPHER_ALGORITHM(pContextSSH))
    {
        ...
        if (CHACHA20_POLY1305_OPENSSH == OUTBOUND_CIPHER_TYPE(pContextSSH))
        {
            status = (OUTBOUND_CIPHER_ALGORITHM(pContextSSH))(MOC_SYM(pContextSSH->hwAccelCookie) OUTBOUND_CIPHER_CONTEXT2(pContextSSH),
                                                              ...
                                                              4/* only encrypt length bytes */, 1, pSequenceNumOut);
        }
        else
#endif
#endif /* (defined(__ENABLE_DIGICERT_CHACHA20__) && defined(__ENABLE_DIGICERT_POLY1305__)) */
...
} /* SSHC_OUT_MESG_sendMessage */


/*------------------------------------------------------------------*/

extern MSTATUS
SSHC_OUT_MESG_allocStructures(sshClientContext *pContextSSH)
{
    MSTATUS status = OK;
...
extern MSTATUS
SSHC_OUT_MESG_deallocStructures(sshClientContext *pContextSSH)
{
    MSTATUS status = OK;
