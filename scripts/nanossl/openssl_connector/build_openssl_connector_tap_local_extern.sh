...
function show_usage
{
  ...
  echo "   --tap-deferred-unload - Build NanoSSL enabling deferred TAP Key Unload."
  echo ""
  exit -1
}
...
echo "***************************************************************"
echo "*** Building local TAP openssl connector with TAP Extern..."
echo "***************************************************************"
...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh $BUILD_OPTIONS $TAP_HYBRID_SIGN_OPTION $FIPS_OPTION $DISABLE_PQC_OPT --ssl --openssl${OSSL_VER} --tap --tpm2 --tap-extern $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT --tap --openssl $OCSP_OPTION $CERT_OPTION $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/tpm2 && ./build.sh $BUILD_OPTIONS --suiteb --openssl_shim
    cd ${MSS_PROJECTS_DIR}/smp_tpm2 && ./build.sh $BUILD_OPTIONS --suiteb --openssl_shim
    ...
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-local tap_extern
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT --tap --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $RSA1024_OPTION $SHA1_OPTION $TLS13_OPTION $OCSP_OPTION $DEFER_UNLOAD_OPTION $RSA8K_OPTION $NANOSSL_OSSL_OPTIONS $STRICT_DH_OPTION nanossl
    ...
    if [ "$pass" == "second" ]; then
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT $REDEFINE_OPTION --tap-local --tap-extern --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $OCSP_OPTION $NANOSSL_OSSL_OPTIONS openssl_shim_lib
    fi
...
make -f Makefile $FIPS_MAKE_OPTION tap=true suiteb=true tap_extern=true mauth=true $SAMPLE_CRYPTOINTERFACE_OPTION $SAMPLE_GDB_OPTION clean openssl_client_tap
if test "$?" != "0"; then
    echo "********************************"
