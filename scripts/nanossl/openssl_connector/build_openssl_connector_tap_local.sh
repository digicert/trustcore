...
function show_usage
{
  ...
  echo "   --no_tpm2         - Build without TPM2 provider."
...
do
    case "$1" in
        ...
        --ossl_config_cmd)
            ...
            ;;
        --no_tpm2)
            echo "Building without TPM2 provider..."
            TPM2_ARG=""
            ;;
        --disable-tcp-init)
...
echo "***************************************************************"
echo "*** Building local openssl shim version of CAP..."
echo "***************************************************************"
...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh $BUILD_OPTIONS $TAP_HYBRID_SIGN_OPTION $FIPS_OPTION $DISABLE_PQC_OPT --ssl --openssl${OSSL_VER} --tap $TPM2_ARG $RC5_DISABLE_CRYPTO_OPTION $STATIC_OPTION $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT --tap --openssl  $OCSP_OPTION $OCSP_CERT_OPTION $STATIC_OPTION $DISABLE_STRICT_CA_CHECK_OPTION $DISABLE_CERT_EXT_CHECK_OPTION $RSA8K_OPTION
    if [[ "$TPM2_ARG" == "--tpm2" ]]; then
        cd ${MSS_PROJECTS_DIR}/tpm2 && ./build.sh $BUILD_OPTIONS --suiteb --openssl_shim $STATIC_OPTION
        cd ${MSS_PROJECTS_DIR}/smp_tpm2 && ./build.sh $BUILD_OPTIONS --suiteb --openssl_shim $STATIC_OPTION
    fi
    cd ${MSS_PROJECTS_DIR}/nanotap2_common && ./build.sh $BUILD_OPTIONS --tap-local $TPM2_ARG $STATIC_OPTION
    cd ${MSS_PROJECTS_DIR}/nanotap2_configparser && ./build.sh $BUILD_OPTIONS $STATIC_OPTION
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-local $TPM2_ARG $STATIC_OPTION nanotap2
    
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT --tap --openssl_shim $DTLS_OPTION $DTLS_SRTP_OPTION $OPENSSL_OPTION $RSA1024_OPTION $SHA1_OPTION $TLS13_OPTION $OCSP_OPTION $STATIC_OPTION $NANOSSL_OSSL_OPTIONS $STRICT_DH_OPTION $RSA8K_OPTION nanossl
    ...
    if [ "$pass" == "first" ]; then
        ...
        if [[ "$STATIC_OPTION" == "" ]]; then
            ...
            then
                make $DEBUG_OPTIONS $FIPS_MAKE_OPTION $REDEFINE_LIB_OPTION $RC5_DISABLE_OPTION $STRICT_DH_OPTION_OSSL cryptointerface=true tap=true build_libs
                cp ${MSS_DIR}/thirdparty/${OPENSSL_LIB_OPTION}/libcrypto.so* ${MSS_DIR}/${BIN_DIR}
            else
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $FIPS_OPTION $REDEFINE_OPTION $DISABLE_PQC_OPT --tap-local --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $OCSP_OPTION $STATIC_OPTION $NANOSSL_OSSL_OPTIONS openssl_shim_lib
    fi
...
if [[ "$STATIC_OPTION" == "" ]]; then
    ...
    make -f Makefile $FIPS_MAKE_OPTION tap=true suiteb=true mauth=true $SAMPLE_CRYPTOINTERFACE_OPTION $SAMPLE_GDB_OPTION clean openssl_client_tap
    if test "$?" != "0"; then
        echo "********************************"
