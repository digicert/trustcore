...
echo "***************************************************************"
echo "*** Building TAP local export openssl connector with TAP Extern..."
echo "***************************************************************"
...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh $BUILD_OPTIONS $TAP_HYBRID_SIGN_OPTION --tap-extern --disable-pqc --ssl --tap --tpm2 --openssl${OSSL_VER} --export --mbed --mbed-path "$MBED_PATH" $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS --disable-pqc --export --openssl --tap $OCSP_OPTION  $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/tpm2 && ./clean.sh && ./build.sh $BUILD_OPTIONS --openssl_shim
    cd ${MSS_PROJECTS_DIR}/smp_tpm2 && ./clean.sh && ./build.sh $BUILD_OPTIONS --openssl_shim
    ...
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-local --tpm2 --export nanotap2
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-local --export tap_extern
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS --export --disable-pqc --tap --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $RSA1024_OPTION $SHA1_OPTION $TLS13_OPTION $OCSP_OPTION $RSA8K_OPTION $NANOSSL_OSSL_OPTIONS nanossl
    ...
    if [ "$pass" == "second" ]; then
        ...
        then
            make $DEBUG_OPTIONS $REDEFINE_LIB_OPTION export=true cryptointerface=true tap=true build_libs
            cp ${MSS_DIR}/thirdparty/${OPENSSL_LIB_OPTION}/libcrypto.so* ${MSS_DIR}/${BIN_DIR}
        else
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $REDEFINE_OPTION --disable-pqc --tap-local --tap-extern --openssl_shim --export $DTLS_OPTION $OPENSSL_OPTION $OCSP_OPTION $NANOSSL_OSSL_OPTIONS openssl_shim_lib
    fi
...
# Build binaries
cd ${MSS_DIR}/thirdparty/${OPENSSL_LIB_OPTION}/sample
make -f Makefile tap=true suiteb=true debug=true tap_extern=true mauth=true export=true $SAMPLE_CRYPTOINTERFACE_OPTION $SAMPLE_GDB_OPTION clean openssl_client_tap
