...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh $BUILD_OPTIONS $TAP_HYBRID_SIGN_OPTION $FIPS_OPTION $DISABLE_PQC_OPT --ssl --openssl${OSSL_VER} --tap $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS $FIPS_OPTION $DISABLE_PQC_OPT --tap $OCSP_OPTION $RSA8K_OPTION
    ...
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-remote --openssl clientcomm
    ...
    if [ "$pass" == "second" ]; then
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $FIPS_OPTION $REDEFINE_OPTION $DISABLE_PQC_OPT $TAP_REMOTE_OPTION --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $OCSP_OPTION $NANOSSL_OSSL_OPTIONS openssl_shim_lib
    fi
...
make -f Makefile $FIPS_MAKE_OPTION $SAMPLE_CRYPTOINTERFACE_OPTION $SAMPLE_GDB_OPTION mauth=true tap=true tap_remote=true suiteb=true clean openssl_client_tap
