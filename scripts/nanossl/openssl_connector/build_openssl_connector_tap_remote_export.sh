...
echo "***************************************************************"
echo "*** Building remote openssl shim version of CAP..."
echo "***************************************************************"
# Build BASE
...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh $BUILD_OPTIONS $TAP_HYBRID_SIGN_OPTION --disable-pqc --ssl --openssl${OSSL_VER} --tap --export --mbed --mbed-path "$MBED_PATH" $RSA8K_OPTION
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS --disable-pqc --tap --export --openssl $OCSP_OPTION $RSA8K_OPTION
    ...
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS --disable-pqc --tap-remote --export --openssl_shim $DTLS_OPTION  $OPENSSL_OPTION $RSA1024_OPTION $SHA1_OPTION $TLS13_OPTION $OCSP_OPTION $RSA8K_OPTION $NANOSSL_OSSL_OPTIONS nanossl
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh $BUILD_OPTIONS --tap-remote --openssl --export clientcomm
    ...
    if [ "$pass" == "second" ]; then
        ...
        then
            make $DEBUG_OPTIONS $REDEFINE_LIB_OPTION tap=true cryptointerface=true export=true build_libs
            cp ${MSS_DIR}/thirdparty/${OPENSSL_LIB_OPTION}/libcrypto.so* ${MSS_DIR}/${BIN_DIR}
        else
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean --disable-pqc $BUILD_OPTIONS $REDEFINE_OPTION $TAP_REMOTE_OPTION --export --openssl_shim $DTLS_OPTION $OPENSSL_OPTION $OCSP_OPTION $NANOSSL_OSSL_OPTIONS openssl_shim_lib
    fi
...
make -f Makefile tap=true tap_remote=true mauth=true suiteb=true export=true $SAMPLE_CRYPTOINTERFACE_OPTION $SAMPLE_GDB_OPTION clean openssl_client_tap
