...
echo "***************************************************************"
echo "*** Building ssl server TAP (remote) version of CAP..."
echo "***************************************************************"
...
do
    ...
    cd ${MSS_PROJECTS_DIR}/crypto && ./clean.sh && ./build.sh $BUILD_OPTIONS $PSS_VAR_SALT_OPTION $TAP_HYBRID_SIGN_OPTION --disable-pqc --ssl --export --tap --mbed --mbed-path "${MBED_PATH}" &&
    cd ${MSS_PROJECTS_DIR}/initialize && ./build.sh $BUILD_OPTIONS &&
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh $BUILD_OPTIONS --disable-pqc --tap --export &&
    ...
    if [ "$pass" == "second" ]; then
        ...
        cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean $BUILD_OPTIONS $TLS13_OPTION --disable-pqc $TAP_REMOTE_OPTION --export $MAUTH_OPTION ssl_server
        if test "$?" != "0"; then
            echo "********************************"
