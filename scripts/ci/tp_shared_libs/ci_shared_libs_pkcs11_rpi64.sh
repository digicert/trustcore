...
function build_tap_off {
  ...
  do
    cd ${MSS_PROJECTS_DIR}/common && ./build.sh ${GDB_ARG} ${DATA_PROTECT_ARG} ${MSG_LOGGER_ARG} --debug --json --uri ${COMMON_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/platform && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/asn1 && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${CVC_ARG} --cms --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocap && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DES_ARG} ${EXPORT_ARG} ${MBED_ARG} ${MBED_PATH_ARG} "${MBED_PATH}" --ssl --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/initialize && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh ${GDB_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} ${DATA_PROTECT_ARG} ${DES_ARG} ${RC4_ARG} ${CVC_ARG} --ocsp --cmc --cms --json-verify ${EXPORT_ARG} --toolchain ${TOOLCHAIN} ${PROXY_ARG} --cert --status-log ${SCRAM_ARG}
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} ${GDB_ARG} ${EXPORT_ARG} --ocsp nanossl --toolchain ${TOOLCHAIN} --mauth ${PROXY_ARG}
    if [ ! -z "${DATA_PROTECT_ARG}" ]; then
        cd ${MSS_PROJECTS_DIR}/data_protection && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${EXPORT_ARG} --toolchain ${TOOLCHAIN}
    fi
  done
...
function build_tap_local {
  ...
  for pass in first second
  do
    cd ${MSS_PROJECTS_DIR}/common && ./build.sh ${GDB_ARG} ${DATA_PROTECT_ARG} ${MSG_LOGGER_ARG} --debug --json --uri ${COMMON_ARG} --toolchain ${TOOLCHAIN} --cmake-opt "-DCM_TAP_TYPE=LOCAL"
    cd ${MSS_PROJECTS_DIR}/platform && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/asn1 && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${CVC_ARG} --cms --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocap && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2_common && ./build.sh --pkcs11 --tap-local --tpm2 ${TEE_ARG} ${DEBUG_ARG} ${GDB_ARG} ${SUITEB_ARG} ${DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2_configparser && ./build.sh ${DEBUG_ARG} ${GDB_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh --pkcs11 --tap-local ${TEE_ARG} ${DEBUG_ARG} ${GDB_ARG} --tpm2 nanotap2 ${DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DES_ARG} --tap --tpm2 --ssl ${EXPORT_ARG} ${MBED_ARG} ${MBED_PATH_ARG} "${MBED_PATH}" --toolchain ${TOOLCHAIN}
    if [ ! -z "${DATA_PROTECT_ARG}" ]; then
        cd ${MSS_PROJECTS_DIR}/data_protection && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${EXPORT_ARG} --toolchain ${TOOLCHAIN}
    fi
    if [ ! -z "${TEE_ARG}" ]; then
        cd ${MSS_PROJECTS_DIR}/smp_tee && ./build.sh ${DEBUG_ARG} ${GDB_ARG} --tee --toolchain ${TOOLCHAIN}
    fi
    cd ${MSS_PROJECTS_DIR}/tpm2 && ./build.sh --pkcs11 ${SUITEB_ARG} ${DEBUG_ARG} ${GDB_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/smp_pkcs11 && ./build.sh ${SUITEB_ARG} ${DEBUG_ARG} ${GDB_ARG} ${PKCS11_ARG} --pkcs11-tools --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/initialize && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${DATA_PROTECT_ARG} ${TAP_DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh ${GDB_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} ${DATA_PROTECT_ARG} ${DES_ARG} ${RC4_ARG} ${CVC_ARG} --ocsp --tap --cms --cmc --json-verify ${EXPORT_ARG} --toolchain ${TOOLCHAIN} ${PROXY_ARG} --cert --status-log ${SCRAM_ARG}
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} ${GDB_ARG} ${EXPORT_ARG} --tap --ocsp nanossl --toolchain ${TOOLCHAIN} --mauth ${PROXY_ARG}
  done
...
function build_tap_remote {
  ...
  for pass in first second
  do
    cd ${MSS_PROJECTS_DIR}/common && ./build.sh ${GDB_ARG} ${DATA_PROTECT_ARG} ${MSG_LOGGER_ARG} --debug --json --uri ${COMMON_ARG} --toolchain ${TOOLCHAIN} --cmake-opt "-DCM_TAP_TYPE=REMOTE"
    cd ${MSS_PROJECTS_DIR}/platform && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/asn1 && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${CVC_ARG} --cms --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocap && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2_common && ./build.sh --pkcs11 ${DEBUG_ARG} ${GDB_ARG} ${SUITEB_ARG} --tap-remote ${DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2_configparser && ./build.sh ${GDB_ARG} ${DEBUG_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh --pkcs11 --tap-remote ${DEBUG_ARG} ${GDB_ARG} --tpm2 --tcp-close-msg clientcomm --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanotap2 && ./build.sh --tap-remote  ${DEBUG_ARG} ${GDB_ARG} nanotap2 ${DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/crypto && ./build.sh ${GDB_ARG} ${DEBUG_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DES_ARG} --tap --tpm2 --ssl ${EXPORT_ARG} ${MBED_ARG} ${MBED_PATH_ARG} "${MBED_PATH}" --toolchain ${TOOLCHAIN}
    if [ ! -z "${DATA_PROTECT_ARG}" ]; then
        cd ${MSS_PROJECTS_DIR}/data_protection && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${EXPORT_ARG} --toolchain ${TOOLCHAIN}
    fi
    ...
    cd ${MSS_PROJECTS_DIR}/initialize && ./build.sh ${DEBUG_ARG} ${GDB_ARG} ${DATA_PROTECT_ARG} ${TAP_DATA_PROTECT_ARG} --toolchain ${TOOLCHAIN}
    cd ${MSS_PROJECTS_DIR}/nanocert && ./build.sh ${GDB_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} ${DATA_PROTECT_ARG} ${DES_ARG} ${RC4_ARG} ${CVC_ARG} --ocsp --tap --cms --cmc --json-verify ${EXPORT_ARG} --toolchain ${TOOLCHAIN} ${PROXY_ARG} --cert --status-log ${SCRAM_ARG}
    cd ${MSS_PROJECTS_DIR}/nanossl && ./build.sh --clean ${GDB_ARG} ${PQC_ARG} ${SUITEB_ARG} ${DEBUG_ARG} --tap --tap-remote ${EXPORT_ARG} --ocsp nanossl --toolchain ${TOOLCHAIN} --mauth ${PROXY_ARG}
  done
