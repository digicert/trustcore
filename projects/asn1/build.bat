...
:argactionstart
    ...
    if "%~1" == "--forcelink" (
        echo Settings flags to force linkage ...
...
:show_usage
    echo.
    echo build.bat --gdb --debug --libtype ^<static^|shared^> --cms --cvc ^<MAKETARGETS^>
    echo.
...
REM function set_toolchain_file()
REM {
REM     case "$1" in
REM         rpi32)
REM             echo "-- Setting toolchain for Raspberry Pi 32-bit";
REM             TOOLCHAIN_FILE="-DCMAKE_TOOLCHAIN_FILE=../shared_cmake/toolchains/arm-linux-gnu-toolchain.cmake"
REM             CMAKE_MOCANA_PLATFORM_NAME="-DCMAKE_MOCANA_PLATFORM=rpi3_raspbian_9.4"
REM             ;;
REM         rpi64)
REM             echo "-- Setting toolchain for Raspberry Pi 64-bit";
REM             TOOLCHAIN_FILE="-DCMAKE_TOOLCHAIN_FILE=../shared_cmake/toolchains/aarch64-linux-gnu-toolchain.cmake"
REM             CMAKE_MOCANA_PLATFORM_NAME="-DCMAKE_MOCANA_PLATFORM=rpi3_raspbian_9.4"
REM             ;;
REM         bbb)
REM             echo "-- Setting toolchain for BeagleBone Black";
REM             TOOLCHAIN_FILE="-DCMAKE_TOOLCHAIN_FILE=../shared_cmake/toolchains/arm-linux-gnu-toolchain.cmake"
REM             CMAKE_MOCANA_PLATFORM_NAME="-DCMAKE_MOCANA_PLATFORM=bbb_ubuntu_16.04"
REM             ;;
REM     esac
REM }
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: build_asn1 - Function to execute CMAKE files and build the asn1 library

:build_asn1
    call:echoIfVerbose %0:Starts
    set libAsn1FileName=%LIB_NAME%.%LIB_EXT%
    echo Building "%libAsn1FileName%"
    ...
    if "%WIN_BUILD_MODE%"=="VSIDE" (
        ...
        echo "!LIB_NAME! library built successfully at !BUILD_TYPE!\!libAsn1FileName!"
    ) else (
        ...
        if !ERRORLEVEL! NEQ 0 (
           echo Failure: NMake Failed to build "!libAsn1FileName!" from "Makefile"
           echo Failure: NMAKE exited with error !ERRORLEVEL!
           echo Refer file "!LOG_FILE!" for details.
           exit /b !ERRORLEVEL!
        )

        if NOT exist !libAsn1FileName! (
           echo Failure: NMake Failed to build "!libAsn1FileName!" from "Makefile"
           echo Refer file "!LOG_FILE!" for details.
           exit /b 2
        )
        echo "!LIB_NAME! library built successfully at %libAsn1FileName%"
    )
    ::popd
...
call:clean_asn1 & IF ERRORLEVEL 1 goto:end %errorlevel%

echo ********** Building Common library ********** >>%LOG_FILE%
call:build_asn1 & IF ERRORLEVEL 1 goto:end %errorlevel%
