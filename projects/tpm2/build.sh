...
PROJECT_NAME=libtpm2
######################
function show_usage
{
  echo ""
  echo "./build.sh --gdb --debug --openssl_shim --toolchain <string> <MAKETARGETS>"
  echo ""
  ...
  echo "   --tc              - Build with TrustCenter support."
  echo "   --pkcs11          - Build with SMP-PKCS11 support."
...
do
    case "$1" in
        ...
        --tpm12)
            ;;
        --tc)
            echo "Building with support for trust center";
            BUILD_OPTIONS+=" -DCM_ENABLE_TRUST_CENTER=ON"
            ;;
        --openssl_shim)
