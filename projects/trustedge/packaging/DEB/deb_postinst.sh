...
create_group ()
{
    # Create the 'trustedge' group if it does not exist
    if command -v groupadd &> /dev/null ; then
        getent group trustedge || groupadd --system trustedge
    elif command -v addgroup &> /dev/null ; then
        getent group trustedge || addgroup --system trustedge
    else
...
create_user ()
{
    # Create the 'trustedge' user if it does not exist
    if command -v useradd &> /dev/null ; then
        id -u "${USERNAME}" > /dev/null 2>&1 || useradd --system -s /bin/false -g trustedge -d ${INSTALL_DIR} "${USERNAME}"
    elif command -v adduser &> /dev/null ; then
        id -u "${USERNAME}" > /dev/null 2>&1 || adduser --system -s /bin/false -G trustedge -h ${INSTALL_DIR} "${USERNAME}"
    else
...
set_access_permission ()
{
    ...
    # Set access restrictions for installed files
...
install_service ()
{
  if [ -f "${SVC_FILE}" ]; then
    ...
    if command -v systemctl &> /dev/null ; then
        ...
        # versions older than 38 do not have journald. use syslog.
