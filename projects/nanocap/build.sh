...
if [[ "$unamestr" == 'Darwin' ]]; then
    SHARED_LIB_NAME=libnanocap.dylib
else
    SHARED_LIB_NAME=libnanocap.so
fi

printf "\n\nBuilding NanoCAP library.\n\n\n"
. clean.sh
mkdir build
cd build
is_static_lib=0
...
do
    case "$1" in
        ...
        --suiteb)
            ...
            ;;
        --ipv6) echo "Building with IPV6 enabled ...";
...
if [ $is_static_lib -eq 0 ]; then
    ...
    cp libs/libnanocap.a ../../../bin_static/
fi
