...
:argactionstart
    ...
    if "%~1" == "--suiteb" (
        echo suiteb is always enabled. Ignoring legacy --suiteb flag.
        goto next
    )
    if "%~1" == "--fips" (
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: clean_nanocap() - call this to clean the build targets and files
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: build_nanocap - Function to execute CMAKE files and build the nanocap library

:build_nanocap
    call:echoIfVerbose %0:Starts
    echo lib-name = %LIB_NAME%
    set libNanocapFileName=%LIB_NAME%.%LIB_EXT%
    echo Building "%libNanocapFileName%"
    ...
    if "%WIN_BUILD_MODE%"=="VSIDE" (
       ...
       call %CMAKE_BIN% --build . --config !BUILD_TYPE! 1>>!LOG_FILE! 2>>&1
        ...
        echo "!LIB_NAME! library built successfully at !BUILD_TYPE!\!libNanocapFileName!"
    ) else (
        ...
        if !ERRORLEVEL! NEQ 0 (
           echo Failure: NMake Failed to build "!libNanocapFileName!" from "Makefile"
           echo Failure: NMAKE exited with error !ERRORLEVEL!
           echo Refer file "!LOG_FILE!" for details.
           exit /b !ERRORLEVEL!
        )

        if NOT exist !libNanocapFileName! (
           echo Failure: NMake Failed to build "!libNanocapFileName!" from "Makefile"
           echo Refer file "!LOG_FILE!" for details.
           exit /b 2
        )

        echo "nanocap library built successfully at !libNanocapFileName!"
    )
    ::popd
    call:echoIfVerbose %0:Ends
exit /b 0
...
call:clean_nanocap & IF ERRORLEVEL 1 goto:end %errorlevel%

echo ********** Building nanocap library ********** >>%LOG_FILE%
call:build_nanocap & IF ERRORLEVEL 1 goto:end %errorlevel%
