...
PROJECT_NAME=mocpkcs11_tools
######################
function show_usage
{
  echo ""
  echo "./build.sh --gdb --debug [--pkcs11-dynamic] [--tap-local | --tap-remote] [--softhsm2 | --cloudhsm | --dssm | --pkcs11-tee] --data-protect --toolchain <string> <MAKETARGETS>"
  echo ""
  ...
  echo "   --data-protect    - Enable data protect"
  echo "   --tap-local       - Build with TAP-Local enabled."
  echo "   --tap-remote      - Build with TAP-Remote enabled."
  echo "   --pkcs11-dynamic  - Build with dynamic loading of PKCS11 libraries"
  echo "   --softhsm2        - Build with support for SoftHSM2."
  echo "   --cloudhsm        - Build with support for Amazon CloudHSM."
  echo "   --pkcs11-tee      - Build with support for TEE PKCS11."
  echo "   --dssm            - Build with support for Digicert SSM."
...
do
    case "$1" in
        ...
        --data-protect)
            ...
            BUILD_OPTIONS+=" -DCM_DATA_PROTECT=ON"
            ;;
        --tap-local)
