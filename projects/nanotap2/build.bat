...
set PROJECT_NAME_CLIENTCOMM=nanotap2_clientcomm
set IS_32BIT_BUILD=0
set IS_64BIT_BUILD=0
set ADD_ARGS=
set CMAKE_TARGET=
set IS_TAP_REMOTE_ENABLED=1
set IS_TAP_ENABLED=1
set IS_CLEAN_BUILD=0
set VS_PLATFORM=x64
set LIB_TYPE=shared
set DO_CLIENTCOMM=false
set DO_NANOTAP2=false
...
if %1==--tap-off (
 ...
 set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_ENABLE_TAP=OFF
 goto next
)
if %1==--tap-local (
...
if %1==--tcp-close-msg (
 ...
 set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_ENABLE_TCP_CLOSE_MSG=ON
 goto next
)
if %1==--fips (
 echo Linking to libmss.lib ...
 set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_ENABLE_FIPS=ON
 goto next
)
if %1==--nanotap2 (
 goto handle_nanotap2_arg
)
if %1==nanotap2 (
:handle_nanotap2_arg
 echo Build NANOTAP2 lib...
 set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_BUILD_TAP_LIB=ON
 set ADD_ARGS=%ADD_ARGS% nanotap2
 set DO_NANOTAP2=true
 goto next
)
if %1==--clientcomm (
 goto handle_clientcomm_arg
)
if %1==clientcomm (
:handle_clientcomm_arg
 echo Build NANOTAP2_CLIENTCOMM lib...
 set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_ENABLE_CLIENTCOMM=ON
 set ADD_ARGS=%ADD_ARGS% nanotap2_clientcomm
 set DO_CLIENTCOMM=true
 goto next
)
if "%1" == "--forcelink" (
...
if "-%ADD_ARGS%-"=="--" (
    ...
    set CMAKE_TARGET=%ADD_ARGS%
)

if %IS_TAP_ENABLED%==0 (
 if %IS_TAP_REMOTE_ENABLED%==1 (
...
:usage
  echo.
  echo    ./build.bat --help --gdb --debug --suiteb --libtype ^<static ^| shared^> [--x64 ^| --x32] --[ tap-local ^| tap-remote ] --forcelink --log ^<log_file^>
  echo.
  ...
  echo    --[ tap-local ^| tap-remote ] - Build client libraries / executables with specific tap functionality
...
:copyToBin
    ...
    if %DO_CLIENTCOMM%==true (
        echo Executing copy bat - %CopyBAT% --srcDir=!src_dir! --linkType=%LIB_TYPE% --binName=%PROJECT_NAME_CLIENTCOMM% --binType=lib
        call %CopyBAT% --srcDir=!src_dir! --linkType=%LIB_TYPE% --binName=%PROJECT_NAME_CLIENTCOMM% --binType=lib
    )
