...
set PROJECT_NAME=nanotap2_common
...
if %1==--tap-off (
    echo IGNORED TAP Off...
    goto next
)
if %1==--tap-local (
    echo IGNORED TAP Local...
    goto next
)
if %1==--tap-remote (
...
:usage
  echo.
  echo    "./build.bat --gdb --debug [--x64 | --x32] --libtype <static | shared> --forcelink --log <log file>"
  echo.
  ...
  echo    --clean          - Set this option to perform clean before build.
  echo    "--log <log file>- Dump compilation logs to specifed log file."
  echo.
EXIT /B 0
