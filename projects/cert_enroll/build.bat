...
:argactionstart
    ...
    if "%~1" == "--fips" (
        echo Linking to libmss for FIPS... 
    ...
    if "%~1" == "--mbed" (
        REM set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_ENABLE_MBED=ON
        goto next
    )
    if "%~1" == "--mbed-path" (
        REM set BUILD_OPTIONS=%BUILD_OPTIONS% -DCM_MBED_PATH=%2
        shift
        goto next
    )
    if "%~1" == "--tap" (
        echo Building with TAP support...
...
:show_usage
    echo.
    echo build.bat --gdb --debug --suiteb --libtype ^<static^|shared^> "--x32"^|"--x64" --forcelink --log ^<build-log-filepath^> ^<MAKETARGETS^>
    echo.
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: clean_cert_enroll() - call this to clean the build targets and files
...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:: build_cert_enroll - Function to execute CMAKE files and build the cert_enroll library

:build_cert_enroll
    call:echoIfVerbose %0:Starts
    echo lib-name = %LIB_NAME%
    set libcert_enrollFileName=%LIB_NAME%.%LIB_EXT%
    echo Building "%libcert_enrollFileName%"
    ...
    if "%WIN_BUILD_MODE%"=="VSIDE" (
        ...
        REM %CMAKE_BIN% %TOOLCHAIN_FILE% %CMAKE_MOCANA_PLATFORM_NAME% CMakeLists.txt ../. -DCMAKE_BUILD_TYPE=${BUILD_TYPE} %BUILD_OPTIONS%
        ...
        if !ERRORLEVEL! NEQ 0 (
           echo Failure: NMake Failed to build "!libcert_enrollFileName!" from "Makefile"
           echo Failure: NMAKE exited with error !ERRORLEVEL!
           echo Refer file "!LOG_FILE!" for details.
           exit /b !ERRORLEVEL!
        )

        if NOT exist !libcert_enrollFileName! (
           echo Failure: NMake Failed to build "!libcert_enrollFileName!" from "Makefile"
           echo Refer file "!LOG_FILE!" for details.
           exit /b 2
        )

        echo "cert_enroll library built successfully at !libcert_enrollFileName!"
    )
    ::popd
    call:echoIfVerbose %0:Ends
exit /b 0
...
call:clean_cert_enroll & IF ERRORLEVEL 1 goto:end %errorlevel%

echo ********** Building cert_enroll library ********** >>%LOG_FILE%
call:build_cert_enroll & IF ERRORLEVEL 1 goto:end %errorlevel%
