...
PROJECT_NAME=digicert_tools
######################
function show_usage
{
  echo ""
  echo "./build.sh --gdb --debug [--tap-local | --tap-remote] --data-protect --hw-accel --toolchain <string> <MAKETARGETS>"
  echo ""
  ...
  echo "   --monolithic      - Build monolithic binaries."
  echo "   --strip           - Strip symbols."
...
do
    case "$1" in
        ...
        --hw-accel)
            ...
            ;;
        --monolithic)
            echo "Building monolithic binaries...";
            BUILD_OPTIONS+=" -DLIB_TYPE:STRING=STATIC"
            ;;
        --strip)
            echo "Building with stripped symbols...";
            BUILD_OPTIONS+=" -DCM_ENABLE_STRIP=ON"
            ;;
        --libtype)
            case "$2" in
                static)
		            echo "Linking to static libraries...";
                    BUILD_OPTIONS+=" -DLIB_TYPE:STRING=STATIC"
                    ;;
                shared)
