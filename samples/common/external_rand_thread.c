...
/* NEEDS_JOIN is defined for now so that DIGICERT_initDigicert() can
...
#ifdef __ENABLE_DIGICERT_FORCE_DEV_RANDOM__
...
static MSTATUS DIGICERT_initDevRandom(ubyte4 read_thresh);
static MSTATUS DIGICERT_ReadRandomStoreWriteKernel(sbyte4 randFd, sbyte4 count);
static MSTATUS DIGICERT_RandomFileStoreClear(void);
static MSTATUS DIGICERT_RandomFileStoreWrite(ubyte4 val);
static int DIGICERT_RandomFileStoreIsFull(void);

static void *
externalRandomThreadStart(void *arg)
{
    (void)DIGICERT_addExternalEntropy(1);
 ...
 * to return early by calling DIGICERT_cancelExternalEntropy(). If it's called from a
 ...
 * through DIGICERT_addExternalEntropyThread(). If you do so, make sure that no other
 * thread calls this or DIGICERT_addExternalEntropyThread() concurrently
 *
 * @param
...
MSTATUS DIGICERT_addExternalEntropy(int async)
{
    MSTATUS status = OK;
    ubyte4 count = 384/32;
    int fd=-1, rval=-1;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        status = DIGICERT_addEntropy32Bits(randVal);
        if (OK != status)
        {
            goto exit;
        }
...
MSTATUS DIGICERT_addExternalEntropy(int async)
{
    ...
    while (!done)
    {
        ...
        bytes_read += rval;
        done = ((MAX_EXT_RAND_BYTES <= bytes_read) && (!async || (0 < DIGICERT_RandomFileStoreIsFull())));
    }
...
/**
 * @brief This function is called from DIGICERT_freeDigicert() to wait for the
 ...
 *
 * <p>NOTE! Do not call this function until you have called DIGICERT_initDigicert(or
 * related function).
 *
 ...
 *             returned error status, use the \c DISPLAY_ERROR macro.
 */
MSTATUS DIGICERT_waitForExternalEntropy(void)
{
#ifdef EXTERN_RAND_THREAD_NEEDS_JOIN
    void      * res;
...
/**
 * @brief This function is called from DIGICERT_freeDigicert() to indicate to a
...
MSTATUS DIGICERT_cancelExternalEntropy(void)
{
...
/**
 * @brief This is similar to DIGICERT_addExternalEntropy, except it does the
 ...
 * will call DIGICERT_addExternalEntropy. Once that function is complete, the
...
MSTATUS DIGICERT_addExternalEntropyThread(void)
{
    MSTATUS	status;
    int		rval;
    void      * res;
 ...
 *
 * @detais This is called in DIGICERT_initDigicert() to get gather entropy before
...
MSTATUS DIGICERT_addExternalEntropyThreadWait(void)
{
    MSTATUS status;
    status = DIGICERT_addExternalEntropyThread();
    if (OK > status)
	goto exit;
    status = DIGICERT_waitForExternalEntropy();
  exit:
    return status;
}
 ...
 */
static MSTATUS
DIGICERT_initDevRandom(ubyte4 read_thresh)
{
    FileDescriptor fp;
 ...
 */
static MSTATUS
DIGICERT_DevRandomAddEntropyFromBuffer(sbyte4 fd, void *buf, size_t size)
{
    struct {
	int ent_count;
	int size;
    ...
    entropy.size = size;
    DIGI_MEMCPY(entropy.data, buf, size);
 ...
 */
static int
DIGICERT_RandomFileStoreIsFull(void)
{
 ...
 */
static MSTATUS
DIGICERT_RandomFileStoreWrite(ubyte4 val)
{
    FileDescriptor fp;
    ubyte4 bytesWritten;

    if (0 < DIGICERT_RandomFileStoreIsFull())
	    return 1;
 ...
 */
static MSTATUS
DIGICERT_RandomFileStoreClear(void)
{
     FileDescriptor fp;
 ...
 */
static MSTATUS
DIGICERT_ReadRandomStoreWriteKernel(sbyte4 randFd, sbyte4 count)
{
    FileDescriptor     fp;
    FileDescriptorInfo fileInfo;
    int 	         len, act_len;
    ...
    status = DIGI_MALLOC((void **)&pBuf, len);
    ...
    if (OK == status)
    {
	    ...
	    if (act_len > 0)
	    {
	        status = DIGICERT_DevRandomAddEntropyFromBuffer(randFd, pBuf, act_len);
	    }
    }
...
#endif /* __DIGICERT_FORCE_ENTROPY__ */
