...
# dh group 20 (ecp384), ESP: AES-CCM-192

TC_NAME="Testcase 137 IKEv2 PSK / NanoSec->StrongSwan / dh group 20 (ecp384), ESP: AES-CCM-192"
TC_IKE_VERSION=2
...
tc_setup_policies() {
    ...
    cat > "$inf" <<EOF
{ laddr $i raddr $r } ipsec { encr_auth_algs any encr_algs any keylength 24 tag 12 }
EOF
    cat > "$sf" <<EOF
{ laddr $i raddr $r } ipsec { encr_auth_algs any encr_algs any keylength 24 tag 12 sa init }
EOF
}
