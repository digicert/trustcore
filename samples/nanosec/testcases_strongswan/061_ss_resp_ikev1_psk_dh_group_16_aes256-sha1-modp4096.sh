...
# dh group 16 (modp4096), ESP: AES-CBC-256 & SHA-1

TC_NAME="Testcase 61 IKEv1 PSK / NanoSec->StrongSwan / dh group 16 (modp4096), ESP: AES-CBC-256 & SHA-1"
TC_IKE_VERSION=1
...
connections {
    nanosec-interop {
        ...
        children {
            child1 {
                mode          = transport
                esp_proposals = aes256-sha1-modp4096
                local_ts      = $r/32
                remote_ts     = $i/32
            }
        }
