...
# dh group 2 (modp1024), ESP: AES-CTR-256 & SHA-256

TC_NAME="Testcase 129 IKEv2 PSK / NanoSec->StrongSwan / dh group 2 (modp1024), ESP: AES-CTR-256 & SHA-256"
TC_IKE_VERSION=2
TC_STRONGSWAN_ROLE="resp"   # StrongSwan is IKE responder
TC_SS_CONN_NAME="nanosec-interop"
TC_SS_CHILD_NAME="child1"

TC_INIT_IKE_FLAGS=(-p qatestingexample -g 2)
...
connections {
    nanosec-interop {
        ...
        proposals = aes256-sha512-modp768-modp1024
        children {
            child1 {
                mode          = transport
                esp_proposals = aes256ctr-sha256
                local_ts      = $r/32
                remote_ts     = $i/32
            }
        }
