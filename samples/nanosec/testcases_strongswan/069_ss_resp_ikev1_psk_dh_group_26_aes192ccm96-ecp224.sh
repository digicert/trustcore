...
# dh group 26 (ecp224), ESP: AES-CCM-192

TC_NAME="Testcase 69 IKEv1 PSK / NanoSec->StrongSwan / dh group 26 (ecp224), ESP: AES-CCM-192"
TC_IKE_VERSION=1
TC_STRONGSWAN_ROLE="resp"   # StrongSwan is IKE responder
TC_SS_CONN_NAME="nanosec-interop"
TC_SS_CHILD_NAME="child1"

TC_INIT_IKE_FLAGS=(-p qatestingexample -g 26)
...
connections {
    nanosec-interop {
        ...
        proposals = aes192-sha512-ecp224
        children {
            child1 {
                mode          = transport
                esp_proposals = aes192ccm96-ecp224
                local_ts      = $r/32
                remote_ts     = $i/32
            }
        }
