...
# dh group 21 (ecp521), ESP: BLOWFISH & AES-XCBC

TC_NAME="Testcase 138 IKEv2 PSK / NanoSec->StrongSwan / dh group 21 (ecp521), ESP: BLOWFISH & AES-XCBC"
TC_IKE_VERSION=2
...
connections {
    nanosec-interop {
        ...
        children {
            child1 {
                mode          = transport
                esp_proposals = blowfish256-aesxcbc
                local_ts      = $r/32
                remote_ts     = $i/32
            }
        }
...
tc_setup_policies() {
    ...
    cat > "$inf" <<EOF
{ laddr $i raddr $r } ipsec { encr_auth_algs aes encr_algs blowfish keylength 32 }
EOF
    cat > "$sf" <<EOF
{ laddr $i raddr $r } ipsec { encr_auth_algs aes encr_algs blowfish keylength 32 sa init }
EOF
}
