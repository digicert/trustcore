...
TC_NAME="Testcase 155 IKEv1 Cert / NanoSec->Strongswan / ECDSA Certificate"
TC_IKE_VERSION=1
TC_STRONGSWAN_ROLE="resp"
TC_SS_CONN_NAME="nanosec-interop"
TC_SS_CHILD_NAME="child1"

if ! tc_gen_certs tc_155 ecdsa ecdsa; then
...
connections {
    nanosec-interop {
        ...
        proposals = aes256-sha1-ecp256
        children {
            child1 {
                mode          = transport
                esp_proposals = aes192gcm96-ecp256
                local_ts      = $r/32
                remote_ts     = $i/32
            }
        }
