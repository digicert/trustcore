# IKEv2 EAP-TLS with RSA Cert and key in PEM format — Strongswan initiates as EAP supplicant, NanoSec responds
# as EAP authenticator (pubkey + eap-sim)

TC_NAME="Testcase 174 IKEv2 EAP-TLS with RSA Cert and key in PEM format / Strongswan(supplicant)->NanoSec(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_174 rsa rsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc174_ca_key="${REPO_DIR}/keystore/keys/tc_174_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_RESP_KEY" -subj "/CN=responder" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc174_ca_key" \
...
tc_setup_policies() {
    ...
    cp -r "$TC_RESP_KEY" "${REPO_DIR}/serverkey.der"
    cat > "$rf" <<EOF
flush;
spdflush;
