# IKEv2 EAP-TLS using ECDSA Cert Signed by RSA CA  — Strongswan initiates as EAP supplicant, NanoSec responds
# as EAP authenticator (pubkey + eap-tls)

TC_NAME="Testcase 177 EAP-TLS using ECDSA Cert Signed by RSA CA / Strongswan(supplicant)->NanoSec(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_177 ecdsa ecdsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc177_ca_key="${REPO_DIR}/keystore/keys/tc_177_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_RESP_KEY" -subj "/CN=responder" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc177_ca_key" \
