# IKEv2 EAP-TLS using RSA Cert and key in PEM format — NanoSec initiates as EAP supplicant, StrongSwan responds as EAP authenticator (pubkey + eap-tls)

TC_NAME="Testcase 161 IKEv2 EAP-TLS using RSA Cert and key in PEM format / NanoSec(supplicant)->StrongSwan(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_161 rsa rsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc161_ca_key="${REPO_DIR}/keystore/keys/tc_161_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_INIT_KEY" -subj "/CN=initiator" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc161_ca_key" \
...
tc_setup_policies() {
    ...
    cp -r "$TC_INIT_KEY" "${REPO_DIR}/clientkey.der"
