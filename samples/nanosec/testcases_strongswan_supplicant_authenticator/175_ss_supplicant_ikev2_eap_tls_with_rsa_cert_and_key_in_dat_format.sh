# IKEv2 EAP-TLS with RSA Cert and key in DAT format — Strongswan initiates as EAP supplicant, NanoSec responds
# as EAP authenticator (pubkey + eap-)

TC_NAME="Testcase 175 IKEv2 EAP-TLS with RSA Cert and key in DAT format / Strongswan(supplicant)->NanoSec(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_175 rsa rsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc175_ca_key="${REPO_DIR}/keystore/keys/tc_175_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_RESP_KEY" -subj "/CN=responder" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc175_ca_key" \
...
tc_setup_policies() {
    ...
    openssl rsa  -in "$TC_RESP_KEY"  -outform DER -out "${REPO_DIR}/key.dat"
    cp -r "${REPO_DIR}/key.dat" "${REPO_DIR}/serverkey.der"
    cat > "$rf" <<EOF
flush;
spdflush;
...
tc_teardown() {
    tc_cleanup_certs tc_175
    rm -f "${REPO_DIR}/server.der" "${REPO_DIR}/serverkey.der" "${REPO_DIR}/ca.der" "${REPO_DIR}/key.dat"
