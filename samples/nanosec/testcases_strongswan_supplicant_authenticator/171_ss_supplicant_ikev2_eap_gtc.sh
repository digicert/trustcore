# IKEv2 EAP-GTC — Strongswan initiates as EAP supplicant, NanoSec responds
# as EAP authenticator (pubkey + eap-gtc)

TC_NAME="Testcase 171 IKEv2 EAP-GTC / Strongswan(supplicant)->NanoSec(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if ! tc_gen_certs tc_171 rsa rsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc171_ca_key="${REPO_DIR}/keystore/keys/tc_171_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_RESP_KEY" -subj "/CN=responder" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc171_ca_key" \
