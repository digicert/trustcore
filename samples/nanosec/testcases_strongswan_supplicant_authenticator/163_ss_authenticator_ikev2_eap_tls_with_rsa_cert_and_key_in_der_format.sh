# IKEv2 EAP-TLS using RSA Cert and Key in DER format — NanoSec initiates as EAP supplicant, StrongSwan responds as EAP authenticator (pubkey + eap-tls)

TC_NAME="Testcase 163 IKEv2 EAP-TLS using RSA Cert and Key in DER format / NanoSec(supplicant)->StrongSwan(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_163 rsa rsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc163_ca_key="${REPO_DIR}/keystore/keys/tc_163_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_INIT_KEY" -subj "/CN=initiator" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc163_ca_key" \
...
tc_setup_policies() {
    ...
    openssl rsa  -in "$TC_INIT_KEY"  -outform DER -out "${REPO_DIR}/key.der"
    cp -r "${REPO_DIR}/key.der" "${REPO_DIR}/clientkey.der"
...
tc_teardown() {
    tc_cleanup_certs tc_163
    rm -f "${REPO_DIR}/ca.der" "${REPO_DIR}/client.der" "${REPO_DIR}/clientkey.der" "${REPO_DIR}/key.der"
