# IKEv2 EAP-TLS using ECDSA Cert with Unencrypted PKCS8 key — NanoSec initiates as EAP supplicant, StrongSwan responds as EAP authenticator (pubkey + eap-tls)

TC_NAME="Testcase 168 IKEv2 EAP-TLS using ECDSA Cert with Unencrypted PKCS8 key / NanoSec(supplicant)->StrongSwan(authenticator), esp:aes128-sha256-modp3072"
TC_IKE_VERSION=2
...
if [[ -z "$TC_SKIP_REASON" ]] && ! tc_gen_certs tc_168 ecdsa ecdsa; then
...
if [[ -z "$TC_SKIP_REASON" ]]; then
    _tc168_ca_key="${REPO_DIR}/keystore/keys/tc_168_ca.key"
    openssl x509 -req \
        -in <(openssl req -new -key "$TC_INIT_KEY" -subj "/CN=initiator" 2>/dev/null) \
        -CA "$TC_CA_CERT" -CAkey "$_tc168_ca_key" \
...
tc_setup_policies() {
    ...
    openssl pkey  -in "$TC_INIT_KEY"  -outform DER -out "${REPO_DIR}/clientkey.der"
...
tc_teardown() {
    tc_cleanup_certs tc_168
    rm -f "${REPO_DIR}/ca.der" "${REPO_DIR}/client.der" "${REPO_DIR}/clientkey.der"
