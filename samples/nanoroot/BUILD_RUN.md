...
| `--keyId`      | Input  | Yes      | Hexadecimal key identifier (required for sign/verify operations). Accepted as hex with or without the `0x` prefix. Use the canonical values from the supported algorithms table, for example RSA-2048 `0x02000001`, ECC P-384 `0x04000002`, and MLDSA-87 `0x13000008`. |
| `--signBuffer` | Flag   | Yes      | Operation mode flag indicating signature generation operation                |
| `--hashType`   | Input  | Yes      | Hash algorithm code: `0` (none), `1` (SHA-256), `2` (SHA-512), `3` (SHA-384) |
...
- RSA-2048, RSA-3072, and ECC P-256 use `--hashType 1` (SHA-256)
- ECC P-384 uses `--hashType 3` (SHA-384)
- RSA-4096, RSA-8192, and ECC P-521 use `--hashType 2` (SHA-512)

**KeyID Details:**

The `--keyId` parameter is a hexadecimal key identifier with the following characteristics:
- **Format**: Hexadecimal string accepted either as 8 hex digits (for example, `02000001`) or with a `0x` prefix (for example, `0x02000001`)
- **Length**: Represents a 4-byte (32-bit) key handle; accepted textual forms are 8 hex digits without the prefix or 10 characters including the optional `0x` prefix
- **Purpose**: Identifies the cryptographic algorithm and key size for signature operations
- **Reference values**: use the exact key IDs listed below. NanoROOT uses distinct identifiers per algorithm family and curve/size rather than a single contiguous prefix for all RSA, ECC, or MLDSA variants.
...
|------------|----------|---------------|-----------|-------------------------|
| RSA        | 2048-bit | `0x02000001` | `1`       | RSA 2K with SHA-256      |
| RSA        | 3072-bit | `0x04000001` | `1`       | RSA 3K with SHA-256      |
| RSA        | 4096-bit | `0x03000001` | `2`       | RSA 4K with SHA-512      |
| RSA        | 8192-bit | `0x05000001` | `2`       | RSA 8K with SHA-512      |
| ECC P-256  | 256-bit  | `0x03000002` | `1`       | P-256 curve with SHA-256 |
| ECC P-384  | 384-bit  | `0x04000002` | `3`       | P-384 curve with SHA-384 |
| ECC P-521  | 521-bit  | `0x05000002` | `2`       | P-521 curve with SHA-512 |
| MLDSA-44   | -        | `0x11000008` | `0`       | MLDSA44 (Post-Quantum)   |
| MLDSA-65   | -        | `0x12000008` | `0`       | MLDSA65 (Post-Quantum)   |
| MLDSA-87   | -        | `0x13000008` | `0`       | MLDSA87 (Post-Quantum)   |
...
**Parameters:**
- `--keyId 0x13000008`: MLDSA-87 algorithm identifier
